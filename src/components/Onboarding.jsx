...
export default function Onboarding({ step, setStep, onFinish }) {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      setError(`저장 실패: ${err?.message || '네트워크를 확인하고 다시 시도해주세요.'}`)
    }
    setSaving(false)
  }
    ...
    () => (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAFAFA] px-6">
        <div className="bg-white rounded-[32px] shadow-2xl p-12 max-w-lg w-full text-center">
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#F4A259] to-[#6366F1] mx-auto mb-6 flex items-center justify-center">
            <span className="text-white text-3xl font-black">A</span>
          </div>
          <p className="text-[11px] tracking-[0.25em] uppercase text-[#F4A259] font-bold mb-2">WELCOME TO</p>
          ...
          <button
            onClick={() => setStep(1)}
            className="w-full py-4 bg-[#F4A259] text-white rounded-[16px] font-bold text-sm hover:bg-[#6366F1] transition-all shadow-lg"
          >
            프로필 설정하기
          </button>
        </div>
    ...
    () => (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAFAFA] px-6">
        <div className="bg-white rounded-[32px] shadow-2xl p-10 max-w-lg w-full">
          <p className="text-[11px] tracking-[0.2em] uppercase text-[#F4A259] font-bold mb-1">WELCOME</p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                className="w-full mt-1 px-4 py-3 bg-white border border-gray-200 rounded-[12px] text-sm text-gray-900 outline-none"
                ...
                {professions.map((t) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-xs font-bold transition-all
                      ${profession === t ? 'bg-[#F4A259] text-white shadow-md' : 'bg-white border border-gray-200 text-gray-500 hover:bg-gray-50 shadow-sm'}`}
                  >
                    {t}
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="mt-1 w-full h-24 bg-white rounded-[16px] border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 text-xs cursor-pointer hover:border-[#F4A259] transition-colors">
                클릭하여 로고 업로드
              </div>
            </div>
          </div>

          <button onClick={handleFinish} disabled={saving} className="w-full mt-8 py-4 bg-[#F4A259] text-white rounded-[16px] font-bold text-sm hover:bg-[#6366F1] transition-all shadow-lg disabled:opacity-50">
            {saving ? '시작 중...' : '시작하기'}
          </button>
          {error && (
            <p className="mt-3 text-center text-xs text-red-500 font-semibold">{error}</p>
          )}
        </div>
      </div>
    ),
    ...
    () => (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAFAFA] px-6">
        <div className="bg-white rounded-[32px] shadow-2xl p-10 max-w-lg w-full">
          ...
          <p className="text-[11px] tracking-[0.2em] uppercase text-[#F4A259] font-bold mb-1">STEP 02</p>
          ...
          <div className="bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#FCAF45] rounded-[20px] p-[1px]">
            <button onClick={() => alert('Instagram 연동은 곧 지원됩니다. 나중에 설정에서 연결할 수 있어요.')} className="w-full bg-white rounded-[19px] py-5 flex items-center justify-center gap-3 hover:bg-gray-50 transition-all">
          ...
          <div className="mt-6 bg-white rounded-[16px] p-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] bg-[#181818] text-[#F4A259] px-3 py-1 rounded-full font-bold">연결 대기</span>
            </div>
          </div>

          <div className="flex gap-3 mt-8">
            <button onClick={() => setStep(1)} className="flex-1 py-4 bg-white text-gray-500 rounded-[16px] font-bold text-sm hover:bg-gray-200 transition-all">이전</button>
            <button onClick={handleFinish} disabled={saving} className="flex-1 py-4 bg-[#F4A259] text-white rounded-[16px] font-bold text-sm hover:bg-[#6366F1] transition-all shadow-lg disabled:opacity-50">{saving ? '시작 중...' : '시작하기'}</button>
          </div>
          <p onClick={handleFinish} className="text-center text-[11px] text-gray-400 mt-3 cursor-pointer hover:text-[#F4A259]">나중에 연결하기 →</p>
        </div>
      </div>
    ),
    ...
    () => (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAFAFA] px-6">
        <div className="bg-white rounded-[32px] shadow-2xl p-10 max-w-lg w-full">
          ...
          <p className="text-[11px] tracking-[0.2em] uppercase text-[#F4A259] font-bold mb-1">STEP 03</p>
          ...
          <div className="border-2 border-dashed border-[#F4A259] bg-[#181818] rounded-[20px] p-8 text-center hover:border-[#F4A259] transition-all cursor-pointer">
            <div className="w-16 h-16 bg-[#181818] rounded-[16px] mx-auto mb-4 flex items-center justify-center">
          ...
          <div className="mt-4 bg-white rounded-[16px] p-4 space-y-2">
          ...
          <div className="flex gap-3 mt-8">
            <button onClick={() => setStep(2)} className="flex-1 py-4 bg-white text-gray-500 rounded-[16px] font-bold text-sm hover:bg-gray-200 transition-all">이전</button>
            <button onClick={() => setStep(4)} className="flex-1 py-4 bg-[#F4A259] text-white rounded-[16px] font-bold text-sm hover:bg-[#6366F1] transition-all shadow-lg">AI 자동 정리 시작</button>
          </div>
        </div>
      </div>
    ),
    ...
    () => (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAFAFA] px-6">
        <div className="bg-white rounded-[32px] shadow-2xl p-10 max-w-xl w-full">
          ...
          <p className="text-[11px] tracking-[0.2em] uppercase text-[#F4A259] font-bold mb-1">STEP 04</p>
          ...
          <div className="space-y-3 max-h-[340px] overflow-y-auto pr-1">
            {[
              ...
              <div key={i} className="bg-white rounded-[16px] p-4 flex items-center gap-4 hover:shadow-md transition-all">
          ...
          <div className="flex items-center justify-between mt-6 text-xs text-gray-400">
            ...
            <span className="text-[#F4A259] cursor-pointer font-semibold">+ 수동으로 프로젝트 추가</span>
          </div>

          <div className="flex gap-3 mt-6">
            <button onClick={() => setStep(3)} className="flex-1 py-4 bg-white text-gray-500 rounded-[16px] font-bold text-sm hover:bg-gray-200 transition-all">이전</button>
            <button
              onClick={handleFinish}
              disabled={saving}
              className="flex-1 py-4 bg-[#F4A259] text-white rounded-[16px] font-bold text-sm hover:bg-[#6366F1] transition-all shadow-lg disabled:opacity-50"
            >
    ...
    <div className="flex items-center gap-1 mb-8">
      {labels.map((label, i) => (
        <div key={i} className="flex items-center gap-1 flex-1">
          <div className={`h-1.5 rounded-full flex-1 transition-all ${i <= current ? 'bg-[#F4A259]' : 'bg-gray-200'}`} />
        </div>
      ))}
    </div>
  )
