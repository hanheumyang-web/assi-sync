...
class SyncEngine {
  ...
  reconcileLocalDeletions() {
    ...
    const missing = tracked.filter(([relPath]) => !fs.existsSync(path.join(this.watchDir, relPath)))
    if (missing.length === 0) return { missing: 0 }

    /* 전부 또는 대부분이 사라졌다 = 폴더를 못 읽는 상황일 가능성이 크다.
       이럴 때 표시를 남기면 나중에 폴더가 돌아와도 영영 안 받는다. 손대지 않는다. */
    if (missing.length >= tracked.length * 0.9) {
      console.warn(`[reconcile] ${missing.length}/${tracked.length} 이 사라짐 — 폴더를 못 읽는 상황으로 보고 건너뛴다`)
      return { missing: missing.length, skipped: 'looks-like-unmounted' }
    }
    ...
    return { missing: missing.length, projects: byProject.size }
  }
