...
class SyncEngine {
  ...
  // 웹에서 soft-delete 된 프로젝트의 로컬 폴더 + state 정리.
  // 서버는 이미 deletedAt 박은 상태. 데스크탑은 그냥 흔적만 지움 (서버 호출 X).
  // idempotent — 폴더 이미 없어도 안전.
  async removeDeletedProjectLocally(projectId) {
    const entries = Object.entries(this.state.syncedFiles)
    ...
    for (const [relPath, entry] of entries) {
      ...
      try {
        ...
        console.warn(`[trash-cleanup] 파일 삭제 실패 ${abs}:`, e.message)
      }
      // 상위 폴더 (projectKey) 도 정리 대상으로 표시
      const parent = path.dirname(abs)
      if (parent && parent !== this.watchDir) folders.add(parent)
      delete this.state.syncedFiles[relPath]
    }

    // 빈 프로젝트 폴더 제거 (사용자 추가 파일 있으면 남김)
    for (const folder of folders) {
      try {
        if (fs.existsSync(folder) && fs.readdirSync(folder).length === 0) {
          fs.rmdirSync(folder)
          // 그 상위 (카테고리) 폴더도 비었으면 제거
        ...
        console.warn(`[trash-cleanup] 폴더 정리 실패 ${folder}:`, e.message)
      }
    }

    // projectCache 는 projectKey(폴더경로) → projectId 매핑. value 일치 항목 제거.
    for (const [key, val] of this.projectCache.entries()) {
      if (val === projectId) this.projectCache.delete(key)
    }
    this.saveState()
    console.log(`[trash-cleanup] 프로젝트 ${projectId} 로컬 정리 완료 (파일 ${entries.length}개)`)
  }
