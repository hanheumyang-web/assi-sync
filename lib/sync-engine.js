...
// [V2 Design] 디자인 소스 파일 — 원본은 바이너리로 저장, 썸네일 추출 시도
...
function isFigma(name) { return getExt(name) === 'fig' }
function isSupported(name) { return isImage(name) || isVideo(name) || isAudio(name) || isDesign(name) }
...
class SyncEngine {
  ...
  async syncFile(filePath, _fromBatch = false) {
    ...
    if (!isImg && !isVid && !isAud && !isFig) return
        ...
        try {
          ...
          try {
            ...
            if (thumbBuffer && thumbBuffer.length > 0) {
              const safeName = fileName.replace(/[^a-zA-Z0-9._-]/g, '_').replace(/\.fig$/i, '')
          ...
          if (thumbUrl && thumbStoragePath) {
            const thumbResult = await this.api.createAsset({
              ...
              fileName: fileName.replace(/\.fig$/i, '') + '.preview.png',
