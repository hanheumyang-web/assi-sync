...
class SyncEngine {
  ...
  async syncStateWithServer() {
    ...
    for (const proj of projects) {
      ...
      if (prev && !prev.deletedAt && proj.deletedAt) continue
      serverProjects.set(proj.name, { id: proj.id, name: proj.name, category: proj.category, deletedAt: proj.deletedAt || null })
    }
    ...
    for (const [key, folder] of localFolders) {
      ...
      if (parts.length >= 2) {
        const localCat = normalizeCategory(parts[0]) || parts[0]
        const serverCat = normalizeCategory(sp.category) || sp.category || ''
        if (localCat && localCat !== serverCat) {
          try {
            await this.api.updateProject(sp.id, { category: localCat })
            console.log(`[분류] 폴더에 맞춰 웹을 바꿨다: ${sp.name} ${serverCat || '(없음)'} → ${localCat}`)
          } catch (e) {
    ...
    for (const [key, folder] of localFolders) {
      ...
      if (sp.deletedAt) {
        try {
          ...
          const r = await this.api.findOrCreateProject(sp.name, cat)
