...
class SyncEngine {
  ...
  async downloadRemoteAsset(asset) {
    // V1: 이미지만. 영상/오디오/sourceFile 은 Phase 1 후반.
    if (asset.sourceFile || asset.isVideo || asset.type === 'audio') {
      console.log(`[Download skip] ${asset.fileName} (video/audio/sourceFile)`)
      return
    }
    ...
    if (!project) {
      console.warn(`[Download skip] ${asset.fileName} — getProject(${asset.projectId}) returned null`)
      return
    }
    ...
    console.log(`[Download start] ${asset.fileName} → ${project.category}/${project.name}`)
