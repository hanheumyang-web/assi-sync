...
class SyncEngine {
  ...
  async removeDeletedProjectLocally(projectInfo) {
    ...
    // 휴지통 폴더 — _Trash/{projectName} (YYYY-MM-DD HH-mm)/
    // 같은 이름 중복 시 카운터 (_2, _3...) 붙임. projectId 는 .meta.json 안에 보관.
    const d = new Date(deletedAt)
    const pad = (n) => String(n).padStart(2, '0')
    const stamp = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}-${pad(d.getMinutes())}-${pad(d.getSeconds())}`
    // 파일명 안전화 — slash, colon 등 제거
    ...
    let counter = 2
    while (fs.existsSync(trashProjectDir)) {
      trashProjectDir = path.join(trashRoot, `${baseName}_${counter}`)
      counter++
    }
    try {
