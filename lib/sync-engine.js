...
class SyncEngine {
  ...
  startDownloadPolling() {
    ...
    const tick = async () => {
      try {
        // bootstrap 모드면 hasMore 끝까지 한 tick 안에서 다 돌림 (사용자가 켜자마자 다 받음)
        const isBootstrap = !this.state.downloadSince
        let totalReceived = 0
        let safety = 0
        while (true) {
          safety++
          if (safety > 50) { // 25,000 자산까지. 이상 시 다음 tick 으로
          ...
          // cursor 모드 페이지네이션 — 더 받을 게 있으면 즉시 다음 페이지
          if (hasMore && nextCursor) {
            this.state.downloadCursor = nextCursor
            this.saveState()
            continue
          }
          // bootstrap 끝 → nextSince 박아서 다음 tick 부턴 incremental
          if (nextSince) {
        ...
        console.log(`[DownloadPoll] tick 완료 — 총 ${totalReceived} assets ${isBootstrap ? '(bootstrap)' : '(incremental)'}`)
      } catch (e) {
  ...
  async downloadRemoteAsset(asset) {
    // [v1.9.16] V1 영상/오디오/sourceFile 스킵 제거 — 사용자 자산 누락 방지.
    // url 있으면 종류 안 가리고 다운로드. sourceFile/Bunny URL 도 fetch 됨.
    // 큰 파일은 시간 걸리지만 누락보다 낫음.
    if (!asset.url || !asset.fileName || !asset.projectId) {
      console.log(`[Download skip] ${asset.fileName || asset.id} (missing url/fileName/projectId)`)
      return
    }
    // [Fix] assetId 가드 — 이미 이 기기에 있는 자산이면 즉시 return (경로 규칙 변경 루프 차단)
    for (const entry of Object.values(this.state.syncedFiles)) {
      if (entry?.assetId === asset.id) {
        console.log(`[Download skip] ${asset.fileName} (already in state.syncedFiles)`)
        return
      }
    }
    // [Fix] fileSize 가드 — null/0 이면 메타 미완성 (Bunny 인코딩 중 등) 으로 보고 다음 tick 으로
    if (asset.fileSize != null && asset.fileSize === 0) {
      console.log(`[Download skip] ${asset.fileName} (fileSize=0, incomplete)`)
      return
    }

    // 어느 로컬 폴더로 받을지 — 프로젝트 경로 역산
    // [v1.9.17] getProject 실패해도 폴백으로 받음 — 자산은 무조건 회수 우선.
    //   프로젝트 fetch 실패 (네트워크/권한/race) 시 _UNCATEGORIZED/{projectId} 로 받음.
    //   project.uid 검사 실패 시도 마찬가지 (서버에서 이미 uid 필터링한 결과니 신뢰 가능).
    const project = await this.api.getProject(asset.projectId).catch((e) => {
      console.warn(`[Download fallback] ${asset.fileName} — getProject 에러: ${e.message}`)
      return null
    })
    ...
    if (project) {
      if (project.uid !== this.uid) {
        // 다른 사용자 자산은 진짜 스킵 (보안)
        console.warn(`[Download skip] ${asset.fileName} — project.uid=${project.uid} !== this.uid=${this.uid}`)
        return
      }
      ...
      console.log(`[Download fallback] ${asset.fileName} → _UNCATEGORIZED/${asset.projectId}`)
    }
    console.log(`[Download start] ${asset.fileName} → ${category}/${projectName}`)
