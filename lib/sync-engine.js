...
class SyncEngine {
  ...
  restoreAssetFiles(assetIds) {
    ...
    for (const name of fs.readdirSync(trashRoot)) {
      ...
      try {
        if (left.length) fs.writeFileSync(metaPath, JSON.stringify({ ...meta, items: left }, null, 2))
  ...
  async removeDeletedProjectLocally(projectInfo) {
    ...
    for (const [relPath, entry] of entries) {
      ...
      if (entry.fileSize == null) continue        // 기록이 없으면 판단 못 하니 넘어간다
      let size = null
      ...
      for (const e of entries) {
        ...
        if (metaPid && metaPid === projectId) {
          try {
            ...
            console.warn(`[trash-restore-cleanup] 삭제 실패 ${dir}:`, err.message)
          }
        }
      }
