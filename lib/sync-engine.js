  ...
  async processShareUpload(share) {
    ...
    for (const asset of pending) {
      ...
      if (!localPath) {
        ...
        console.error(`[Share] Local file not found: asset=${asset.id} fileName=${asset.fileName} projectId=${share.projectId} syncedFiles=${syncedCount}`)
        this.onFileStatus({
          path: `share/${share.id}/${asset.fileName}`,
          status: 'failed',
          fileName: asset.fileName,
          error: `로컬 파일 없음 (synced: ${syncedCount})`,
        })
        continue
      }
