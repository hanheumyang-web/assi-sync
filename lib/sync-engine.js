...
class SyncEngine {
  ...
  startDownloadPolling() {
    ...
    // 첫 부팅: downloadSince null 유지 → 서버의 모든 자산 (since=0) 가져옴.
    // 본인이 이미 올린 자산은 assetId 가드 + contentHash 체크로 skip.
    // 첫 실행 시 다른 기기/웹/IG 에서 만든 기존 자산을 받아오는 핵심 분기.
    // 이후 nextSince 저장 → 다음 폴링부터 incremental.
    if (this.state.downloadSince === undefined) {
  ...
  async downloadRemoteAsset(asset) {
    ...
    if (asset.sourceFile || asset.isVideo || asset.type === 'audio') return
    ...
    for (const entry of Object.values(this.state.syncedFiles)) {
      if (entry?.assetId === asset.id) return
    }
