...
class SyncEngine {
  ...
  startDownloadPolling() {
    ...
    // [v1.9.14] 매 시작 시 downloadSince 강제 null — 옛 v1.9.12 가 박은 "now" 값 누적 방지.
    // assetId 가드 + contentHash 가드 가 본인 업로드 재다운로드 방어.
    // 한 번 fresh poll 돌고 nextSince 저장하면 그 다음부턴 incremental.
    this.state.downloadSince = null
    this.saveState()
    console.log('[DownloadPoll] 시작 — downloadSince reset (since=0)')

    const tick = async () => {
      try {
        const { assets, nextSince } = await this.api.listAssetsSince(this.state.downloadSince, 200)
        console.log(`[DownloadPoll] tick: ${assets?.length || 0} assets, nextSince=${nextSince}`)
