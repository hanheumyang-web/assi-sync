...
class SyncEngine {
  ...
  async syncFile(filePath, _fromBatch = false) {
    ...
    // ── Phase 2 W2 — viewer 격리 가드 ──
    // workspaceManager 있으면 파일의 워크스페이스 컨텍스트 + 내 role 확인.
    // viewer 면 업로드 금지 + .local-changes/ 로 복사 + 트레이 알림 (세션당 1회).
    if (this.workspaceManager) {
      try {
        const ctx = this.workspaceManager.resolveContext(filePath)
        if (ctx && ctx !== PERSONAL_CONTEXT) {
          const role = this.workspaceManager.getRole(ctx)
          if (role === 'viewer') {
            const root = this.workspaceManager.getWorkspaceRoot(ctx)
            isolateLocalChange({ filePath, workspaceRoot: root })
            if (this.viewerNotifier) this.viewerNotifier.record(ctx)
            console.log(`[viewer-guard] isolated ${relPath} (ws=${ctx})`)
            return
          }
        }
      } catch (e) {
        console.warn('[viewer-guard] check failed (proceeding as owner):', e.message)
      }
    }
