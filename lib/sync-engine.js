...
class SyncEngine {
  ...
  async removeDeletedProjectLocally(projectInfo) {
    ...
    // 휴지통 폴더 — _Trash/{projectId}__{deletedAtSafe}/
    const safeStamp = String(deletedAt).replace(/[:.]/g, '-')
    const trashRoot = path.join(this.watchDir, '_Trash')
    const trashProjectDir = path.join(trashRoot, `${projectId}__${safeStamp}`)
    try {
      if (!fs.existsSync(trashRoot)) fs.mkdirSync(trashRoot, { recursive: true })
      if (!fs.existsSync(trashProjectDir)) fs.mkdirSync(trashProjectDir, { recursive: true })
