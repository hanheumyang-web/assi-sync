...
class SyncEngine {
  ...
  // 새 구조: [분류]/[중간...]/[마지막=프로젝트]/file
  // 1차 폴더 = 분류, 중간 폴더는 _로 연결, 마지막 폴더 = 프로젝트명
  // depth 2: [project]/file → 프로젝트명 = parts[0]
  // depth 3: [category]/[project]/file → 프로젝트명 = parts[1]
  // depth 4+: [category]/[mid1]/[mid2]/[project]/file → 프로젝트명 = mid1_mid2_project
  getProjectName(filePath) {
    const parts = path.relative(this.watchDir, filePath).split(path.sep)
    if (parts.length < 2) return null
    if (parts.length === 2) return parts[0]
    // parts[0] = 분류, parts[1...-1] = 중간+프로젝트, parts[-1] = 파일
  ...
  async syncFile(filePath, _fromBatch = false) {
    ...
    try {
      ...
      if (isImg) {
        ...
        // Video: don't read entire file into memory (3GB+ would OOM)
      ...
      const url = isVid
        ? await this.api.uploadFileStream(filePath, storagePath, contentType, compressedSize)
      ...
      if (isVid) {
        ...
        const bunnyResult = await this.uploadToBunny(filePath, fileName, projectId, assetId, relPath, compressedSize)
  ...
  async checkPendingShares() {
    try {
      const { shares } = await this.api.getPendingShares()
      ...
      console.error('[Share] Polling error:', err.message)
    }
  }
  ...
  async processShareUpload(share) {
    ...
    console.log(`[Share] Processing share ${share.id}: ${pending.length} pending / ${assets.length} total`)
    ...
    for (const asset of pending) {
      // state에서 assetId로 로컬 파일 경로 역추적 (fallback: fileName + projectId, 파일 스캔)
      const localPath = this.findLocalPath(asset.id, asset.fileName, share.projectId)
      if (!localPath) {
        console.error(`[Share] Local file not found for asset ${asset.id} (${asset.fileName}) — syncedFiles keys:`, Object.keys(this.state.syncedFiles).slice(0, 5))
        continue
      }

      console.log(`[Share] Uploading ${asset.fileName} from ${localPath}`)
      ...
      try {
        ...
        // R2 presigned URL로 PUT 업로드
        const fileBuffer = fs.readFileSync(localPath)
        const uploadRes = await fetch(asset.uploadUrl, {
          method: 'PUT',
          headers: {
    ...
    if (uploadedCount === total) {
      ...
      console.error(`[Share] All uploads failed for share ${share.id}, marking as failed`)
      try {
