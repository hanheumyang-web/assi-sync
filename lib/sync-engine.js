// ASSI Sync Engine v2 — Secure edition
// No more Firebase Admin SDK or hardcoded API keys
// All operations go through the Vercel API backend

const chokidar = require('chokidar')
const sharp = require('sharp')
const path = require('path')
const fs = require('fs')
const crypto = require('crypto')
const { execFile } = require('child_process')
const os = require('os')

// ffmpeg / ffprobe 경로 (패키징 후에도 동작하도록 .replace 처리)
const ffmpegPath = require('ffmpeg-static').replace('app.asar', 'app.asar.unpacked')
const ffprobePath = require('ffprobe-static').path.replace('app.asar', 'app.asar.unpacked')
// ffprobe 가 이 컴퓨터에서 도는지: null=아직 모름, true=됨, false=안 됨(=ffmpeg 로 대신한다)
let ffprobe쓸수있나 = null


const MAX_DIM = 2048
const MAX_SIZE = 7 * 1024 * 1024
const JPEG_QUALITY = 92
const VIDEO_MAX_DIM = 3840  // 4K 한계 — Bunny Stream 최대 지원

const CONTENT_TYPE_MAP = {
  jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', gif: 'image/gif',
  webp: 'image/webp', heic: 'image/heic', heif: 'image/heif', bmp: 'image/bmp',
  tif: 'image/tiff', tiff: 'image/tiff', avif: 'image/avif',
  mp4: 'video/mp4', mov: 'video/quicktime', avi: 'video/x-msvideo', mkv: 'video/x-matroska',
  webm: 'video/webm', m4v: 'video/x-m4v', wmv: 'video/x-ms-wmv', flv: 'video/x-flv',
  cr2: 'image/x-canon-cr2', nef: 'image/x-nikon-nef', arw: 'image/x-sony-arw',
  dng: 'image/x-adobe-dng', raf: 'image/x-fuji-raf',
  // audio
  mp3: 'audio/mpeg', wav: 'audio/wav', m4a: 'audio/mp4', flac: 'audio/flac',
  aac: 'audio/aac', ogg: 'audio/ogg', opus: 'audio/opus', wma: 'audio/x-ms-wma',
}

const IMAGE_EXTS = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'heic', 'heif', 'bmp', 'tif', 'tiff', 'avif', 'cr2', 'nef', 'arw', 'dng', 'raf'])
const VIDEO_EXTS = new Set(['mp4', 'mov', 'avi', 'mkv', 'webm', 'm4v', 'wmv', 'flv'])
const AUDIO_EXTS = new Set(['mp3', 'wav', 'm4a', 'flac', 'aac', 'ogg', 'opus', 'wma'])
// [V2 Design] 디자인 소스 파일 지원 중단 (2026-04-20)
// - .fig 은 Figma 독점 포맷 + 내부 썸네일이 전체 캔버스 오버뷰 와이드 띠로 저화질
// - ASSI 의 "폴더에 넣으면 바로 깔끔하게 보임" 약속을 지킬 수 없음
// - 대안: Figma 에서 "Export frames as PDF" → PDF 파이프라인이 프레임별 고화질 JPEG 로 자동 변환
const DESIGN_EXTS = new Set()
const PDF_EXTS = new Set(['pdf'])
const IGNORED = new Set(['thumbs.db', 'desktop.ini', '.ds_store'])

function fileTitle(fileName = '') {
  return String(fileName || '').replace(/\.[^.]+$/, '').trim()
}

const { extractFigThumbnail } = require('./fig-thumbnail')
const { extractPdfToTmp, cleanupTmpDir } = require('./pdf-extractor')

// Phase 0 방어선 모듈 — 양방향 동기 도입 전 사전 통합 (업로드 루프 방지)
const { RecentlyDownloaded } = require('./infinite-loop-guard')
const { isMarkedSynced } = require('./xattr-marker')
const { UploadThrottle } = require('./upload-throttle')

// 기본 카테고리는 더 이상 하드코딩하지 않음 — 프로젝트 폴더에서 자동 인식
const DEFAULT_CATEGORIES = []
function normalizeCategory(name) {
  if (!name) return null
  return name.trim().toUpperCase()
}

function getExt(name) { return name?.split('.').pop()?.toLowerCase() }

const HEIC_EXTS = new Set(['heic', 'heif'])
const RAW_EXTS = new Set(['cr2', 'nef', 'arw', 'dng', 'raf'])

/* HEIC·RAW → JPEG. 맥은 운영체제가 가진 변환기(sips)를 쓰고, 윈도는 HEIC 만 순수 JS 변환기로 푼다.
   RAW 는 윈도에서 아직 못 푼다 — 사람이 알아들을 말로 알린다. */
async function 사진을JPEG로(filePath) {
  const ext = getExt(filePath)
  const 뭔지 = HEIC_EXTS.has(ext) ? '아이폰 사진(HEIC)' : 'RAW 파일'
  if (process.platform === 'darwin') {
    const out = path.join(os.tmpdir(), `pofol-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`)
    try {
      await new Promise((resolve, reject) => execFile('/usr/bin/sips', ['-s', 'format', 'jpeg', '-s', 'formatOptions', '92', filePath, '--out', out], (err) => err ? reject(err) : resolve()))
      return fs.readFileSync(out)
    } catch (e) {
      throw new Error(`${뭔지}을 변환하지 못했어요. 사진 앱에서 JPEG로 내보내 주세요.`)
    } finally { try { fs.unlinkSync(out) } catch (_) {} }
  }
  if (HEIC_EXTS.has(ext)) {
    try {
      const convert = require('heic-convert')
      return Buffer.from(await convert({ buffer: fs.readFileSync(filePath), format: 'JPEG', quality: 0.92 }))
    } catch (e) {
      throw new Error('아이폰 사진(HEIC)을 변환하지 못했어요. JPEG로 내보내 주세요.')
    }
  }
  throw new Error('RAW 파일은 Windows에서 아직 못 올려요. JPEG로 내보내 주세요.')
}
function guessContentType(name) { return CONTENT_TYPE_MAP[getExt(name)] || 'application/octet-stream' }
function isImage(name) { return IMAGE_EXTS.has(getExt(name)) }
function isVideo(name) { return VIDEO_EXTS.has(getExt(name)) }
function isAudio(name) { return AUDIO_EXTS.has(getExt(name)) }
function isDesign(name) { return DESIGN_EXTS.has(getExt(name)) }
function isFigma(name) { const e = getExt(name); return e === 'fig' || e === 'make' }
function isPdf(name) { return PDF_EXTS.has(getExt(name)) }
function isSupported(name) { return isImage(name) || isVideo(name) || isAudio(name) || isDesign(name) || isPdf(name) }
function isIgnored(name) { const b = name.split(/[/\\]/).pop().toLowerCase(); return b.startsWith('.') || b === '_trash' || IGNORED.has(b) }
function fmt(bytes) {
  if (bytes < 1024) return `${bytes}B`
  if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(0)}KB`
  return `${(bytes / 1024 / 1024).toFixed(1)}MB`
}

class SyncEngine {
  constructor({ uid, watchDir, statePath, api, onProgress, onFileStatus, onError, onNewFolder, onFolderRemoved, onFolderDeletionRequested }) {
    this.uid = uid
    this.watchDir = watchDir
    this.statePath = statePath
    this.api = api // ApiClient instance
    this.onProgress = onProgress || (() => {})
    this.onFileStatus = onFileStatus || (() => {})
    this.onError = onError || (() => {})
    this.onNewFolder = onNewFolder || (() => true)
    this.onFolderRemoved = onFolderRemoved || (() => {})
    this.onFolderDeletionRequested = onFolderDeletionRequested || (() => {})
    this.approvedFolders = new Set()
    this.deniedFolders = new Set()
    this.pendingFolders = new Map()
    this.batchingFolders = new Set() // 배치 처리 중인 폴더 (중복 방지)
    this.newProjectIds = new Set()   // 이번에 새로 만든 프로젝트 — 첫 동기화 뒤 사이트에 싣는 데 쓴다
    this.limitHit = null             // 요금제 상한에 걸리면 { message, limit, current } — 걸린 뒤엔 서버를 다시 두드리지 않는다
    this.watcher = null
    this.state = { syncedFiles: {} }
    this.failedFiles = new Map()
    this.projectCache = new Map()
    this._ready = false

    // ── Rename detection buffers ──
    this.pendingUnlinks = new Map()   // relPath → { filePath, size, projectKey, timer }
    this.pendingUnlinkDirs = new Map() // folderKey → { folderPath, parentKey, timer }

    // ── Phase 0 방어선 (업로드 루프·폭주 차단) ──
    // 현재 단방향 업로드라 recentlyDownloaded 는 placeholder (다운로드 구현 시 mark() 호출됨).
    // xattr/throttle 은 즉시 효과 — 같은 파일 반복 업로드 또는 이미 sync 마킹된 파일 재처리 차단.
    this.recentlyDownloaded = new RecentlyDownloaded()
    this.uploadThrottle = new UploadThrottle({ maxPerMinute: 5 })

    this.loadState()
  }

  /* 로컬에서 사라진 자산을 기억한다 — 다시는 안 되살린다.
     ⚠️ 로컬이 기준이다. 로컬에 없는 것은 '잃어버린 것' 이 아니라 '사용자가 치운 것' 이다. */
  markLocallyRemoved(assetId, relPath) {
    if (!assetId) return
    this.state.locallyRemoved ||= {}
    this.state.locallyRemoved[assetId] = { at: new Date().toISOString(), relPath: relPath || null }
    this.saveState()
  }

  /**
   * 우리가 방금 옮긴 경로를 표시한다 — 2026-09-07
   *
   * ⚠️ 앱이 폴더를 _Trash 로 옮기면 원래 자리에서 사라진다. 그러면 감시기가 그걸
   *    "사용자가 지웠다" 로 읽고 **"웹에서도 삭제할까요?"** 를 띄웠다.
   *    자기가 옮겨놓고 자기가 놀란 것이다. 거기서 「예」 를 누르면 서버에 삭제로 기록되고,
   *    다음에 켤 때 또 옮기고 또 묻는다 — 끝없이 돈다. (테스터 제보)
   *    그래서 우리가 건드린 자리는 잠깐 못 본 척한다.
   */
  markSelfMoved(relOrFolder) {
    this._selfMoved ||= new Map()
    this._selfMoved.set(relOrFolder, Date.now())
  }

  /** 우리가 방금 옮긴 자리인가 (30초 안) */
  isSelfMoved(relOrFolder) {
    if (!this._selfMoved) return false
    const now = Date.now()
    for (const [k, t] of this._selfMoved) {
      if (now - t > 30_000) { this._selfMoved.delete(k); continue }
      if (k === relOrFolder || relOrFolder.startsWith(k + '/') || k.startsWith(relOrFolder + '/')) return true
    }
    return false
  }

  isLocallyRemoved(assetId) {
    return !!(assetId && this.state.locallyRemoved && this.state.locallyRemoved[assetId])
  }

  /* '치웠다' 표시를 지운다 → 다음 폴링에서 다시 받아온다.
     폴더 삭제 확인창에서 **취소**를 누른 경우가 이 길이다 (사용자가 되돌리고 싶다는 뜻). */
  restoreLocallyRemoved(folderKey) {
    if (!this.state.locallyRemoved) return 0
    let n = 0
    for (const [assetId, info] of Object.entries(this.state.locallyRemoved)) {
      const inFolder = !folderKey
        || (info?.relPath && (info.relPath === folderKey || info.relPath.startsWith(folderKey + '/')))
      if (inFolder) { delete this.state.locallyRemoved[assetId]; n++ }
    }
    if (n) this.saveState()
    console.log(`[restoreLocallyRemoved] ${folderKey || '전체'} — ${n}건 되돌림`)
    return n
  }

  /* 잘못 남은 '치웠다' 표시를 걷어낸다 — 2026-09-07 의 뒷정리.
     옮겨진 것을 지운 것으로 오해하던 시절에 156건이 잘못 표시됐다.
     표시된 자산의 파일이 지금 감시 폴더 안에 멀쩡히 있으면, 그 표시는 틀린 것이다.
     (표시가 남아 있으면 그 사진들은 앞으로 영영 안 내려온다) */
  cleanupStaleRemovalMarks() {
    const lr = this.state.locallyRemoved
    if (!lr || !Object.keys(lr).length || !this.watchDir || !fs.existsSync(this.watchDir)) return 0
    const byAsset = new Map()
    for (const [rel, entry] of Object.entries(this.state.syncedFiles || {})) {
      if (entry?.assetId) byAsset.set(entry.assetId, rel)
    }
    let cleared = 0
    for (const assetId of Object.keys(lr)) {
      const rel = byAsset.get(assetId)
      if (rel && fs.existsSync(path.join(this.watchDir, rel))) { delete lr[assetId]; cleared++ }
    }
    if (cleared) {
      this.saveState()
      console.log(`[reconcile] 파일이 멀쩡히 있는데 '치웠다' 로 표시돼 있던 ${cleared}건을 걷어냈다`)
    }
    return cleared
  }

  /**
   * 앱이 꺼져 있는 동안 로컬에서 없어진 것을 찾아낸다 — 2026-09-04
   *
   * 왜 필요한가: 로컬 삭제 → 서버 반영은 앱이 켜져 있을 때만 돈다. 앱을 끄고 폴더를 정리하면
   *   아무도 못 보고, 다음에 켜면 서버 것이 도로 내려와 정리한 것을 덮었다. (테스터 제보)
   *
   * ⚠️ 서버를 함부로 지우지 않는다. 여기서 하는 일은 두 가지뿐이다.
   *    ① '치웠다' 표시를 남겨 **되살아나지 않게** 한다 (이건 로컬 기록이라 안전하다)
   *    ② 사용자에게 "웹에서도 지울까요?" 를 물어본다 (지우는 건 사람이 정한다)
   *
   * ⚠️ 폴더가 통째로 안 보이면 (외장하드 미연결·다른 폴더 선택·iCloud 비움) 아무것도 하지 않는다.
   *    그 상태에서 '지워졌다' 고 판단하면 남의 포트폴리오를 통째로 날린다.
   */
  /* 감시 폴더 안의 모든 파일을 '이름|크기' 로 색인한다.
     파일이 사라진 것처럼 보일 때, 다른 폴더로 옮겨진 것인지 찾아보는 데 쓴다. */
  _indexFilesByNameSize() {
    const index = new Map()
    const skip = new Set(['_trash', '.ds_store', 'node_modules', '.git'])
    const walk = (dir, rel) => {
      let entries = []
      try { entries = fs.readdirSync(dir, { withFileTypes: true }) } catch { return }
      for (const e of entries) {
        if (e.name.startsWith('.') || skip.has(e.name.toLowerCase())) continue
        const full = path.join(dir, e.name)
        const r = rel ? `${rel}/${e.name}` : e.name
        if (e.isDirectory()) { walk(full, r); continue }
        let size = null
        try { size = fs.statSync(full).size } catch { continue }
        const key = `${e.name}|${size}`
        const arr = index.get(key)
        if (arr) arr.push(r); else index.set(key, [r])
      }
    }
    walk(this.watchDir, '')
    return index
  }

  reconcileLocalDeletions() {
    this.cleanupStaleRemovalMarks()
    const tracked = Object.entries(this.state.syncedFiles || {})
    if (tracked.length === 0) return { missing: 0, skipped: 'nothing-tracked' }
    if (!fs.existsSync(this.watchDir)) {
      console.warn('[reconcile] 감시 폴더가 없다 — 아무것도 하지 않는다')
      return { missing: 0, skipped: 'watchdir-missing' }
    }

    const absent = tracked.filter(([relPath]) => !fs.existsSync(path.join(this.watchDir, relPath)))
    if (absent.length === 0) return { missing: 0 }

    /* ⚠️ '원래 자리에 없다' 와 '지웠다' 는 다르다. — 2026-09-07
       카테고리가 바뀌거나 사용자가 폴더를 정리하면 파일은 **옮겨졌을 뿐**인데,
       예전엔 그걸 전부 '지웠다' 로 보고 "웹에서도 지울까요?" 를 띄웠다.
       실제로 프로젝트들이 개인작업/ → Instagram/ 으로 옮겨졌을 때
       멀쩡한 156개가 '치웠다' 로 표시됐다.

       그래서 지우기로 판단하기 전에, 감시 폴더 어딘가에 같은 파일이 있는지 먼저 찾는다.
       (우리가 방금 옮긴 자리라고 표시해 둔 것도 당연히 제외한다) */
    const index = this._indexFilesByNameSize()
    const missing = []
    let movedCount = 0
    for (const [relPath, entry] of absent) {
      if (this.isSelfMoved?.(relPath)) { movedCount++; delete this.state.syncedFiles[relPath]; continue }
      const name = relPath.split('/').pop()
      const found = entry?.fileSize != null ? index.get(`${name}|${entry.fileSize}`) : null
      /* ⚠️ 이미 다른 자산의 자리로 잡혀 있는 파일은 후보가 아니다.
         사진 이름은 카메라 연번(IMG_0001.jpg)이라 프로젝트가 달라도 이름·크기가 곧잘 겹친다.
         그걸 짝으로 인정하면, 사용자가 A 를 정말로 지웠는데 "B 로 옮겼구나" 하고
         삭제를 못 본 척하게 된다. 임자 없는 자리만 후보로 친다. */
      const free = found ? found.filter(r => !this.state.syncedFiles[r]) : null
      if (free && free.length) {
        /* 다른 폴더에서 찾았다 — 옮겨진 것이다. 기록만 새 자리로 옮기고 넘어간다. */
        this.state.syncedFiles[free[0]] = { ...entry }
        delete this.state.syncedFiles[relPath]
        movedCount++
        continue
      }
      missing.push([relPath, entry])
    }
    if (movedCount) {
      console.log(`[reconcile] ${movedCount}개는 지워진 게 아니라 다른 폴더로 옮겨진 것이었다 — 기록만 고쳤다`)
      this.saveState()
    }
    if (missing.length === 0) return { missing: 0, moved: movedCount }

    /* 옮겨진 것을 걸러내고도 대부분이 없다 = 폴더를 못 읽는 상황일 가능성이 크다
       (외장하드 미연결·iCloud 비움). 이럴 때 표시를 남기면 나중에 폴더가 돌아와도 영영 안 받는다.
       ⚠️ 이 판단은 반드시 '옮김' 을 걸러낸 뒤에 해야 한다. 먼저 하면, 폴더를 통째로
          재정리했을 때 전부 사라진 것처럼 보여 옮김 판정까지 가지도 못한다. */
    /* 판단의 기준은 '몇 개가 없나' 가 아니라 '지금 뭐라도 보이나' 다.
       ① 폴더에 파일이 하나도 안 보이면 — 개수와 무관하게 판단하지 않는다.
          (아이클라우드가 비웠거나, 폴더를 못 읽는 상황과 구별할 방법이 없다)
       ② 자료가 제법 있는데 90% 이상이 없어졌으면 — 부분적으로 못 읽는 상황으로 본다. */
    const ENOUGH_TO_JUDGE = 8
    const nothingVisible = index.size === 0
    if (missing.length >= tracked.length * 0.9 && (nothingVisible || tracked.length >= ENOUGH_TO_JUDGE)) {
      console.warn(
        `[reconcile] ${missing.length}/${tracked.length} 이 사라짐` +
        (nothingVisible ? ' · 폴더에 보이는 파일이 없다' : '') +
        ' — 폴더를 못 읽는 상황으로 보고 건너뛴다'
      )
      return { missing: missing.length, moved: movedCount, skipped: 'looks-like-unmounted' }
    }

    const byProject = new Map()
    for (const [relPath, entry] of missing) {
      this.markLocallyRemoved(entry?.assetId, relPath)
      delete this.state.syncedFiles[relPath]
      const pid = entry?.projectId
      if (!pid) continue
      const g = byProject.get(pid) || { projectId: pid, fileCount: 0, folderKey: relPath.split('/').slice(0, -1).join('/') }
      g.fileCount++
      byProject.set(pid, g)
    }
    this.saveState()
    console.log(`[reconcile] 앱이 꺼진 사이 ${missing.length}개가 로컬에서 없어졌다 — 되살리지 않는다`)

    /* 사람에게 묻는다. 서버를 지울지는 사람이 정한다. */
    /* ⚠️ 묻기 전에 그 프로젝트가 웹에 아직 있는지 본다 — 2026-09-08.
       이미 웹에서도 지워진 것을 두고 "웹에서도 지울까요?" 를 물으면,
       '예' 를 눌러도 없는 것을 지우려다 오류만 난다. 물을 이유가 없다.
       서버를 못 물어보면(통신 문제) 예전처럼 묻는다 — 조용히 넘기는 것보다 낫다. */
    this._askAboutMissing(byProject)
    return { missing: missing.length, moved: movedCount, projects: byProject.size }
  }

  async _askAboutMissing(byProject) {
    let alive = null
    try {
      const { projects } = await this.api.getProjectsByUid()
      alive = new Set((projects || []).filter(p => !p.deletedAt).map(p => p.id))
    } catch (e) {
      console.warn('[reconcile] 서버 확인 실패 — 그냥 묻는다:', e.message)
    }
    for (const g of byProject.values()) {
      if (alive && g.projectId && !alive.has(g.projectId)) {
        console.log(`[reconcile] ${g.folderKey} — 웹에도 이미 없다. 묻지 않는다`)
        continue
      }
      this.onFolderDeletionRequested?.({
        folderKey: g.folderKey,
        folderName: g.folderKey.split('/').pop() || g.folderKey,
        projectId: g.projectId,
        fileCount: g.fileCount,
        whileClosed: true,
      })
    }
  }

  loadState() {
    try {
      if (fs.existsSync(this.statePath))
        this.state = JSON.parse(fs.readFileSync(this.statePath, 'utf-8'))
    } catch { this.state = { syncedFiles: {} } }
  }

  saveState() {
    fs.writeFileSync(this.statePath, JSON.stringify(this.state, null, 2))
  }

  getRelPath(filePath) {
    return path.relative(this.watchDir, filePath).split(path.sep).join('/')
  }

  // 폴더 구조: [카테고리]/...[프로젝트]/파일
  // 1차 폴더 = 항상 카테고리, 1차 직속 파일은 무시
  // 파일의 직속 부모 폴더 = 프로젝트 (2차든 3차든 최종 폴더가 프로젝트)
  // 예: BEAUTY/Nike/Campaign1/img.jpg → 카테고리=BEAUTY, 프로젝트=Campaign1
  //     BEAUTY/Solo/img.jpg           → 카테고리=BEAUTY, 프로젝트=Solo
  //     FIGMA/file.fig                → 카테고리=null, 프로젝트=FIGMA  (2-part: 카테고리 폴더 자체가 프로젝트)
  //     file.jpg                      → 무시 (watchDir 직속 파일)
  getProjectName(filePath) {
    const parts = path.relative(this.watchDir, filePath).split(path.sep)
    // watchDir 직속 파일은 프로젝트 판정 불가 → 무시
    if (parts.length < 2) return null
    // 2-part (FOLDER/file): 폴더 자체가 프로젝트
    if (parts.length === 2) return parts[0]
    // 3-part (분류/프로젝트/file): 프로젝트 폴더 이름
    if (parts.length === 3) return parts[1]
    /* 4단 이상: 분류 다음부터 파일 바로 위 폴더까지 이어 붙인다 — 2026-09-25 검수.
       FASHION/202603/올리브영/스틸이미지/a.jpg → "202603_올리브영_스틸이미지".
       예전엔 마지막 폴더(스틸이미지)만 써서, 촬영마다 같은 이름을 쓰면 웹에서 구분이 안 됐다.
       앱 첫 화면 그림은 처음부터 이렇게 약속하고 있었다 — 동작을 약속에 맞춘다. */
    return parts.slice(1, -1).join('_')
  }

  getCategoryName(filePath) {
    const parts = path.relative(this.watchDir, filePath).split(path.sep)
    if (parts.length < 2) return null
    // 2-part (FOLDER/file) 은 그 폴더가 곧 카테고리이자 프로젝트
    // — 사용자가 만든 폴더명을 그대로 존중 (FIGMA, DESIGN, 무엇이든)
    // 백엔드 기본값 'FASHION' 에 빠지지 않게 반드시 folder 이름 넘김.
    if (parts.length === 2) {
      const raw = parts[0]
      return normalizeCategory(raw) || raw
    }
    const raw = parts[0]
    const normalized = normalizeCategory(raw)
    return DEFAULT_CATEGORIES.includes(normalized) ? normalized : raw
  }

  getProjectKey(filePath) {
    const parts = path.relative(this.watchDir, filePath).split(path.sep)
    if (parts.length < 2) return null
    // 파일명 제외한 전체 경로 = 프로젝트 고유 키
    // 예: BEAUTY/Nike/Campaign1 (3+part) 또는 FIGMA (2-part)
    return parts.slice(0, -1).join('/')
  }

  async findOrCreateProject(name, projectKey, category) {
    if (this.projectCache.has(projectKey)) return this.projectCache.get(projectKey)

    /* 상한에 한 번 걸렸으면 새 프로젝트마다 서버에 묻지 않는다 — 같은 대답이 파일 수만큼 돌아왔었다 (2026-09-25).
       [다시 시도]·새로고침이 limitHit 를 비우면 그때 한 번 다시 묻는다 (요금제를 올렸을 수 있다). */
    if (this.limitHit) {
      const e = new Error(this.limitHit.message); e.code = 'project-limit'; throw e
    }
    let result
    try {
      result = await this.api.findOrCreateProject(name, category, projectKey)
    } catch (e) {
      if (e.code === 'project-limit') {
        this.limitHit = { message: e.message, limit: e.limit, current: e.current }
        this.onError({ kind: 'project-limit', message: e.message, limit: e.limit, current: e.current })
      }
      throw e
    }
    if (result?.created && result.projectId) this.newProjectIds.add(result.projectId)
    if (!result?.projectId) {
      throw new Error(`[findOrCreateProject] API returned no projectId for "${name}"`)
    }

    // ★ v1.7.9 방어선: 서버가 projectId 반환했더라도 실제 문서가 유효한지 검증
    // (유령 projectId 방지 — 이전에 write 실패/race condition으로 인한 찌꺼기 방지)
    try {
      const proj = await this.api.getProject(result.projectId)
      if (!proj || !proj.uid) {
        throw new Error(`Project ${result.projectId} has no uid field — invalid document`)
      }
      if (proj.uid !== this.uid) {
        throw new Error(`Project ${result.projectId} uid mismatch: doc=${proj.uid}, user=${this.uid}`)
      }
    } catch (e) {
      console.error(`[SyncEngine] Project validation failed for "${name}" (${result.projectId}):`, e.message)
      throw new Error(`프로젝트 검증 실패: ${e.message}`)
    }

    this.projectCache.set(projectKey, result.projectId)
    return result.projectId
  }

  // ── Content-hash dedup ──
  // 파일 내용물의 sha256 (hex). 스트림 방식이라 메모리 부담 없음.
  // 영상 1GB ≈ 2초, 이미지 무시 가능.
  computeFileHash(filePath) {
    return new Promise((resolve, reject) => {
      const hash = crypto.createHash('sha256')
      const stream = fs.createReadStream(filePath)
      stream.on('data', (chunk) => hash.update(chunk))
      stream.on('end', () => resolve(hash.digest('hex')))
      stream.on('error', reject)
    })
  }

  async processImage(filePath) {
    const ext = getExt(filePath)
    const originalSize = fs.statSync(filePath).size
    /* 아이폰 사진(HEIC)·카메라 RAW 는 우리 이미지 처리기가 못 연다 — 2026-09-25 검수 (맥에서 직접 확인).
       먼저 JPEG 로 바꾸고 나머지는 보통 사진과 똑같이 간다. 바뀐 파일은 이름도 .jpg 로 올라간다. */
    const converted = HEIC_EXTS.has(ext) || RAW_EXTS.has(ext)
    const buffer = converted ? await 사진을JPEG로(filePath) : fs.readFileSync(filePath)
    const meta = await sharp(buffer, { failOn: 'none' }).metadata()

    if (buffer.length <= MAX_SIZE && meta.width <= MAX_DIM && meta.height <= MAX_DIM) {
      return { buffer, skipped: true, converted, originalSize, compressedSize: buffer.length }
    }

    const result = await sharp(buffer, { failOn: 'none' })
      .resize(MAX_DIM, MAX_DIM, { fit: 'inside', withoutEnlargement: true })
      .jpeg({ quality: JPEG_QUALITY })
      .toBuffer()

    return { buffer: result, skipped: false, converted, originalSize, compressedSize: result.length }
  }

  async uploadToStorage(buffer, storagePath, contentType) {
    return this.api.uploadFile(buffer, storagePath, contentType)
  }

  /* ── 미디어 정보 읽기 ────────────────────────────────────────────────
     ffprobe 배포판이 애플 실리콘용이 아니라 못 도는 컴퓨터가 있다.
     그래서 ffprobe 를 먼저 써보고, 안 되면 ffmpeg 가 뱉는 안내문을 읽는다.
     ffmpeg 는 두 아키텍처 다 정상이라 어느 컴퓨터에서도 값이 나온다. */
  async _미디어정보(filePath) {
    if (ffprobe쓸수있나 !== false) {
      try {
        const so = await new Promise((resolve, reject) => {
          execFile(ffprobePath, ['-v', 'error', '-print_format', 'json',
            '-show_format', '-show_streams', '-select_streams', 'v:0', filePath],
          { maxBuffer: 10 * 1024 * 1024 }, (err, so) => err ? reject(err) : resolve(so))
        })
        ffprobe쓸수있나 = true
        const j = JSON.parse(so)
        const st = (j.streams || [])[0] || {}
        return {
          길이: parseFloat(j.format?.duration) || 0,
          폭: st.width || 0,
          높이: st.height || 0,
          태그: { ...(j.format?.tags || {}), ...(st.tags || {}) },
        }
      } catch (e) {
        if (ffprobe쓸수있나 === null) {
          ffprobe쓸수있나 = false
          console.warn('[미디어] ffprobe 를 못 써서 ffmpeg 로 읽습니다:', e.message)
        }
      }
    }
    // ffmpeg 는 파일 정보를 stderr 로 뱉는다. 출력 파일이 없으니 항상 오류로 끝나지만 글은 남는다.
    const 글 = await new Promise(resolve => {
      execFile(ffmpegPath, ['-hide_banner', '-i', filePath],
        { maxBuffer: 10 * 1024 * 1024 }, (err, so, se) => resolve(String(se || '')))
    })
    let 길이 = 0
    const d = 글.match(/Duration:\s*(\d+):(\d+):(\d+(?:\.\d+)?)/)
    if (d) 길이 = (+d[1]) * 3600 + (+d[2]) * 60 + parseFloat(d[3])
    let 폭 = 0, 높이 = 0
    const v = 글.match(/Stream #\d+:\d+[^\n]*: Video:[^\n]*?(\d{2,5})x(\d{2,5})/)
    if (v) { 폭 = +v[1]; 높이 = +v[2] }
    const 태그 = {}
    for (const 줄 of 글.split('\n')) {
      const m = 줄.match(/^\s{4,}([A-Za-z_\-]+)\s*:\s*(.+?)\s*$/)
      if (m && !태그[m[1]]) 태그[m[1]] = m[2]
    }
    return { 길이, 폭, 높이, 태그 }
  }

  /**
   * 오디오 파일 메타데이터 파싱 + embedded cover art 추출
   * @returns {{ metadata: { title, artist, album, duration }, coverBuffer: Buffer|null, coverType: string }}
   */
  async processAudio(filePath) {
    // 1) 태그 + 길이 읽기
    let metadata = { title: '', artist: '', album: '', duration: 0 }
    try {
      const 정보 = await this._미디어정보(filePath)
      const tags = 정보.태그 || {}
      // 케이스 인센시티브 (ID3v1/MP4/iTunes 다양성)
      const get = (...keys) => {
        for (const k of keys) {
          for (const [tk, tv] of Object.entries(tags)) {
            if (tk.toLowerCase() === k.toLowerCase() && tv) return tv
          }
        }
        return ''
      }
      metadata = {
        title: get('title'),
        artist: get('artist') || get('composer'),
        album: get('album'),
        duration: Math.round(정보.길이 || 0),
      }
    } catch (e) {
      console.warn('[Audio] 태그 읽기 실패:', e.message)
    }

    // 2) ffmpeg — embedded cover art 추출 (attached_pic 스트림 or 비디오 첫 프레임)
    let coverBuffer = null
    let coverType = 'image/jpeg'
    const tempCover = path.join(os.tmpdir(), `assi-cover-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.jpg`)
    const tryExtract = (args) => new Promise((resolve, reject) => {
      execFile(ffmpegPath, args, { maxBuffer: 10 * 1024 * 1024 }, (err) => {
        if (err) return reject(err)
        resolve()
      })
    })
    try {
      // 1차 시도: attached_pic 스트림 그대로 추출
      await tryExtract(['-i', filePath, '-an', '-vcodec', 'copy', '-map', '0:v?', '-y', tempCover])
    } catch {
      try {
        // 2차 시도: 비디오 스트림 첫 프레임
        await tryExtract(['-i', filePath, '-vframes', '1', '-f', 'image2', '-y', tempCover])
      } catch (e) {
        console.warn('[Audio] cover extraction failed:', e.message)
      }
    }
    try {
      if (fs.existsSync(tempCover)) {
        const stat = fs.statSync(tempCover)
        if (stat.size > 0) coverBuffer = fs.readFileSync(tempCover)
        try { fs.unlinkSync(tempCover) } catch {}
      }
    } catch {}

    return { metadata, coverBuffer, coverType }
  }

  /**
   * 영상 해상도 체크 → 4K 초과 시 ffmpeg로 리사이즈
   * @returns {{ outputPath: string, width: number, height: number, resized: boolean, originalSize: number, compressedSize: number }}
   */
  async processVideo(filePath, relPath, fileName) {
    // 1) 해상도 확인
    const 정보 = await this._미디어정보(filePath)
    const w = 정보.폭
    const h = 정보.높이
    if (!w || !h) throw new Error('영상 스트림을 찾을 수 없습니다')

    const stat = fs.statSync(filePath)
    const originalSize = stat.size

    console.log(`[Video] ${fileName}: ${w}×${h}, ${(originalSize / 1024 / 1024).toFixed(1)}MB`)

    // 2) 4K 이하면 리사이즈 불필요
    if (w <= VIDEO_MAX_DIM && h <= VIDEO_MAX_DIM) {
      return { outputPath: filePath, width: w, height: h, resized: false, originalSize, compressedSize: originalSize }
    }

    // 3) 4K 초과 → ffmpeg 리사이즈
    console.log(`[Video] ${fileName}: ${w}×${h} → 4K 리사이즈 필요`)
    this.onFileStatus({ path: relPath, status: 'uploading', progress: 5, fileName, isVideo: true, phase: '영상 리사이즈 중...' })

    // 비율 유지하면서 긴 변을 3840으로 축소, 홀수 해상도 방지 (divisible by 2)
    const scale = w >= h
      ? `${VIDEO_MAX_DIM}:-2`   // 가로가 더 길면 가로를 3840으로
      : `-2:${VIDEO_MAX_DIM}`   // 세로가 더 길면 세로를 3840으로

    const tmpDir = os.tmpdir()
    const outputPath = path.join(tmpDir, `assi_resize_${Date.now()}_${fileName}`)

    await new Promise((resolve, reject) => {
      const args = [
        '-i', filePath,
        '-vf', `scale=${scale}`,
        '-c:v', 'libx264',
        '-crf', '18',           // 거의 무손실급 품질
        '-preset', 'medium',    // 속도-품질 균형
        '-c:a', 'aac',          // 오디오 AAC로 통일
        '-b:a', '192k',
        '-movflags', '+faststart',  // 웹 스트리밍 최적화
        '-y',                   // 덮어쓰기
        outputPath,
      ]

      console.log(`[Video] ffmpeg 리사이즈 시작: ${w}×${h} → scale=${scale}`)
      const proc = execFile(ffmpegPath, args, { maxBuffer: 10 * 1024 * 1024 }, (err) => {
        if (err) return reject(err)
        resolve()
      })

      // ffmpeg stderr로 진행 상태 로깅
      if (proc.stderr) {
        proc.stderr.on('data', (data) => {
          const line = data.toString().trim()
          if (line.includes('frame=')) {
            console.log(`[ffmpeg] ${line.substring(0, 80)}`)
          }
        })
      }
    })

    const outStat = fs.statSync(outputPath)
    console.log(`[Video] ${fileName}: 리사이즈 완료 ${(originalSize / 1024 / 1024).toFixed(1)}MB → ${(outStat.size / 1024 / 1024).toFixed(1)}MB`)

    return { outputPath, width: w, height: h, resized: true, originalSize, compressedSize: outStat.size }
  }

  /* 영상 길이를 재고 10% 지점 한 장을 뽑는다. 못 재면 1초 지점으로 물러선다. */
  async _영상표지뽑기(filePath) {
    let 길이 = 0
    try { 길이 = (await this._미디어정보(filePath)).길이 || 0 } catch {}
    const 지점 = 길이 > 0 ? Math.max(0.1, 길이 * 0.1) : 1
    const 임시 = path.join(os.tmpdir(), `pofol-thumb-${Date.now()}.jpg`)
    const 됐나 = await new Promise(resolve => {
      execFile(ffmpegPath, ['-ss', String(지점), '-i', filePath, '-frames:v', '1',
        '-vf', 'scale=1280:-2', '-q:v', '3', '-y', 임시],
      { maxBuffer: 10 * 1024 * 1024 }, err => resolve(!err))
    })
    if (!됐나) return null
    try {
      const buf = fs.readFileSync(임시)
      fs.unlinkSync(임시)
      return buf.length > 0 ? buf : null
    } catch { return null }
  }

  async uploadToBunny(bufferOrPath, fileName, projectId, assetId, relPath, fileSize) {
    let videoId = null   // 실패했을 때 빈 자리를 치우려면 밖에서도 알아야 한다
    try {
      // 1. Create Bunny video + get TUS auth from backend
      const created = await this.api.createBunnyVideo(`${projectId}_${fileName}`)
      videoId = created.videoId
      const { tusAuth, embedUrl } = created

      const displaySize = fileSize ? fmt(fileSize) : (typeof bufferOrPath === 'string' ? '' : fmt(bufferOrPath.length))
      this.onFileStatus({ path: relPath, status: 'uploading', progress: 50, fileName, isVideo: true, size: displaySize, phase: 'Bunny 업로드 중...' })

      // 2. Upload via TUS protocol (chunked for large files, single PATCH for small)
      await this.api.tusUpload(bufferOrPath, videoId, tusAuth, fileSize)

      // 3. Update asset with Bunny info
      await this.api.updateAsset(assetId, {
        videoHost: 'bunny',
        bunnyVideoId: videoId,
        embedUrl,
        bunnyStatus: 'processing',
        bunnyUploadedAt: new Date().toISOString(),
      })

      return { bunnyVideoId: videoId, embedUrl }
    } catch (err) {
      console.error('[Bunny] Upload failed for', fileName, err.message)
      /* ⚠️ 빈 자리를 치운다 — 2026-09-08.
            Bunny 화면에 'Uploading 0 Bytes' 영상이 수십 개 쌓여 있었다. 자리를 만든 뒤
            올리기가 실패하면 빈 자리가 남고, 다음 시도가 새 자리를 또 만든다.
            (치우기가 또 실패해도 조용히 넘어간다 — 원래 하려던 일은 이미 실패한 뒤다.) */
      if (videoId) {
        await this.api.deleteBunnyVideo(videoId).catch(e => console.warn('[Bunny] 빈 자리 못 치움:', e?.message))
      }
      return null
    }
  }

  // 기존 호환용 (백그라운드 폴링) — 더 이상 사용 안 함
  async pollBunnyEncoding(bunnyVideoId, assetId, projectId, fileName) {
    return this.pollBunnyEncodingAwait(assetId, projectId, fileName, null, null)
  }

  /* 영상 인코딩을 **뒤에서** 지켜본다 — 줄 선 다른 파일을 막지 않는다.
     2026-09-08. 예전엔 여기서 최대 60분을 붙잡고 있어서, 영상 하나가
     뒤에 선 파일 전부를 멈춰 세웠다. */
  watchEncodingInBackground(bunnyVideoId, assetId, projectId, fileName, relPath, sizeStr) {
    if (!bunnyVideoId) {
      console.warn(`[Bunny] ${fileName} — 영상 id 를 못 받았다. 올린 것으로 처리한다`)
      if (relPath) this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: true, size: sizeStr || '' })
      return
    }
    this._encodingWatches ||= new Set()
    const key = `${bunnyVideoId}`
    if (this._encodingWatches.has(key)) return          // 같은 영상을 두 번 지켜보지 않는다
    this._encodingWatches.add(key)
    console.log(`[Bunny] ${fileName} 인코딩은 뒤에서 지켜본다 — 다음 파일로 넘어간다`)
    this.pollBunnyEncodingAwait(assetId, projectId, fileName, relPath, sizeStr, bunnyVideoId)
      .catch(e => console.warn(`[Bunny] ${fileName} 인코딩 지켜보기 실패:`, e.message))
      .finally(() => this._encodingWatches.delete(key))
  }

  // 영상 인코딩 완료까지 대기 — done/failed 상태를 직접 리턴
  async pollBunnyEncodingAwait(assetId, projectId, fileName, relPath, sizeStr, videoId = null) {
    /* 영상이 여럿이면 인스턴스 변수는 서로 덮어쓴다 — 넘겨받은 것을 우선한다 */
    const bunnyVideoId = videoId || this._lastBunnyVideoId
    if (!bunnyVideoId) {
      console.error(`[Bunny] No bunnyVideoId for asset ${assetId}`)
      if (relPath) this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: true, size: sizeStr || '' })
      return
    }

    const maxAttempts = 120 // max 60 min (30s interval)
    for (let i = 0; i < maxAttempts; i++) {
      await new Promise(r => setTimeout(r, 30000))

      // 진행 상태 업데이트
      if (relPath) {
        const mins = Math.floor((i + 1) * 0.5)
        this.onFileStatus({ path: relPath, status: 'encoding', progress: Math.min(95, 92 + i * 0.1), fileName, isVideo: true, size: sizeStr || '', phase: `인코딩 중... (${mins}분 경과)` })
      }

      try {
        const { status } = await this.api.checkBunnyStatus(bunnyVideoId)

        if (status === 4) {
          // Encoding complete → save thumbnail
          try {
            const { thumbnailUrl } = await this.api.saveBunnyThumbnail(bunnyVideoId, projectId)
            /* 우리가 10% 지점에서 이미 뽑아 뒀으면 그걸 그대로 둔다 — Bunny 것으로 덮지 않는다 */
            const 이미 = await this.api.getAsset?.(assetId).catch(() => null)
            const 표지있음 = !!이미?.videoThumbnailUrl
            await this.api.updateAsset(assetId, {
              bunnyStatus: 'ready',
              ...(표지있음 ? {} : { videoThumbnailUrl: thumbnailUrl }),
              bunnyEncodedAt: new Date().toISOString(),
            })
            const project = await this.api.getProject(projectId)
            if (project && !project.thumbnailUrl) {
              await this.api.updateProject(projectId, { thumbnailUrl })
            }
            console.log(`[Bunny] ${fileName} encoding complete, thumbnail saved`)
          } catch (thumbErr) {
            await this.api.updateAsset(assetId, { bunnyStatus: 'ready' })
            console.log(`[Bunny] ${fileName} encoding complete (thumbnail save failed)`)
          }
          if (relPath) this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: true, size: sizeStr || '' })
          return
        } else if (status === 5) {
          await this.api.updateAsset(assetId, { bunnyStatus: 'error', bunnyError: 'Encoding failed' })
          console.log(`[Bunny] ${fileName} encoding failed`)
          if (relPath) this.onFileStatus({ path: relPath, status: 'failed', progress: 0, fileName, isVideo: true, error: '인코딩 실패' })
          return
        }
      } catch (err) {
        console.error(`[Bunny] Polling error for ${fileName}:`, err.message)
      }
    }

    // 타임아웃 — 60분 넘으면 일단 done 처리 (Bunny에서 나중에 완료될 수 있음)
    console.log(`[Bunny] ${fileName} encoding timeout (60min), marking done`)
    if (relPath) this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: true, size: sizeStr || '' })
  }

  // PDF → 페이지별 JPEG 시퀀스로 분해해서 일반 이미지 asset 으로 업로드.
  // 원본 PDF 자체는 sourceFile 로 저장 (다운로드 섹션에만 노출).
  // state.syncedFiles[pdfRelPath] = { pdfAssetId, projectId, pages: [{assetId, fileName}], contentHash }
  async syncPdfFile(pdfPath, projectId, relPath) {
    const fileName = path.basename(pdfPath)
    let tmpDir = null
    try {
      const stat = fs.statSync(pdfPath)
      const fileSize = stat.size
      const sizeStr = fmt(fileSize)

      this.onFileStatus({ path: relPath, status: 'uploading', progress: 5, fileName, size: sizeStr, phase: 'PDF 페이지 추출 중...' })

      const result = await extractPdfToTmp(pdfPath, {
        onProgress: ({ done, total }) => {
          const pct = Math.min(30, 5 + Math.round((done / total) * 25))
          this.onFileStatus({ path: relPath, status: 'uploading', progress: pct, fileName, size: sizeStr, phase: `PDF 페이지 추출 중 (${done}/${total})` })
        },
      })
      tmpDir = result.tmpDir
      const pages = result.pages
      const totalPages = pages.length

      const uploadedPages = []
      let newlyAddedImageCount = 0
      let firstPageUrl = null

      // 각 페이지를 일반 이미지 asset 으로 업로드 (page-level content hash dedup)
      for (let i = 0; i < pages.length; i++) {
        const pg = pages[i]
        const pageBuffer = fs.readFileSync(pg.tmpPath)
        const pageHash = crypto.createHash('sha256').update(pageBuffer).digest('hex')
        const pageRelPath = `${relPath}#${pg.pageFileName}`
        const progressBase = 30 + Math.round((i / totalPages) * 65)
        this.onFileStatus({ path: relPath, status: 'uploading', progress: progressBase, fileName, size: sizeStr, phase: `페이지 ${i + 1}/${totalPages} 업로드 중...` })

        // page-level dedup: 동일한 페이지 이미지가 이미 있으면 재사용
        let pageAssetId = null
        let pageUrl = null
        let pageStoragePath = null
        let reused = false
        try {
          const dedup = await this.api.findAssetByContentHash(pageHash).catch(() => null)
          /* 같은 이유로 PDF 쪽도 **다른 폴더 것은 끌어오지 않는다** (위 설명 참고). */
          const matched = (dedup?.asset && dedup.asset.projectId === projectId) ? dedup.asset : null
          if (matched && matched.id) {
            pageAssetId = matched.id
            pageUrl = matched.url
            pageStoragePath = matched.storagePath || ''
            reused = true
          }
        } catch {}

        if (!pageAssetId) {
          pageStoragePath = `users/${this.uid}/projects/${projectId}/${Date.now()}_${pg.pageFileName}`
          pageUrl = await this.uploadToStorage(pageBuffer, pageStoragePath, 'image/jpeg')
          const { assetId: aid } = await this.api.createAsset({
            uid: this.uid, projectId,
            fileName: pg.pageFileName,
            title: fileTitle(pg.pageFileName),
            description: '',
            fileSize: pg.byteSize,
            fileType: 'image/jpeg',
            isVideo: false,
            url: pageUrl,
            storagePath: pageStoragePath,
            parentPdfFileName: fileName,
            derivedFromPdf: true,
            pdfPageIndex: pg.pageIndex,
            pdfPageTotal: pg.total,
            contentHash: pageHash,
            createdAt: new Date().toISOString(),
          })
          pageAssetId = aid
          newlyAddedImageCount++
        }

        if (i === 0) firstPageUrl = pageUrl
        uploadedPages.push({ assetId: pageAssetId, fileName: pg.pageFileName, reused })
      }

      // PDF 원본은 sourceFile 로 업로드 (다운로드용)
      this.onFileStatus({ path: relPath, status: 'uploading', progress: 95, fileName, size: sizeStr, phase: 'PDF 원본 저장 중...' })
      const pdfHash = await this.computeFileHash(pdfPath).catch(() => null)
      const pdfStoragePath = `users/${this.uid}/projects/${projectId}/${Date.now()}_${fileName}`
      const pdfUrl = await this.api.uploadFileStream(pdfPath, pdfStoragePath, 'application/pdf', fileSize)
      const { assetId: pdfAssetId } = await this.api.createAsset({
        uid: this.uid, projectId, fileName,
        fileSize,
        fileType: 'application/pdf',
        sourceFile: true,
        sourceType: 'pdf',
        pdfPageTotal: totalPages,
        url: pdfUrl,
        storagePath: pdfStoragePath,
        contentHash: pdfHash || null,
        createdAt: new Date().toISOString(),
      })

      // 프로젝트 카운터 + 썸네일 업데이트
      const project = await this.api.getProject(projectId).catch(() => null)
      const updates = {}
      if (newlyAddedImageCount > 0) {
        updates._increments = { imageCount: newlyAddedImageCount }
      }
      if (project && !project.thumbnailUrl && firstPageUrl) {
        updates.thumbnailUrl = firstPageUrl
      }
      if (Object.keys(updates).length > 0) {
        await this.api.updateProject(projectId, updates)
      }

      this.state.syncedFiles[relPath] = {
        assetId: pdfAssetId,
        projectId,
        storagePath: pdfStoragePath,
        syncedAt: new Date().toISOString(),
        fileSize,
        contentHash: pdfHash || null,
        pdfPages: uploadedPages,
      }
      this.saveState()
      this.failedFiles.delete(relPath)

      this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, size: sizeStr, phase: `PDF ${totalPages}페이지 완료` })
    } catch (err) {
      console.error('[PDF] Upload failed for', fileName, err.message)
      this.onFileStatus({ path: relPath, status: 'failed', progress: 0, fileName, error: err.message || 'PDF 처리 실패' })
      this.failedFiles.set(relPath, { error: err.message, retries: (this.failedFiles.get(relPath)?.retries || 0) + 1 })
    } finally {
      cleanupTmpDir(tmpDir)
    }
  }

  async syncFile(filePath, _fromBatch = false) {
    const projectName = this.getProjectName(filePath)
    if (!projectName) return

    const relPath = this.getRelPath(filePath)
    if (this.state.syncedFiles[relPath]) return

    // ── Phase 0 Layer 2 — xattr 마킹 체크 ──
    // 동기화 다운로드로 받은 파일이면 여기서 바로 return (업로드 루프 차단)
    // 실패 시 false 반환이라 정상 진행. 현재는 download 미구현이라 대부분 false.
    if (await isMarkedSynced(filePath)) {
      return
    }

    let fileName = path.basename(filePath)
    let contentType = guessContentType(fileName)
    const isVid = isVideo(fileName)
    const isImg = isImage(fileName)
    const isAud = isAudio(fileName)
    const isFig = isFigma(fileName)
    const isPdfFile = isPdf(fileName)
    if (!isImg && !isVid && !isAud && !isFig && !isPdfFile) return

    // New folder detection
    const projectKey = this.getProjectKey(filePath)

    // 배치 처리 중인 폴더의 파일: 배치 호출이 아니면 스킵 (중복 방지)
    if (this._ready && projectKey && this.batchingFolders.has(projectKey) && !_fromBatch) return

    if (this._ready && projectKey && !this.approvedFolders.has(projectKey)) {
      if (this.deniedFolders.has(projectKey)) {
        const pending = this.pendingFolders.get(projectKey)
        if (pending) {
          const folderPath = path.join(this.watchDir, ...projectKey.split('/'))
          try {
            const entries = fs.readdirSync(folderPath)
            pending.fileCount = entries.filter(e => isSupported(e)).length
          } catch {}
        }
        return
      }

      const alreadySynced = Object.keys(this.state.syncedFiles).some(k => k.startsWith(projectKey + '/'))
      if (!alreadySynced) {
        const folderPath = path.join(this.watchDir, ...projectKey.split('/'))
        let fileCount = 0
        try {
          const entries = fs.readdirSync(folderPath)
          fileCount = entries.filter(e => isSupported(e)).length
        } catch {}

        const approved = await this.onNewFolder({ name: projectName, path: projectKey, fileCount })
        if (!approved) {
          this.deniedFolders.add(projectKey)
          this.pendingFolders.set(projectKey, {
            name: projectName, path: projectKey, fileCount,
            addedAt: new Date().toISOString(),
          })
          return
        }
        this.approvedFolders.add(projectKey)

        // 새 폴더 자동 승인 → 폴더 안의 모든 파일을 알파벳 순으로 정렬해 일괄 처리
        // (1번 파일이 자동으로 썸네일이 됨)
        this.batchingFolders.add(projectKey)
        try {
          const folderFiles = []
          try {
            const entries = fs.readdirSync(folderPath)
            for (const e of entries) {
              if (isSupported(e)) folderFiles.push(path.join(folderPath, e))
            }
            folderFiles.sort((a, b) => a.localeCompare(b))
          } catch {}
          for (const f of folderFiles) {
            await this.syncFile(f, true)
          }
        } finally {
          this.batchingFolders.delete(projectKey)
        }
        if (this._ready) await this._첫사이트()   // 지켜보는 중에 새 폴더가 들어와도 사이트에 싣는다
        return
      } else {
        this.approvedFolders.add(projectKey)
      }
    }

    this.onFileStatus({ path: relPath, status: 'uploading', progress: 0, fileName, isVideo: isVid })

    let videoFilePath = filePath  // 영상: 리사이즈된 경로 (또는 원본)
    let videoResized = false

    try {
      const projectKey = this.getProjectKey(filePath)
      const category = this.getCategoryName(filePath)
      const projectId = await this.findOrCreateProject(projectName, projectKey, category)

      // ── PDF: 페이지별 JPEG 시퀀스로 쪼개서 일반 이미지처럼 업로드 ──
      // 각 페이지가 독립 asset 이라 기존 포트폴리오 뷰어(세로 스크롤) 가 그대로 동작.
      // PDF 원본은 sourceFile 로 별도 저장 → 포트폴리오 다운로드 섹션에서 노출.
      if (isPdfFile) {
        await this.syncPdfFile(filePath, projectId, relPath)
        return
      }

      // ── Content-hash dedup ──
      // 이미 같은 내용물의 자산이 서버에 있으면 Storage/Bunny 재업로드 안 하고 metadata만 갱신.
      // 영상 1GB ≈ 2초 해시 비용 — 5~30분 Bunny 재인코딩 회피하면 무조건 이득.
      let contentHash = null
      let fileSizeForState = 0
      try {
        const stat = fs.statSync(filePath)
        fileSizeForState = stat.size
        this.onFileStatus({ path: relPath, status: 'uploading', progress: 5, fileName, isVideo: isVid, phase: '파일 확인 중...' })
        contentHash = await this.computeFileHash(filePath)

        // ── Phase 0 Layer 1 — recentlyDownloaded Map 체크 ──
        // 다운로드 직후(60초 내) 같은 hash 재등장하면 루프로 간주하고 무시
        if (this.recentlyDownloaded.has(contentHash)) {
          this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: isVid, reused: true, phase: '동기화 에코 — 스킵' })
          return
        }

        // ── Phase 0 Throttle — 60초 내 5회 초과 시 자동 정지 ──
        if (!this.uploadThrottle.shouldAllow(contentHash)) {
          console.warn(`[Throttle] ${fileName} 업로드 60초 5회 초과 — 자동 정지. 무한 루프 의심.`)
          this.onFileStatus({ path: relPath, status: 'failed', progress: 0, fileName, isVideo: isVid, error: '반복 업로드 자동 정지 (루프 의심)' })
          this.failedFiles.set(relPath, { error: 'upload throttled', retries: 99 })
          return
        }

        const dedup = await this.api.findAssetByContentHash(contentHash).catch(() => null)
        const matchedRaw = dedup?.asset
        /* ⚠️ **다른 폴더에 있는 사진은 끌어오지 않는다** — 2026-09-08 대표님 결정.

           예전엔 같은 내용의 파일을 만나면 **그 사진을 이 폴더로 옮겨왔다.**
           그래서 맥 폴더 두 곳에 같은 영상을 두면 **먼저 폴더에서 사라졌다.**

           맥 폴더가 기준인데 폴더가 둘이면 기준도 둘이다. 억지로 하나로 묶으면
           한쪽에서 이름을 바꿀 때마다 다른 쪽과 싸우고 복사본이 불어난다.
           **폴더마다 제 사진을 갖는다.** 같은 파일을 두 폴더에 일부러 둔 사람은
           두 벌이 되는 것이 오히려 자연스럽다.

           (웹에서 같은 사진을 두 프로젝트에 넣는 것은 그대로 한 벌을 같이 쓴다 —
            거기는 폴더가 없어서 싸울 상대가 없다.) */
        const matched = (matchedRaw && matchedRaw.projectId === projectId) ? matchedRaw : null
        if (matchedRaw && !matched) {
          console.log(`[dedup] 같은 내용이 다른 폴더에 있다 — 끌어오지 않고 이 폴더에 따로 올린다: ${fileName}`)
        }
        if (matched && matched.id) {
          /* ⚠️ **휴지통에 든 것을 재사용하면 안 된다** — 2026-09-08 테스터 사고.
                Korea Blockchain Week 폴더에 3개, 웹에는 2개. 없어진 하나는 앱이
                "재사용 — 재업로드 없음" 이라고 했는데, 재사용한 그 사진이 **휴지통에 있었다.**
                그래서 웹에서는 보이지 않았다. 파일이 맥 폴더에 버젓이 있다는 건
                사용자가 그 사진을 원한다는 뜻이다 — 휴지통에서 꺼내 준다. */
          if (matched.deletedAt) {
            console.log(`[dedup] 재사용할 사진이 휴지통에 있다 — 꺼낸다: ${fileName}`)
            await this.api.untrashAssets([matched.id]).catch(e => console.warn('[dedup] 휴지통에서 못 꺼냈다:', e?.message))
          }
          // 같은 폴더 안에서 다시 만난 파일 — 그대로 재사용한다. 이름만 다르면 이름만 고친다.
          const isRenameNeeded = matched.fileName !== fileName
          if (isRenameNeeded) {
            await this.api.updateAsset(matched.id, { fileName, updatedAt: new Date().toISOString() })
          }

          // 새 프로젝트 썸네일 갱신 (비어 있으면 — 영상은 videoThumbnailUrl, 이미지는 url)
          try {
            const newProj = await this.api.getProject(projectId)
            if (newProj && !newProj.thumbnailUrl) {
              const thumbCandidate = isVid ? matched.videoThumbnailUrl : matched.url
              if (thumbCandidate) {
                await this.api.updateProject(projectId, { thumbnailUrl: thumbCandidate })
              }
            }
          } catch {}

          // state 갱신 — relPath ↔ assetId 매핑 + contentHash 캐시
          this.state.syncedFiles[relPath] = {
            assetId: matched.id,
            projectId,
            storagePath: matched.storagePath || '',
            syncedAt: new Date().toISOString(),
            fileSize: fileSizeForState,
            contentHash,
          }
          this.saveState()
          this.failedFiles.delete(relPath)

          const sizeStr = fmt(fileSizeForState)
          console.log(`[Dedup] ${fileName} reused existing asset ${matched.id} (hash match) — no re-upload`)
          this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: isVid, size: sizeStr, reused: true, phase: '재사용 — 재업로드 없음' })
          return
        }
      } catch (hashErr) {
        console.warn(`[Dedup] hash check failed for ${fileName}: ${hashErr.message} — fallback to normal upload`)
        // 해시 계산/조회 실패해도 기존 흐름으로 진행
      }

      // ── [V2 Design] Figma .fig 전용 처리 ──
      // 1) .fig 원본은 소스파일로 업로드 (다운로드 용도, 그리드에 안 보임)
      // 2) ZIP 파싱 성공 시 내부 썸네일 PNG 을 별도 이미지 에셋으로 업로드
      //    → 그리드에서 프로젝트 커버로 사용
      // 3) 썸네일 추출 실패해도 계속 진행 (같은 폴더의 png/jpg 가 커버 역할)
      if (isFig) {
        try {
          const stat = fs.statSync(filePath)
          const fileSize = stat.size
          const sizeStr = fmt(fileSize)

          // (1) 썸네일 추출 시도 (ZIP 파싱)
          this.onFileStatus({ path: relPath, status: 'uploading', progress: 10, fileName, size: sizeStr, phase: 'Figma 썸네일 추출 중...' })
          let thumbUrl = null
          let thumbStoragePath = null
          try {
            const thumbBuffer = await extractFigThumbnail(filePath)
            if (thumbBuffer && thumbBuffer.length > 0) {
              const safeName = fileName.replace(/[^a-zA-Z0-9._-]/g, '_').replace(/\.(fig|make)$/i, '')
              thumbStoragePath = `users/${this.uid}/projects/${projectId}/${Date.now()}_${safeName}_thumb.png`
              this.onFileStatus({ path: relPath, status: 'uploading', progress: 25, fileName, size: sizeStr, phase: '썸네일 업로드 중...' })
              thumbUrl = await this.uploadToStorage(thumbBuffer, thumbStoragePath, 'image/png')
              console.log(`[Figma] Thumbnail extracted: ${fileName} (${fmt(thumbBuffer.length)})`)
            } else {
              console.log(`[Figma] No thumbnail in ${fileName} (non-ZIP format or missing entry) — skip thumb upload`)
            }
          } catch (thErr) {
            console.warn(`[Figma] Thumb extract failed for ${fileName}: ${thErr.message}`)
          }

          // (2) .fig 원본 파일 Storage 업로드
          this.onFileStatus({ path: relPath, status: 'uploading', progress: 50, fileName, size: sizeStr, phase: '.fig 업로드 중...' })
          const storagePath = `users/${this.uid}/projects/${projectId}/${Date.now()}_${fileName}`
          const url = await this.api.uploadFileStream(filePath, storagePath, 'application/octet-stream', fileSize)

          // (3) 썸네일 이미지 에셋 생성 (있을 때만 — 그리드 렌더용)
          let thumbAssetId = null
          if (thumbUrl && thumbStoragePath) {
            const thumbResult = await this.api.createAsset({
              uid: this.uid, projectId,
              fileName: fileName.replace(/\.(fig|make)$/i, '') + '.preview.png',
              title: fileTitle(fileName),
              description: '',
              fileSize: 0, // 썸네일 크기는 중요치 않음
              fileType: 'image/png',
              isVideo: false,
              url: thumbUrl,
              storagePath: thumbStoragePath,
              // 부모 .fig 와 연결
              parentFigFileName: fileName,
              derivedFromFig: true,
              createdAt: new Date().toISOString(),
            })
            thumbAssetId = thumbResult.assetId
          }

          // (4) .fig 원본 에셋 생성 (sourceFile 플래그 → 그리드에선 숨김, 다운로드 섹션에만 표시)
          this.onFileStatus({ path: relPath, status: 'uploading', progress: 80, fileName, size: sizeStr, phase: 'Firestore 기록 중...' })
          const { assetId } = await this.api.createAsset({
            uid: this.uid, projectId, fileName,
            fileSize,
            fileType: 'application/figma',
            sourceFile: true,
            sourceType: 'figma',
            url, storagePath,
            contentHash: contentHash || null,
            createdAt: new Date().toISOString(),
          })

          // (5) 프로젝트 카운터/썸네일 업데이트
          // - .fig 자체는 imageCount/videoCount 증가시키지 않음 (sourceFile 이라서)
          // - 썸네일 추출된 경우 그건 imageCount +1 (일반 이미지 취급)
          const project = await this.api.getProject(projectId).catch(() => null)
          const updates = {}
          if (thumbAssetId) {
            updates._increments = { imageCount: 1 }
            if (project && !project.thumbnailUrl) {
              updates.thumbnailUrl = thumbUrl
            }
          }
          if (Object.keys(updates).length > 0) {
            await this.api.updateProject(projectId, updates)
          }

          // (6) state 저장
          this.state.syncedFiles[relPath] = {
            assetId, projectId, storagePath,
            thumbAssetId: thumbAssetId || null,
            syncedAt: new Date().toISOString(),
            fileSize: fileSizeForState || fileSize,
            contentHash: contentHash || null,
          }
          this.saveState()
          this.failedFiles.delete(relPath)

          this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, size: sizeStr, phase: thumbUrl ? 'Figma 파일 + 썸네일 완료' : 'Figma 파일 완료 (썸네일 없음)' })
        } catch (err) {
          console.error('[Figma] Upload failed for', fileName, err.message)
          this.onFileStatus({ path: relPath, status: 'failed', progress: 0, fileName, error: err.message || 'Figma 업로드 실패' })
          this.failedFiles.set(relPath, { error: err.message, retries: (this.failedFiles.get(relPath)?.retries || 0) + 1 })
        }
        return // ── .fig 는 여기서 종료 ──
      }

      // ── 오디오 전용 처리 (이미지/영상과 분리된 경로로 early return) ──
      if (isAud) {
        try {
          const stat = fs.statSync(filePath)
          const fileSize = stat.size
          const sizeStr = fmt(fileSize)

          this.onFileStatus({ path: relPath, status: 'uploading', progress: 10, fileName, isAudio: true, phase: '메타/자켓 추출 중...' })
          // 1) ffprobe + ffmpeg로 태그/자켓 추출
          const { metadata, coverBuffer } = await this.processAudio(filePath)

          // 2) 자켓 Storage 업로드 (있을 경우)
          let albumArtUrl = null
          if (coverBuffer) {
            const safeName = fileName.replace(/[^a-zA-Z0-9._-]/g, '_')
            const artPath = `users/${this.uid}/audio-art/${Date.now()}_${safeName}.jpg`
            try {
              this.onFileStatus({ path: relPath, status: 'uploading', progress: 30, fileName, isAudio: true, size: sizeStr, phase: '자켓 업로드 중...' })
              albumArtUrl = await this.uploadToStorage(coverBuffer, artPath, 'image/jpeg')
              console.log(`[Audio] Cover uploaded: ${fileName} (${fmt(coverBuffer.length)})`)
            } catch (e) {
              console.warn(`[Audio] Cover upload failed: ${e.message}`)
            }
          }

          // 3) 오디오 파일 Storage 업로드
          this.onFileStatus({ path: relPath, status: 'uploading', progress: 50, fileName, isAudio: true, size: sizeStr, phase: '오디오 업로드 중...' })
          const storagePath = `users/${this.uid}/projects/${projectId}/${Date.now()}_${fileName}`
          const url = await this.api.uploadFileStream(filePath, storagePath, contentType, fileSize)

          this.onFileStatus({ path: relPath, status: 'uploading', progress: 85, fileName, isAudio: true, size: sizeStr, phase: 'Firestore 기록 중...' })
          // 4) Firestore asset 문서 생성
          const { assetId } = await this.api.createAsset({
            uid: this.uid, projectId, fileName,
            fileSize,
            fileType: contentType,
            type: 'audio',
            isAudio: true,
            url, storagePath,
            audio: {
              title: metadata.title || fileName.replace(/\.[^.]+$/, ''),
              artist: metadata.artist || '',
              album: metadata.album || '',
              duration: metadata.duration || 0,
              albumArtUrl: albumArtUrl || null,
              fallbackVideoUrl: null,
            },
            contentHash: contentHash || null,
            createdAt: new Date().toISOString(),
          })

          // 5) 프로젝트 카운터 + 썸네일 + 타입 마킹
          const project = await this.api.getProject(projectId).catch(() => null)
          const updates = { _increments: { audioCount: 1 } }
          const isPureAudioProject = project && !project.type && !(project.imageCount > 0) && !(project.videoCount > 0)
          if (isPureAudioProject) {
            updates.type = 'audio'
            updates.category = project.category || 'audio'
          }
          if (project && !project.thumbnailUrl && albumArtUrl) {
            updates.thumbnailUrl = albumArtUrl
          }
          await this.api.updateProject(projectId, updates)

          // 6) state 저장
          this.state.syncedFiles[relPath] = {
            assetId, projectId, storagePath,
            syncedAt: new Date().toISOString(),
            fileSize: fileSizeForState || fileSize,
            contentHash: contentHash || null,
          }
          this.saveState()
          this.failedFiles.delete(relPath)

          this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isAudio: true, size: sizeStr })
        } catch (err) {
          console.error('[Audio] Upload failed for', fileName, err.message)
          this.onFileStatus({ path: relPath, status: 'failed', progress: 0, fileName, isAudio: true, error: err.message || '오디오 업로드 실패' })
          this.failedFiles.set(relPath, { error: err.message, retries: (this.failedFiles.get(relPath)?.retries || 0) + 1 })
        }
        return // ── 오디오는 여기서 종료 (이미지/영상 로직 스킵) ──
      }

      let buffer, originalSize, compressedSize

      if (isImg) {
        const result = await this.processImage(filePath)
        buffer = result.buffer
        originalSize = result.originalSize
        compressedSize = result.compressedSize
        if (result.converted) { fileName = fileName.replace(/\.[^.]+$/, '') + '.jpg'; contentType = 'image/jpeg' }
      } else {
        // Video: 해상도 체크 → 4K 초과 시 ffmpeg 리사이즈
        try {
          const result = await this.processVideo(filePath, relPath, fileName)
          videoFilePath = result.outputPath
          videoResized = result.resized
          originalSize = result.originalSize
          compressedSize = result.compressedSize
        } catch (probeErr) {
          console.warn(`[Video] processVideo 실패, 원본 사용: ${probeErr.message}`)
          const stat = fs.statSync(filePath)
          originalSize = stat.size
          compressedSize = stat.size
        }
      }

      const sizeStr = `${fmt(originalSize)}${(isImg || videoResized) && originalSize !== compressedSize ? '\u2192' + fmt(compressedSize) : ''}`
      this.onFileStatus({ path: relPath, status: 'uploading', progress: 10, fileName, isVideo: isVid, size: sizeStr })

      const storagePath = `users/${this.uid}/projects/${projectId}/${Date.now()}_${fileName}`

      // Video: create Firestore doc first
      let assetId
      let bunnyVideoIdForThisFile = null   // 영상마다 따로 잡는다 — 인스턴스 변수는 서로 덮어쓴다
      if (isVid) {
        const { assetId: aid } = await this.api.createAsset({
          uid: this.uid, projectId, fileName, fileSize: originalSize,
          title: fileTitle(fileName), description: '',
          fileType: contentType, isVideo: true, url: '', storagePath,
          contentHash: contentHash || null,
          createdAt: new Date().toISOString(),
        })
        assetId = aid
        this.onFileStatus({ path: relPath, status: 'uploading', progress: 20, fileName, isVideo: isVid, size: sizeStr })
      }

      // Upload to Firebase Storage via signed URL (stream for video, buffer for image)
      // 영상은 원본을 Storage에 올림 (원본 공유용), Bunny에만 리사이즈본 사용
      const url = isVid
        ? await this.api.uploadFileStream(filePath, storagePath, contentType, originalSize)
        : await this.uploadToStorage(buffer, storagePath, contentType)
      this.onFileStatus({ path: relPath, status: 'uploading', progress: 40, fileName, isVideo: isVid, size: sizeStr })

      if (isVid) {
        await this.api.updateAsset(assetId, { url })

        /* ⚠️ **영상 썸네일을 우리가 먼저 뽑는다 — 길이의 10% 지점** (2026-09-09 대표님 지시).
              예전엔 Bunny 인코딩이 끝나야 썸네일이 생겼고, 실패하면 영영 없었다.
              썸네일 없는 영상이 커버에 걸리면 표지 없이 영상 첫 프레임이 그대로 깔려서,
              "커버를 바꿨는데 옛날 것으로 돌아간다" 로 보였다 (dioor.mp4 가 그 경우다).
              맨 앞(0초)은 검은 화면이거나 페이드인이라 못 쓴다 — 10% 가 안전하다.
              올리자마자 생기므로 인코딩을 기다릴 필요도 없다. */
        try {
          const 표지 = await this._영상표지뽑기(videoFilePath)
          if (표지) {
            const 표지경로 = `thumbnails/${this.uid}/${projectId}/${Date.now()}_${fileName.replace(/\.[^.]+$/, '')}.jpg`
            const 표지주소 = await this.uploadToStorage(표지, 표지경로, 'image/jpeg')
            await this.api.updateAsset(assetId, { videoThumbnailUrl: 표지주소 })
            const proj = await this.api.getProject(projectId).catch(() => null)
            if (proj && !proj.thumbnailUrl) await this.api.updateProject(projectId, { thumbnailUrl: 표지주소 })
            console.log(`[표지] 10% 지점에서 뽑았다: ${fileName}`)
          }
        } catch (e) { console.warn('[표지] 못 뽑았다:', e?.message) }

        // Upload to Bunny via TUS — chunked (50MB at a time, not whole file in memory)
        const bunnyResult = await this.uploadToBunny(videoFilePath, fileName, projectId, assetId, relPath, compressedSize)
        bunnyVideoIdForThisFile = bunnyResult?.bunnyVideoId || null
        this._lastBunnyVideoId = bunnyVideoIdForThisFile   // 옛 호출부 호환
        this.onFileStatus({ path: relPath, status: 'encoding', progress: 90, fileName, isVideo: isVid, size: sizeStr, phase: '인코딩 대기 중...' })
      } else {
        const { assetId: aid } = await this.api.createAsset({
          uid: this.uid, projectId, fileName, fileSize: compressedSize,
          title: fileTitle(fileName), description: '',
          originalSize: originalSize, // 원본 파일 크기 (무압축 공유용)
          fileType: contentType, isVideo: false, url, storagePath,
          contentHash: contentHash || null,
          createdAt: new Date().toISOString(),
        })
        assetId = aid
      }

      // Update project counters
      const project = await this.api.getProject(projectId)
      const updates = {
        _increments: { [isVid ? 'videoCount' : 'imageCount']: 1 },
      }
      if (project && !project.thumbnailUrl && !isVid) {
        updates.thumbnailUrl = url
      }
      await this.api.updateProject(projectId, updates)

      // Record in state
      this.state.syncedFiles[relPath] = {
        assetId, projectId, storagePath,
        syncedAt: new Date().toISOString(),
        fileSize: fileSizeForState || originalSize,
        contentHash: contentHash || null,
      }
      this.saveState()
      this.failedFiles.delete(relPath)

      if (isVid) {
        /* ⚠️ 여기서 기다리면 안 된다 — 2026-09-08 테스터 제보.
           파일은 한 개씩 줄 세워 처리하는데, 영상만 "Bunny 서버가 인코딩을 끝낼 때까지"
           최대 60분을 붙잡고 있었다. 그동안 뒤에 선 파일 90개가 통째로 멈췄다.
           (249MB 영상 하나에서 16분째 멈춰 있다는 제보)

           올리는 일은 이미 끝났다. 인코딩은 Bunny 서버가 하는 일이라 우리가 붙잡을 이유가 없다.
           상태는 뒤에서 따로 지켜보고, 줄은 바로 다음 파일로 넘긴다. */
        this.onFileStatus({ path: relPath, status: 'encoding', progress: 92, fileName, isVideo: isVid, size: sizeStr, phase: '영상 인코딩 중...' })
        this.watchEncodingInBackground(bunnyVideoIdForThisFile, assetId, projectId, fileName, relPath, sizeStr)
      } else {
        this.onFileStatus({ path: relPath, status: 'done', progress: 100, fileName, isVideo: isVid, size: sizeStr })
      }
    } catch (err) {
      this.failedFiles.set(relPath, filePath)
      this.onFileStatus({ path: relPath, status: 'failed', progress: 0, fileName, isVideo: isVid, error: err.message })
    } finally {
      // 리사이즈된 임시파일 정리
      if (videoResized && videoFilePath !== filePath) {
        try { fs.unlinkSync(videoFilePath) } catch (_) {}
      }
    }
  }

  getPendingFolders() {
    return [...this.pendingFolders.entries()].map(([key, data]) => {
      const folderPath = path.join(this.watchDir, ...key.split('/'))
      try {
        const entries = fs.readdirSync(folderPath)
        data.fileCount = entries.filter(e => isSupported(e)).length
      } catch {}
      return { key, ...data }
    })
  }

  async approvePendingFolder(projectKey) {
    this.deniedFolders.delete(projectKey)
    this.approvedFolders.add(projectKey)
    this.pendingFolders.delete(projectKey)

    const folderPath = path.join(this.watchDir, ...projectKey.split('/'))
    try {
      const entries = fs.readdirSync(folderPath)
      for (const entry of entries) {
        const filePath = path.join(folderPath, entry)
        const stat = fs.statSync(filePath)
        if (stat.isFile() && isSupported(entry)) {
          await this.syncFile(filePath)
        }
      }
    } catch (err) {
      this.onError({ message: `Upload failed: ${projectKey}`, error: err.message })
    }
  }

  removePendingFolder(projectKey) {
    this.deniedFolders.delete(projectKey)
    this.pendingFolders.delete(projectKey)
  }

  getSyncedFolders() {
    const folderMap = new Map()
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      const parts = relPath.split('/')
      if (parts.length < 2) continue
      const key = parts.slice(0, -1).join('/')
      const name = parts[parts.length - 2]
      if (!folderMap.has(key)) {
        folderMap.set(key, { key, name, path: key, fileCount: 0, projectId: entry.projectId })
      }
      folderMap.get(key).fileCount++
    }

    const result = []
    for (const [key, data] of folderMap) {
      const folderPath = path.join(this.watchDir, ...key.split('/'))
      if (fs.existsSync(folderPath)) {
        result.push(data)
      }
    }
    return result
  }

  async resyncFolder(projectKey) {
    for (const relPath of Object.keys(this.state.syncedFiles)) {
      if (relPath.startsWith(projectKey + '/')) {
        delete this.state.syncedFiles[relPath]
      }
    }
    this.saveState()
    this.projectCache.delete(projectKey)

    const folderPath = path.join(this.watchDir, ...projectKey.split('/'))
    try {
      const entries = fs.readdirSync(folderPath)
      for (const entry of entries) {
        const filePath = path.join(folderPath, entry)
        const stat = fs.statSync(filePath)
        if (stat.isFile() && isSupported(entry)) {
          await this.syncFile(filePath)
        }
      }
    } catch (err) {
      this.onError({ message: `Re-upload failed: ${projectKey}`, error: err.message })
    }
  }

  /* 탐색기의 '이 폴더 지우기'.

     ⚠️ 예전엔 여기서 **영구 삭제**를 했다 — 2026-09-07 에 발견.
        저장소의 사진 파일 자체를 지우고(deleteFile), 자산·프로젝트도 완전 삭제였다.
        되돌릴 방법이 아예 없었다. 그런데 앱의 다른 모든 곳과 웹사이트는
        "지워도 휴지통에 30일 보관" 이라고 안내한다. 사용자는 되돌릴 수 있다고
        믿고 누른다. 약속을 지키는 쪽으로 동작을 바꿨다.

     이제 웹사이트의 삭제와 똑같이 휴지통으로 보낸다 (30일 뒤 서버가 정리).
     로컬 원본은 다음 폴링에서 폴더 안 _Trash 로 옮겨지고,
     웹에서 되살리면 원래 자리로 돌아온다. */
  async deleteSyncedFolder(projectKey) {
    const entries = Object.entries(this.state.syncedFiles)
      .filter(([relPath]) => relPath.startsWith(projectKey + '/'))

    const projectId = entries.length ? entries[0][1]?.projectId : null
    if (!projectId) {
      console.warn(`[deleteSyncedFolder] ${projectKey} — 올린 기록이 없다. 아무것도 하지 않는다`)
      return { ok: false, reason: 'not-synced' }
    }

    try {
      await this.api.softDeleteProject(projectId)
    } catch (e) {
      console.warn(`[deleteSyncedFolder] 휴지통으로 보내기 실패:`, e.message)
      return { ok: false, error: e.message }
    }

    /* 기록은 지우지 않는다 — 로컬 원본을 _Trash 로 옮길 때 이 기록으로
       "정말 우리가 올린 그 파일인지" 를 확인한다. 지우면 확인할 근거가 사라진다. */
    this.projectCache.delete(projectKey)
    console.log(`[deleteSyncedFolder] ${projectKey} → 웹 휴지통 (30일 보관)`)
    try { await this.triggerDownloadPollNow?.() } catch {}
    return { ok: true, projectId }
  }

  /* ──────────────────────────────────────────────────────────
     파일 하나하나를 휴지통으로 — 중복 정리에서 쓴다.

     ⚠️ 프로젝트 통째로 옮기는 것(_moveProjectToTrash)은 이미 있었는데,
        중복 파일을 하나씩 지울 때는 클라우드만 지우고 컴퓨터의 원본은
        그대로 남았다. 그러면 정리를 해도 디스크가 안 준다.

     ⚠️ 되돌릴 수 있어야 하므로 '원래 어디 있었는지' 를 같이 적어둔다.
        .meta.json 에 { assetId, relPath } 를 남기고, 되돌릴 때 그 자리로 옮긴다.
     ────────────────────────────────────────────────────────── */
  trashAssetFiles(assetIds) {
    const ids = new Set(assetIds || [])
    if (!ids.size || !this.watchDir) return { moved: 0 }
    const d = new Date()
    const pad = n => String(n).padStart(2, '0')
    const stamp = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}-${pad(d.getMinutes())}-${pad(d.getSeconds())}`
    const dir = path.join(this.watchDir, '_Trash', `중복 정리 (${stamp})`)
    const moved = []
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles || {})) {
      if (!entry || !ids.has(entry.assetId)) continue
      const abs = path.join(this.watchDir, relPath)
      if (!fs.existsSync(abs)) continue
      /* 그 사이에 파일이 바뀌었으면 우리가 알던 그 파일이 아니다 — 손대지 않는다 */
      if (entry.fileSize != null) {
        let now = null
        try { now = fs.statSync(abs).size } catch {}
        if (now != null && now !== entry.fileSize) {
          console.warn(`[trash-file] 내용이 바뀐 파일이라 건너뛴다: ${relPath}`)
          continue
        }
      }
      try {
        fs.mkdirSync(dir, { recursive: true })
        let dest = path.join(dir, path.basename(relPath))
        if (fs.existsSync(dest)) {
          const ext = path.extname(dest), stem = dest.slice(0, dest.length - ext.length)
          let n = 2
          while (fs.existsSync(`${stem}_${n}${ext}`)) n++
          dest = `${stem}_${n}${ext}`
        }
        fs.renameSync(abs, dest)
        moved.push({ assetId: entry.assetId, relPath, file: path.basename(dest) })
        /* 감시 기록에서 뺀다 — 안 빼면 사라진 파일로 보고 서버에서 또 지우려 든다 */
        delete this.state.syncedFiles[relPath]
      } catch (e) {
        console.warn('[trash-file] 옮기기 실패:', relPath, e.message)
      }
    }
    if (moved.length) {
      try {
        fs.writeFileSync(path.join(dir, '.meta.json'),
          JSON.stringify({ kind: 'dupe', movedAt: new Date().toISOString(), items: moved }, null, 2))
      } catch {}
      this.saveState?.()
    }
    console.log(`[trash-file] ${moved.length}개를 _Trash 로 옮겼다`)
    return { moved: moved.length }
  }

  /** 되돌리기 — .meta.json 을 뒤져 원래 자리로 옮긴다. */
  restoreAssetFiles(assetIds) {
    const ids = new Set(assetIds || [])
    const trashRoot = path.join(this.watchDir || '', '_Trash')
    if (!ids.size || !fs.existsSync(trashRoot)) return { restored: 0 }
    let restored = 0
    for (const name of fs.readdirSync(trashRoot)) {
      const dir = path.join(trashRoot, name)
      const metaPath = path.join(dir, '.meta.json')
      if (!fs.existsSync(metaPath)) continue
      let meta
      try { meta = JSON.parse(fs.readFileSync(metaPath, 'utf8')) } catch { continue }
      if (meta.kind !== 'dupe' || !Array.isArray(meta.items)) continue
      const left = []
      for (const it of meta.items) {
        if (!ids.has(it.assetId)) { left.push(it); continue }
        const from = path.join(dir, it.file)
        const to = path.join(this.watchDir, it.relPath)
        try {
          if (!fs.existsSync(from)) continue
          /* 그 자리에 이미 파일이 있으면 덮어쓰지 않는다 — 옆에 번호를 붙여 되돌린다.
             덮어쓰면 사용자가 그 사이에 새로 넣은 파일이 소리 없이 사라진다. */
          let dest = to
          if (fs.existsSync(dest)) {
            const ext = path.extname(dest), stem = dest.slice(0, dest.length - ext.length)
            let n = 2
            while (fs.existsSync(`${stem}_${n}${ext}`)) n++
            dest = `${stem}_${n}${ext}`
          }
          fs.mkdirSync(path.dirname(dest), { recursive: true })
          fs.renameSync(from, dest)
          restored++
        } catch (e) {
          console.warn('[trash-file] 되돌리기 실패:', it.relPath, e.message)
          left.push(it)
        }
      }
      try {
        if (left.length) {
          fs.writeFileSync(metaPath, JSON.stringify({ ...meta, items: left }, null, 2))
        } else {
          /* 목록이 비었어도 폴더에 다른 게 남아 있으면 지우지 않는다.
             사용자가 직접 넣어 둔 파일일 수 있다 — 메모 파일만 치운다. */
          const rest = fs.readdirSync(dir).filter(n => n !== '.meta.json' && n !== '.DS_Store')
          if (rest.length === 0) fs.rmSync(dir, { recursive: true, force: true })
          else fs.unlinkSync(metaPath)
        }
      } catch {}
    }
    console.log(`[trash-file] ${restored}개를 원래 자리로 되돌렸다`)
    return { restored }
  }

  // 웹에서 soft-delete 된 프로젝트 → 로컬 파일을 _Trash 폴더로 이동 (30일 후 자동 영구 삭제).
  // server 가 보내준 정보: { id, deletedAt, name } 또는 그냥 id 문자열 (구버전 호환)
  // idempotent — 이미 처리됐으면 스킵.
  async removeDeletedProjectLocally(projectInfo) {
    const projectId = typeof projectInfo === 'string' ? projectInfo : projectInfo.id
    const deletedAt = typeof projectInfo === 'object' ? projectInfo.deletedAt : new Date().toISOString()
    const projectName = typeof projectInfo === 'object' ? projectInfo.name : null

    // 동시성 가드 — setInterval tick + triggerDownloadPollNow 가 동시에 fire 되면
    // 같은 projectId 가 동시에 처리돼서 _2, _3 폴더가 양산됨. 60초 윈도우로 dedupe.
    this._recentTrashOps ||= new Map() // projectId → timestamp
    const last = this._recentTrashOps.get(projectId)
    if (last && Date.now() - last < 60 * 1000) {
      console.log(`[trash-move] ${projectId} 이미 60초 내 처리됨 — skip`)
      return
    }
    this._recentTrashOps.set(projectId, Date.now())

    const entries = Object.entries(this.state.syncedFiles)
      .filter(([_, entry]) => entry.projectId === projectId)
    if (entries.length === 0) return  // 동기화 안 되어 있던 프로젝트

    /* ⚠️ 여기서 반드시 신원을 확인한다. — 2026-09-07 테스터 제보
       예전엔 경로만 보고 그 자리에 있는 파일을 그대로 쓸어 담았다.
       사용자가 그 폴더를 지웠다 **다시 만들어 놨으면**, 멀쩡한 새 파일이 통째로 휴지통에 딸려갔다.
       서버가 "이 프로젝트 지워졌다" 고 해도, 로컬에 지금 있는 파일이 그 프로젝트 것이라는 보장은 없다.

       판단: 우리가 기록해 둔 크기와 지금 파일의 크기가 다르면 우리 것이 아니다.
       하나라도 다르면 **아무것도 건드리지 않는다.** 반쯤 옮기는 게 제일 나쁘다.
       (로컬이 기준이다 — 애매하면 손대지 않는 쪽이 맞다) */
    const foreign = []
    let present = 0      // 지금 디스크에 실제로 있는 파일 수
    let verified = 0     // 그중 우리 기록과 크기가 맞아떨어진 수
    for (const [relPath, entry] of entries) {
      const abs = path.join(this.watchDir, relPath)
      if (!fs.existsSync(abs)) continue          // 이미 없는 것은 상관없다
      present++
      if (entry.fileSize == null) continue        // 기록이 없다 — 아래에서 통째로 막는다
      let size = null
      try { size = fs.statSync(abs).size } catch { continue }
      if (size !== entry.fileSize) foreign.push({ relPath, 기록: entry.fileSize, 지금: size })
      else verified++
    }

    /* 파일은 있는데 대조할 기록이 하나도 없으면 — 옛 버전이 남긴 기록이다.
       확인할 방법이 없으니 옮기지 않는다. 놔두면 사용자가 지우면 그만이지만,
       잘못 옮기면 사용자가 잃는다. */
    if (present > 0 && verified === 0 && foreign.length === 0) {
      console.warn(
        `[trash-move] ${projectId} — 우리가 올린 것이라는 근거(파일 크기)가 기록에 없다. ` +
        `확인할 수 없으므로 **아무것도 옮기지 않는다.**`
      )
      for (const [relPath] of entries) delete this.state.syncedFiles[relPath]
      this.saveState()
      return
    }

    if (foreign.length > 0) {
      console.warn(
        `[trash-move] ${projectId} — 로컬 파일이 우리가 올린 것과 다르다 (${foreign.length}개). ` +
        `사용자가 폴더를 다시 만든 것으로 보고 **아무것도 옮기지 않는다.**`
      )
      for (const f of foreign.slice(0, 5)) {
        console.warn(`             ${f.relPath}  기록 ${f.기록} · 지금 ${f.지금}`)
      }
      /* 낡은 기록만 지운다 — 그래야 다음에 또 같은 판단을 반복하지 않는다 */
      for (const [relPath] of entries) delete this.state.syncedFiles[relPath]
      this.saveState()
      return
    }

    // 휴지통 폴더 — _Trash/{projectName} (YYYY-MM-DD HH-mm-ss)/
    // 같은 이름 충돌 시 동작:
    //   - 기존 폴더의 .meta.json 에 같은 projectId → 그 폴더에 합침 (새 폴더 안 만듦)
    //   - 다른 projectId → ID 첫 6자 suffix 붙여서 구분 ('dogfood (...) [abc123]')
    //   - .meta.json 없거나 망가짐 → 안전하게 ID suffix
    const d = new Date(deletedAt)
    const pad = (n) => String(n).padStart(2, '0')
    const stamp = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}-${pad(d.getMinutes())}-${pad(d.getSeconds())}`
    const safeName = (projectName || '(이름 없음)').replace(/[\\/:*?"<>|]/g, '_').trim() || '(이름 없음)'
    const baseName = `${safeName} (${stamp})`
    const trashRoot = path.join(this.watchDir, '_Trash')
    let trashProjectDir = path.join(trashRoot, baseName)

    if (fs.existsSync(trashProjectDir)) {
      // 기존 폴더 검사 — 같은 projectId 면 합침, 다르면 ID suffix
      let existingPid = null
      try {
        const m = JSON.parse(fs.readFileSync(path.join(trashProjectDir, '.meta.json'), 'utf8'))
        existingPid = m.projectId
      } catch {}
      if (existingPid && existingPid !== projectId) {
        // 다른 프로젝트 — ID 첫 6자 붙여서 구분
        trashProjectDir = path.join(trashRoot, `${baseName} [${projectId.slice(0, 6)}]`)
      }
      // 같으면 그대로 trashProjectDir 유지 → 파일 merge
    }

    try {
      if (!fs.existsSync(trashRoot)) fs.mkdirSync(trashRoot, { recursive: true })
      fs.mkdirSync(trashProjectDir, { recursive: true })
      // 복구/추적용 메타 (사람이 읽을 수 있게)
      fs.writeFileSync(path.join(trashProjectDir, '.meta.json'), JSON.stringify({
        projectId, projectName, deletedAt, movedAt: new Date().toISOString(),
      }, null, 2))
    } catch (e) {
      console.warn('[trash-move] 휴지통 폴더 생성 실패:', e.message)
    }

    /* 우리가 옮길 자리를 먼저 표시한다 — 감시기가 이걸 '사용자 삭제' 로 오인하지 않게. */
    for (const [relPath] of entries) {
      this.markSelfMoved(relPath)
      const dir = relPath.split('/').slice(0, -1).join('/')
      if (dir) this.markSelfMoved(dir)
    }

    const folders = new Set()
    const movedItems = []   // 되돌릴 때 쓸 '원래 자리' 기록
    for (const [relPath, entry] of entries) {
      const abs = path.join(this.watchDir, relPath)
      // 평탄 구조 — _Trash/{프로젝트이름 (시간)}/{파일명}. 카테고리/프로젝트 중첩 X.
      // 동명 파일 충돌 시 카운터: img.jpg, img_2.jpg, img_3.jpg ...
      const fileName = path.basename(relPath)
      let dest = path.join(trashProjectDir, fileName)
      if (fs.existsSync(dest)) {
        const ext = path.extname(fileName)
        const stem = fileName.slice(0, fileName.length - ext.length)
        let n = 2
        while (fs.existsSync(path.join(trashProjectDir, `${stem}_${n}${ext}`))) n++
        dest = path.join(trashProjectDir, `${stem}_${n}${ext}`)
      }
      let ok = false
      try {
        if (fs.existsSync(abs)) {
          fs.renameSync(abs, dest)
          ok = true
        }
      } catch (e) {
        try {
          if (fs.existsSync(abs)) {
            fs.copyFileSync(abs, dest)
            fs.unlinkSync(abs)
            ok = true
          }
        } catch (e2) {
          console.warn(`[trash-move] 이동 실패 ${abs} → ${dest}:`, e2.message)
        }
      }
      if (ok) movedItems.push({ assetId: entry.assetId, relPath, file: path.basename(dest) })
      const parent = path.dirname(abs)
      if (parent && parent !== this.watchDir) folders.add(parent)
      delete this.state.syncedFiles[relPath]
    }

    // 빈 프로젝트 폴더 (원본 위치) 제거 — 사용자 추가 파일 있으면 남김
    for (const folder of folders) {
      try {
        if (fs.existsSync(folder) && fs.readdirSync(folder).length === 0) {
          fs.rmdirSync(folder)
          const cat = path.dirname(folder)
          if (cat && cat !== this.watchDir && fs.existsSync(cat) && fs.readdirSync(cat).length === 0) {
            fs.rmdirSync(cat)
          }
        }
      } catch (e) {
        console.warn(`[trash-move] 원본 폴더 정리 실패 ${folder}:`, e.message)
      }
    }

    /* 옮긴 뒤에 '어디서 왔는지' 를 다시 적는다 — 되돌릴 때 이 기록만이 근거다 */
    try {
      fs.writeFileSync(path.join(trashProjectDir, '.meta.json'), JSON.stringify({
        projectId, projectName, deletedAt, movedAt: new Date().toISOString(), items: movedItems,
      }, null, 2))
    } catch {}

    for (const [key, val] of this.projectCache.entries()) {
      if (val === projectId) this.projectCache.delete(key)
    }
    this.saveState()
    console.log(`[trash-move] 프로젝트 ${projectId} → _Trash 이동 완료 (파일 ${entries.length}개)`)
  }

  // 웹에서 복구된 프로젝트 → 로컬 _Trash 의 해당 잔재 폴더 영구 삭제.
  // .meta.json 의 projectId 매치로 식별. 여러 개 잔재 있으면 (delete-restore-delete 사이클) 다 정리.
  async cleanupRestoredProjectLocally(projectId) {
    if (!this.watchDir) return
    const trashRoot = path.join(this.watchDir, '_Trash')
    if (!fs.existsSync(trashRoot)) return
    let removed = 0
    try {
      const entries = fs.readdirSync(trashRoot, { withFileTypes: true })
      for (const e of entries) {
        if (!e.isDirectory()) continue
        const dir = path.join(trashRoot, e.name)
        let metaPid = null
        try {
          const meta = JSON.parse(fs.readFileSync(path.join(dir, '.meta.json'), 'utf8'))
          metaPid = meta.projectId
        } catch {}
        if (metaPid && metaPid === projectId) {
          try {
            this.restoreProjectFromLocalTrash(dir)
            removed++
          } catch (err) {
            console.warn(`[trash-restore-cleanup] 되돌리기 실패 ${dir}:`, err.message)
          }
        }
      }
      if (removed > 0) console.log(`[trash-restore-cleanup] 복구된 프로젝트 ${projectId} 의 로컬 _Trash 잔재 ${removed}개 정리`)
    } catch (e) {
      console.warn('[trash-restore-cleanup] 스캔 실패:', e.message)
    }
  }

  /* 웹에서 프로젝트를 되살렸을 때 — 로컬 _Trash 에 있던 원본을 **원래 자리로 돌려놓는다.**

     ⚠️ 예전엔 이 폴더를 그냥 지웠다. 웹에서 다시 받아올 거라는 전제였는데,
        이제 웹→로컬은 사용자가 버튼을 눌러야만 일어난다. 그래서 지우면 그냥 없어진다.
        되돌릴 자리를 모르면 **아무것도 지우지 않고 그대로 둔다** (30일 뒤 자동 정리에 맡긴다). */
  restoreProjectFromLocalTrash(dir) {
    let meta = null
    try { meta = JSON.parse(fs.readFileSync(path.join(dir, '.meta.json'), 'utf8')) } catch {}
    const items = Array.isArray(meta?.items) ? meta.items : []
    if (items.length === 0) {
      console.log(`[trash-restore] 원래 자리를 모른다 — 그대로 둔다: ${path.basename(dir)}`)
      return 0
    }
    let back = 0
    const left = []
    for (const it of items) {
      const from = path.join(dir, it.file)
      if (!fs.existsSync(from)) continue
      let to = path.join(this.watchDir, it.relPath)
      try {
        if (fs.existsSync(to)) {
          /* 그 사이에 사용자가 같은 이름으로 새 파일을 넣었을 수 있다 — 덮어쓰지 않는다 */
          const ext = path.extname(to), stem = to.slice(0, to.length - ext.length)
          let n = 2
          while (fs.existsSync(`${stem}_${n}${ext}`)) n++
          to = `${stem}_${n}${ext}`
        }
        this.markSelfMoved?.(it.relPath)
        fs.mkdirSync(path.dirname(to), { recursive: true })
        fs.renameSync(from, to)
        back++
      } catch (e) {
        console.warn(`[trash-restore] 되돌리기 실패 ${it.relPath}:`, e.message)
        left.push(it)
      }
    }
    try {
      const rest = fs.readdirSync(dir).filter(n => n !== '.meta.json' && n !== '.DS_Store')
      if (rest.length === 0 && left.length === 0) fs.rmSync(dir, { recursive: true, force: true })
      else fs.writeFileSync(path.join(dir, '.meta.json'), JSON.stringify({ ...meta, items: left }, null, 2))
    } catch {}
    console.log(`[trash-restore] ${back}개를 원래 자리로 돌려놨다`)
    return back
  }

  // 로컬 _Trash 폴더 청소 — 30일 지난 폴더 영구 삭제 + 옛 이름 마이그레이션.
  // 앱 시작 시 + 매일 1회 호출.
  async cleanupLocalTrash() {
    if (!this.watchDir) return
    const trashRoot = path.join(this.watchDir, '_Trash')
    if (!fs.existsSync(trashRoot)) return

    const THIRTY_DAYS_MS = 30 * 24 * 60 * 60 * 1000
    const now = Date.now()
    let removed = 0
    let renamed = 0
    try {
      const entries = fs.readdirSync(trashRoot, { withFileTypes: true })
      for (const e of entries) {
        if (!e.isDirectory()) continue
        let dir = path.join(trashRoot, e.name)
        let deletedAtMs = null
        let meta = null
        try {
          meta = JSON.parse(fs.readFileSync(path.join(dir, '.meta.json'), 'utf8'))
          if (meta.deletedAt) deletedAtMs = new Date(meta.deletedAt).getTime()
        } catch {}
        if (deletedAtMs === null) {
          try { deletedAtMs = fs.statSync(dir).mtimeMs } catch { continue }
        }

        // 30일 지난 폴더 → 영구 삭제
        if (now - deletedAtMs > THIRTY_DAYS_MS) {
          try {
            fs.rmSync(dir, { recursive: true, force: true })
            removed++
          } catch (err) {
            console.warn(`[trash-cleanup] 영구 삭제 실패 ${dir}:`, err.message)
          }
          continue
        }

        // 옛 형식 (`{projectId}__{ISOstamp}`) 이름이고 meta 가 있으면 새 이름으로 rename
        const oldPattern = /^[A-Za-z0-9]{16,}__\d{4}-/
        if (oldPattern.test(e.name) && meta && meta.deletedAt) {
          const d = new Date(meta.deletedAt)
          const pad = (n) => String(n).padStart(2, '0')
          const stamp = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}-${pad(d.getMinutes())}-${pad(d.getSeconds())}`
          const safeName = (meta.projectName || '(이름 없음)').replace(/[\\/:*?"<>|]/g, '_').trim() || '(이름 없음)'
          let target = path.join(trashRoot, `${safeName} (${stamp})`)
          let c = 2
          while (fs.existsSync(target)) { target = path.join(trashRoot, `${safeName} (${stamp})_${c}`); c++ }
          try {
            fs.renameSync(dir, target)
            renamed++
          } catch (err) {
            console.warn(`[trash-migrate] 이름 변경 실패 ${dir}:`, err.message)
          }
        }
      }
      if (removed > 0) console.log(`[trash-cleanup] 30일 지난 휴지통 폴더 ${removed}개 영구 삭제`)
      if (renamed > 0) console.log(`[trash-migrate] 옛 이름 ${renamed}개 → 새 형식으로 변경`)
    } catch (e) {
      console.warn('[trash-cleanup] 휴지통 스캔 실패:', e.message)
    }

    // 추가 마이그레이션 — 서버에서 이미 복구된 (=alive) 프로젝트의 로컬 _Trash 잔재 정리.
    // 시작 시 한 번 서버 상태 조회해서 비교. 신규 사용자/업데이트 사용자 모두 해당.
    try {
      const { projects } = await this.api.getProjectsByUid()
      const aliveIds = new Set(
        (projects || [])
          .filter(p => !p.deletedAt)
          .map(p => p.id)
      )
      let restoredCleaned = 0
      const remaining = fs.readdirSync(trashRoot, { withFileTypes: true })
      for (const e of remaining) {
        if (!e.isDirectory()) continue
        const dir = path.join(trashRoot, e.name)
        let metaPid = null
        try {
          const meta = JSON.parse(fs.readFileSync(path.join(dir, '.meta.json'), 'utf8'))
          metaPid = meta.projectId
        } catch {}
        if (metaPid && aliveIds.has(metaPid)) {
          // 서버에서 살아있는 프로젝트 = 복구됨 → 원본을 원래 자리로 돌려놓는다
          try {
            this.restoreProjectFromLocalTrash(dir)
            restoredCleaned++
          } catch (err) {
            console.warn(`[trash-restore-cleanup] 시작 시 정리 실패 ${dir}:`, err.message)
          }
        }
      }
      if (restoredCleaned > 0) console.log(`[trash-restore-cleanup] 이미 복구된 프로젝트의 로컬 잔재 ${restoredCleaned}개 정리`)
    } catch (e) {
      console.warn('[trash-restore-cleanup] 서버 조회 실패 — 시작 시 마이그레이션 스킵:', e.message)
    }
  }

  async retryFile(relPath) {
    const filePath = this.failedFiles.get(relPath)
    if (!filePath) return
    this.limitHit = null
    delete this.state.syncedFiles[relPath]
    this.failedFiles.delete(relPath)
    await this.syncFile(filePath)
  }

  async retryAllFailed() {
    this.limitHit = null
    const failed = [...this.failedFiles.entries()]
    for (const [relPath, filePath] of failed) {
      delete this.state.syncedFiles[relPath]
      this.failedFiles.delete(relPath)
      await this.syncFile(filePath)
    }
  }

  // 전체 폴더 재스캔 — 누락된 파일 찾아서 다시 업로드
  async rescan() {
    if (!this.watchDir) return

    const allFiles = []
    const scanDir = (dir, depth = 0) => {
      if (depth > 5) return
      try {
        const entries = fs.readdirSync(dir, { withFileTypes: true })
        for (const e of entries) {
          if (e.name.startsWith('.') || isIgnored(e.name)) continue
          const full = path.join(dir, e.name)
          if (e.isDirectory()) { scanDir(full, depth + 1); continue }
          if (isSupported(full)) allFiles.push(full)
        }
      } catch {}
    }
    scanDir(this.watchDir)
    // syncedFiles에 없는 것 + 실패한 것 재시도
    const unsyncedFiles = allFiles.filter(f => {
      const rel = this.getRelPath(f)
      return !this.state.syncedFiles[rel]
    })
    const failed = [...this.failedFiles.entries()]

    const total = unsyncedFiles.length + failed.length
    if (total === 0) {
      this.onProgress({ phase: 'watching', total: Object.keys(this.state.syncedFiles).length, completed: Object.keys(this.state.syncedFiles).length })
      return
    }

    this.onProgress({ phase: 'scanning', total, completed: 0 })

    let i = 0
    for (const filePath of unsyncedFiles) {
      await this.syncFile(filePath)
      i++
      this.onProgress({ phase: 'syncing', total, completed: i })
    }
    for (const [relPath, filePath] of failed) {
      delete this.state.syncedFiles[relPath]
      this.failedFiles.delete(relPath)
      await this.syncFile(filePath)
      i++
      this.onProgress({ phase: 'syncing', total, completed: i })
    }

    this.onProgress({ phase: 'watching', total: Object.keys(this.state.syncedFiles).length, completed: Object.keys(this.state.syncedFiles).length })
    await this._첫사이트()
  }

  /* 올리기가 한 차례 끝나면 사이트까지 — 2026-09-25 검수.
     예전엔 폴더를 다 올려도 사이트엔 하나도 안 나왔다 (앱이 만든 프로젝트는 '공개' 표시가 없었다).
     인스타 가져오기와 같은 서버 함수를 쓴다: 최신 12개까지 공개, 사이트가 없으면 읽기 좋은 주소로 발행. */
  async _첫사이트() {
    if (!this.newProjectIds.size) return
    const ids = [...this.newProjectIds]
    this.newProjectIds.clear()
    try {
      const site = await this.api.firstSiteFromSync(ids)
      if (site?.slug) this.onProgress({ phase: 'site_ready', slug: site.slug, created: !!site.created, madePublic: site.madePublic || 0 })
    } catch (e) { console.warn('[첫사이트] 실패:', e.message) }
  }

  async syncStateWithServer() {
    /* ⚠️ 예전엔 여기서 "스캔 중 · 파일 0개를 찾았어요" 를 띄웠다 — 2026-09-08.
       이 함수는 30초마다 도는 뒷정리인데, 시작만 알리고 **끝났다는 신호를 안 보냈다.**
       그래서 화면이 영영 "0개" 에 머물렀다. 파일이 234개 있고 업로드가 잘 되는데도.
       뒷정리는 조용히 하고, 끝나면 진짜 개수를 알린다. */

    // Get all projects for this user from backend
    const { projects } = await this.api.getProjectsByUid()

    /* ⚠️ 살아 있는 프로젝트를 먼저 고른다 — 2026-09-08 실제 폴더 시험이 잡았다.
          같은 이름이 둘(하나는 휴지통)이면 나중 것이 덮어써서 휴지통 쪽에 붙을 수 있었다.
          휴지통 것만 있으면 deletedAt 을 같이 적어 두고, 쓸 때 되살린다. */
    /* ⚠️ 프로젝트를 **이름으로만** 찾으면 웹에서 이름을 바꾼 순간 못 찾는다 — 2026-09-09.
          서버가 심어 준 폴더 경로·폴더 이름을 열쇠로 함께 들고 찾는다.
          찾는 순서: 폴더 경로 → 폴더 이름(후보 하나일 때) → 이름. */
    const serverProjects = new Map()
    const byFolderKey = new Map()
    const folderNameHits = new Map()
    for (const proj of projects) {
      const rec = { id: proj.id, name: proj.name, category: proj.category, syncedCategory: proj.syncedCategory, folderKey: proj.folderKey, folderName: proj.folderName, deletedAt: proj.deletedAt || null }
      const prev = serverProjects.get(proj.name)
      if (!(prev && !prev.deletedAt && proj.deletedAt)) serverProjects.set(proj.name, rec)
      if (proj.folderKey && !proj.deletedAt) byFolderKey.set(proj.folderKey, rec)
      if (proj.folderName && !proj.deletedAt) {
        const list = folderNameHits.get(proj.folderName) || []
        list.push(rec); folderNameHits.set(proj.folderName, list)
      }
    }
    const byFolderName = new Map()
    for (const [nm, list] of folderNameHits) if (list.length === 1) byFolderName.set(nm, list[0])
    const 서버프로젝트찾기 = (key, name) =>
      byFolderKey.get(key) || byFolderName.get(name) || serverProjects.get(name)

    // Scan local folders
    const localFolders = new Map()
    const scanDir = (dir, depth = 0) => {
      if (depth > 5) return
      try {
        const entries = fs.readdirSync(dir, { withFileTypes: true })
        for (const e of entries) {
          if (e.name.startsWith('.') || isIgnored(e.name)) continue
          const full = path.join(dir, e.name)
          if (e.isDirectory()) {
            scanDir(full, depth + 1)
          } else if (e.isFile() && isSupported(e.name)) {
            const rel = path.relative(this.watchDir, full)
            const parts = rel.split(path.sep)
            if (parts.length < 2) continue
            const key = parts.slice(0, -1).join('/')
            const name = parts[parts.length - 2]
            if (!localFolders.has(key)) {
              localFolders.set(key, { name, files: [] })
            }
            localFolders.get(key).files.push({ full, fileName: e.name, relPath: parts.join('/') })
          }
        }
      } catch {}
    }
    scanDir(this.watchDir)

    // Match local folders with server projects, rebuild sync state
    // 먼저: 서버 카테고리가 바뀌었으면 로컬 폴더 이동 (웹 → 로컬 동기화)
    for (const [key, folder] of localFolders) {
      const sp = 서버프로젝트찾기(key, folder.name)
      if (!sp) continue

      const parts = key.split('/')
      // [V1.9.4] 2단계 구조 (FOLDER/file) — 폴더명 = 카테고리 = 프로젝트
      // 서버 카테고리가 폴더명과 다르면 서버를 폴더명으로 업데이트 (잘못된 FASHION 등 치환)
      if (parts.length === 1) {
        const localCat = normalizeCategory(parts[0]) || parts[0]
        const serverCat = normalizeCategory(sp.category)
        if (serverCat !== localCat) {
          try {
            await this.api.updateProject(sp.id, { category: localCat })
            console.log(`[Category sync] ${sp.name}: "${sp.category}" → "${localCat}"`)
          } catch (e) {
            console.warn(`[Category sync] ${sp.name} 실패: ${e.message}`)
          }
        }
      }
      /* ⚠️ **사용자 폴더를 우리가 옮기지 않는다** — 2026-09-09.
            예전엔 로컬 카테고리가 서버와 다르면 "서버가 바뀐 것" 으로 보고
            사용자의 폴더를 서버에 맞춰 **되돌려 옮기고, 빈 카테고리 폴더는 지웠다.**

            그런데 앱은 둘을 구별하지 못한다:
              (가) 웹에서 분류를 바꿨다  → 옮기는 게 맞다
              (나) **사람이 폴더를 새로 정리했다** → 옮기면 정리한 것이 되돌아간다

            폴더를 정리하고 다시 연결하면 늘 (나) 인데 (가) 로 처리해서,
            공들여 정리한 구조가 조용히 원래대로 돌아갔다. 빈 폴더까지 지웠다.

            이 앱의 규칙은 **로컬이 기준**이다. 맥 폴더가 정답이므로 서버를 폴더에 맞춘다.
            남의 컴퓨터에 있는 파일을 묻지 않고 옮기는 것보다, 웹의 분류가 따라오는 편이 안전하다. */
      /* ── 분류 맞추기 (양방향) ──────────────────────────────
         ⚠️ 예전엔 "다르다" 만 알고 **누가 바꿨는지는 추측**했다. 늘 "웹이 바뀐 것" 으로 보고
            사용자 폴더를 되돌려 옮겼다. 폴더를 정리한 사람에게는 재앙이었다.
         이제 **마지막으로 맞췄을 때의 값(syncedCategory)** 을 같이 들고 있으므로
         어느 쪽이 바뀌었는지 계산으로 안다. 계산으로 알면 물어볼 이유가 없다 — 자동으로 한다.
         둘 다 바뀐 드문 경우는 **폴더를 따른다** (이 앱은 로컬이 기준이다). */
      if (parts.length >= 2) {
        const 폴더분류 = normalizeCategory(parts[0]) || parts[0]
        const 웹분류 = normalizeCategory(sp.category) || sp.category || ''
        const 기준 = sp.syncedCategory ? (normalizeCategory(sp.syncedCategory) || sp.syncedCategory) : null

        if (폴더분류 && 폴더분류 !== 웹분류) {
          /* 웹이 기준과 다르면 = 웹에서 바꿨다 → 폴더를 옮긴다.
             (폴더도 같이 바뀐 드문 경우도 웹을 따른다 — 규칙을 하나로 둔다) */
          const 웹에서바꿨다 = !!기준 && 웹분류 !== 기준
          if (웹에서바꿨다) {
            /* 웹에서 바꾼 게 확실하다 → 폴더를 옮긴다. 사용자가 이미 뜻을 밝힌 것이다. */
            const 옮김 = this._폴더분류옮기기(key, parts, sp.category || '미분류', localFolders, folder)
            if (옮김) {
              this._분류알림 = this._분류알림 || []
              this._분류알림.push({ 이름: sp.name, 에서: 폴더분류, 으로: 웹분류 })
              try { await this.api.updateProject(sp.id, { syncedCategory: sp.category || '미분류' }) } catch {}
              continue
            }
            /* 못 옮겼으면(권한·잠김) 웹을 폴더에 맞춘다 — 어긋난 채 두지 않는다 */
          }
          try {
            await this.api.updateProject(sp.id, { category: 폴더분류, syncedCategory: 폴더분류 })
            console.log(`[분류] 폴더를 따랐다: ${sp.name} ${웹분류 || '(없음)'} → ${폴더분류}`)
          } catch (e) {
            console.warn(`[분류] 웹을 못 바꿨다: ${sp.name} — ${e?.message}`)
          }
        } else if (폴더분류 && 기준 !== 폴더분류) {
          /* 값은 같은데 기준이 안 적혀 있다 (옛 자료) — 지금 값을 기준으로 심어 둔다 */
          try { await this.api.updateProject(sp.id, { syncedCategory: 폴더분류 }) } catch {}
        }
      }
    }

    // 다시 스캔 (폴더 이동 후 변경된 구조 반영)
    localFolders.clear()
    scanDir(this.watchDir)

    for (const [key, folder] of localFolders) {
      const sp = 서버프로젝트찾기(key, folder.name)
      if (!sp) continue

      /* 폴더가 맥에 있다는 건 사용자가 이 프로젝트를 원한다는 뜻 — 휴지통에 있으면 되살려서 쓴다.
         (서버 findOrCreateProject 가 프로젝트와 딸린 사진을 함께 꺼낸다) */
      if (sp.deletedAt) {
        try {
          const cat = key.split('/').length >= 2 ? (normalizeCategory(key.split('/')[0]) || key.split('/')[0]) : null
          const r = await this.api.findOrCreateProject(sp.name, cat, key)
          if (r?.projectId) { sp.id = r.projectId; sp.deletedAt = null; console.log(`[adopt] 휴지통에 있던 프로젝트를 되살렸다: ${sp.name}`) }
        } catch (e) { console.warn('[adopt] 프로젝트를 못 되살렸다:', e?.message); continue }
      }

      this.projectCache.set(key, sp.id)
      this.approvedFolders.add(key)

      /* ⚠️ **열쇠를 여기서 심는다** — 2026-09-09.
            이 길로 찾은 프로젝트는 서버를 안 부르므로(캐시에 넣고 끝),
            서버가 열쇠를 심을 기회가 없다. 그러면 옛 사용자는 영영 이름으로만 이어져서
            웹에서 이름을 바꾸는 순간 또 두 벌이 된다.
            폴더를 한 번 훑기만 해도 옛 자료가 열쇠를 갖도록 여기서 채운다. */
      if (sp.folderKey !== key || sp.folderName !== folder.name || !sp.syncedName) {
        try {
          await this.api.updateProject(sp.id, {
            folderKey: key,
            folderName: folder.name,
            syncedName: folder.name,
            ...(sp.syncedCategory ? {} : { syncedCategory: sp.category || '' }),
          })
          sp.folderKey = key; sp.folderName = folder.name; sp.syncedName = folder.name
          console.log(`[열쇠] 폴더 경로를 심었다: ${sp.name} ← ${key}`)
        } catch (e) { console.warn('[열쇠] 못 심었다:', e?.message) }
      }

      // 로컬 → 서버 카테고리 동기화 (서버에 카테고리가 없는 경우)
      const parts = key.split('/')
      if (parts.length >= 2) {
        const rawCat = parts[0]
        const category = normalizeCategory(rawCat)
        if (!sp.category && category) {
          try {
            await this.api.updateProject(sp.id, { category })
          } catch {}
        }
      }

      // Get assets for this project from backend
      const { assets } = await this.api.getAssetsByProject(sp.id)
      const serverFiles = new Set()
      const assetMap = new Map()
      for (const asset of assets) {
        /* 살아 있는 것을 우선 기억한다 — 같은 이름이 둘이면 휴지통 든 쪽이 덮어쓰지 않게 */
        const prev = assetMap.get(asset.fileName)
        if (prev && !prev.deletedAt && asset.deletedAt) continue
        serverFiles.add(asset.fileName)
        assetMap.set(asset.fileName, { assetId: asset.id, storagePath: asset.storagePath || '', fileSize: asset.fileSize, deletedAt: asset.deletedAt || null })
      }

      for (const f of folder.files) {
        if (!serverFiles.has(f.fileName)) continue
        const info = assetMap.get(f.fileName)
        let localSize = null
        try { localSize = fs.statSync(path.join(this.watchDir, f.relPath)).size } catch {}
        const 같은파일인가 = !(info.fileSize > 0 && localSize != null && localSize !== info.fileSize)

        /* ⚠️ **이미 올렸다고 적혀 있어도, 웹에서 휴지통에 들어가 있으면 꺼낸다** — 2026-09-09.
              테스터 제보: "동기화 새로고침 계속 해봤는데 웹에서는 자꾸 파일이 안 보이네."
              서버가 같은 이름 사진을 찾으면 내용만 덮어쓰고 휴지통 표시는 안 지웠다.
              앱은 성공으로 적고 넘어가니, 다시 눌러도 **스스로 낫지 않는 상태**가 됐다.
              그렇게 갇힌 사진들을 여기서 되살린다. 크기가 다르면 다른 파일이므로 건드리지 않는다. */
        if (info.deletedAt && 같은파일인가 && this.state.syncedFiles[f.relPath]) {
          console.log(`[복구] 올린 걸로 적혀 있는데 휴지통에 있다 — 꺼낸다: ${f.relPath}`)
          try { await this.api.untrashAssets([info.assetId]); info.deletedAt = null }
          catch (e) { console.warn('[복구] 못 꺼냈다:', e?.message) }
        }
        if (this.state.syncedFiles[f.relPath]) continue

        /* ⚠️ **휴지통에 든 사진을 '이미 올라간 것' 으로 삼으면 안 된다** — 2026-09-08.
              실제 폴더 시험에서 잡혔다: 기록을 비우고 다시 동기화하면, 휴지통에 든 사진을
              이름·크기가 같다는 이유로 그대로 '올라간 것' 으로 적고 넘어갔다. 웹에서는 안 보인다.
              파일이 맥 폴더에 있다는 건 사용자가 원한다는 뜻이다 — 휴지통에서 꺼낸다. */
        if (info.deletedAt && !(info.fileSize > 0 && localSize != null && localSize !== info.fileSize)) {
          console.log(`[adopt] 휴지통에 든 사진과 같다 — 꺼내서 쓴다: ${f.relPath}`)
          try { await this.api.untrashAssets([info.assetId]) } catch (e) { console.warn('[adopt] 휴지통에서 못 꺼냈다:', e?.message); continue }
        }

        /* 이름이 같다고 같은 파일은 아니다.
           서버가 크기를 알려준 경우엔 비교해서, 다르면 '이미 올라간 것' 으로 치지 않는다.
           그래야 컴퓨터에서 바꾼 파일이 그대로 올라간다 (로컬이 기준이다). */
        if (info.fileSize > 0 && localSize != null && localSize !== info.fileSize) {
          console.log(`[adopt] 이름은 같은데 내용이 다르다 — 새 파일로 본다: ${f.relPath}`)
          continue
        }

        this.state.syncedFiles[f.relPath] = {
          assetId: info.assetId,
          projectId: sp.id,
          storagePath: info.storagePath,
          /* ⚠️ 크기를 반드시 같이 적는다 — 2026-09-07 테스터 제보의 진짜 원인.
             나중에 웹에서 '이 프로젝트 지웠다' 는 신호가 왔을 때, 지금 디스크에 있는
             파일이 정말 그 자산인지 확인할 유일한 근거가 이것이다.
             여기가 비어 있으면 확인을 못 해서 멀쩡한 폴더가 통째로 휴지통에 딸려갔다. */
          fileSize: info.fileSize > 0 ? info.fileSize : localSize,
          adopted: true,
        }
      }
    }

    // ── 웹에서 asset이 다른 프로젝트로 이동된 경우 → 로컬 파일도 따라 이동 ──
    // 모든 서버 자산을 (assetId → {projectId, fileName, projectName, category}) 인덱스로 모음
    try {
      const assetIndex = new Map()
      for (const sp of projects) {
        try {
          const { assets } = await this.api.getAssetsByProject(sp.id)
          for (const asset of assets) {
            assetIndex.set(asset.id, {
              projectId: sp.id,
              projectName: sp.name,
              category: sp.category,
              fileName: asset.fileName,
              영상인가: !!asset.isVideo,
              표지: asset.videoThumbnailUrl || asset.thumbnailUrl || null,
              프로젝트표지: sp.thumbnailUrl || null,
            })
          }
        } catch {}
      }

      /* ⚠️ **표지 없는 옛 영상을 채운다** — 2026-09-09.
            1.9.64 부터 영상을 올릴 때 길이의 10% 지점을 표지로 뽑는다. 하지만 그 전에
            올라간 영상에는 표지가 없다. Bunny 인코딩이 실패했으면 영영 안 생긴다.
            표지 없는 영상이 포트폴리오 커버에 걸리면 "커버를 바꿨는데 옛날 것으로
            돌아간다" 로 보인다 (대표님이 겪은 dioor.mp4 가 이 경우다).
            컴퓨터에 원본 파일이 아직 있으면 지금 뽑아서 채운다. 한 번만 하면 된다. */
      try {
        for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
          const 서버 = assetIndex.get(entry.assetId)
          if (!서버 || !서버.영상인가 || 서버.표지) continue
          const 원본 = path.join(this.watchDir, ...relPath.split('/'))
          if (!fs.existsSync(원본)) continue
          const 표지 = await this._영상표지뽑기(원본).catch(() => null)
          if (!표지) continue
          const 이름 = relPath.split('/').pop().replace(/\.[^.]+$/, '')
          const 경로 = `thumbnails/${this.uid}/${서버.projectId}/${Date.now()}_${이름}.jpg`
          const 주소 = await this.uploadToStorage(표지, 경로, 'image/jpeg')
          await this.api.updateAsset(entry.assetId, { videoThumbnailUrl: 주소 })
          if (!서버.프로젝트표지) {
            await this.api.updateProject(서버.projectId, { thumbnailUrl: 주소 }).catch(() => {})
          }
          서버.표지 = 주소
          console.log(`[표지] 옛 영상에 표지를 채웠다: ${relPath}`)
        }
      } catch (e) { console.warn('[표지] 옛 영상 채우기 실패:', e?.message) }

      // state.syncedFiles 순회하며 projectId/fileName 어긋남 감지
      const moveOps = []
      for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
        const serverAsset = assetIndex.get(entry.assetId)
        if (!serverAsset) continue // 서버에서 삭제된 asset — 건드리지 않음 (다른 로직이 처리)

        const serverProjectId = serverAsset.projectId
        const serverFileName = serverAsset.fileName
        const localFileName = relPath.split('/').pop()

        if (serverProjectId === entry.projectId && serverFileName === localFileName) continue
        // 어긋남 발견 — 웹에서 이동/이름변경됨
        moveOps.push({ relPath, entry, serverAsset })
      }

      for (const op of moveOps) {
        const { relPath, entry, serverAsset } = op
        const oldFullPath = path.join(this.watchDir, ...relPath.split('/'))
        if (!fs.existsSync(oldFullPath)) {
          // 로컬에 파일 없음 — state만 정리
          delete this.state.syncedFiles[relPath]
          continue
        }

        // 새 위치 결정: {watchDir}/{category}/{projectName}/{fileName}
        const newCategory = serverAsset.category || '미분류'
        const newProjectName = serverAsset.projectName
        const newFileName = serverAsset.fileName
        const newFullPath = path.join(this.watchDir, newCategory, newProjectName, newFileName)
        const newRelPath = path.relative(this.watchDir, newFullPath).split(path.sep).join('/')

        if (oldFullPath === newFullPath) continue

        try {
          // 대상 폴더 준비
          const newDir = path.dirname(newFullPath)
          if (!fs.existsSync(newDir)) fs.mkdirSync(newDir, { recursive: true })

          // 충돌 방지: 같은 위치에 이미 파일 있으면 skip (state만 업데이트)
          if (fs.existsSync(newFullPath)) {
            console.log(`[RemoteMove] 대상 위치에 파일 존재, state만 갱신: ${newRelPath}`)
          } else {
            // chokidar가 unlink+add 던질 텐데 state를 미리 새 경로로 박아두면 syncFile/tryMatchRename 모두 무시함
            this.state.syncedFiles[newRelPath] = {
              ...entry,
              projectId: serverAsset.projectId,
              syncedAt: new Date().toISOString(),
            }
            delete this.state.syncedFiles[relPath]

            // 새 폴더는 자동 승인 (웹에서의 명시적 이동이므로)
            const newProjectKey = [newCategory, newProjectName].join('/')
            this.approvedFolders.add(newProjectKey)
            this.projectCache.set(newProjectKey, serverAsset.projectId)

            fs.renameSync(oldFullPath, newFullPath)
            console.log(`[RemoteMove] 로컬 파일 이동: ${relPath} → ${newRelPath}`)
            this.onFileStatus({ path: newRelPath, status: 'renamed', fileName: newFileName, oldFileName: relPath.split('/').pop() })
          }

          // 옛 폴더가 비었으면 정리
          try {
            const oldDir = path.dirname(oldFullPath)
            const remaining = fs.readdirSync(oldDir).filter(n => !n.startsWith('.') && !IGNORED.has(n.toLowerCase()))
            if (remaining.length === 0) fs.rmdirSync(oldDir)
          } catch {}
        } catch (mvErr) {
          console.warn(`[RemoteMove] 이동 실패 ${relPath} → ${newRelPath}: ${mvErr.message}`)
        }
      }
    } catch (err) {
      console.warn('[RemoteMove] 처리 실패:', err.message)
    }

    this.saveState()
    /* 뒷정리가 끝났다고 알린다 — 안 알리면 화면이 이전 상태에 머문다 */
    const n = Object.keys(this.state.syncedFiles).length
    this.onProgress({ phase: 'watching', total: n, completed: n })
  }

  /* ── 웹을 비우고 내 폴더 기준으로 다시 올리기 ──
     폴더를 정리하면(이름 바꾸기·옮기기·합치기) 웹에는 옛 프로젝트가 남는다.
     그 상태로 다시 연결하면 같은 작업이 두 벌이 되거나 엉뚱한 곳에 붙는다.
     여기서 웹을 휴지통에 넣고, 앱 기록을 비우고, 폴더를 처음부터 다시 읽는다.

     ⚠️ **맥 폴더의 파일은 하나도 건드리지 않는다.** 웹 쪽만 다시 그린다.
     ⚠️ 지우는 게 아니라 휴지통이다. 폴더에 같은 이름이 있으면 되살아나고,
        폴더에 없는 것만 휴지통에 남는다.
     ⚠️ 폴더가 웹보다 훨씬 적으면 **막는다.** 외장 디스크가 빠졌거나 폴더를 옮긴
        상태에서 누르면 웹에 있던 작업이 통째로 사라진 것처럼 보인다. */
  /* 포맷하면 **다시 올라갈 파일이 몇 개인지** 미리 센다.
     0 이면 파일을 하나도 다시 안 올린다는 뜻이다 (휴지통에서 꺼내 쓴다).
     0 이 아니면 그만큼 용량과 시간이 또 든다 — 사람에게 그 숫자를 보여주고 정하게 한다.

     짝을 찾는 방법은 서버와 같다: 폴더 경로 → 폴더 이름 → 이름.
     셋 다 안 맞는 폴더의 파일이 다시 올라간다. */
  async 포맷미리보기() {
    const { projects } = await this.api.getProjectsByUid()
    const 전부 = projects || []
    const 경로 = new Set(전부.map(p => p.folderKey).filter(Boolean))
    const 폴더이름수 = new Map()
    for (const p of 전부) if (p.folderName) 폴더이름수.set(p.folderName, (폴더이름수.get(p.folderName) || 0) + 1)
    const 이름 = new Set(전부.map(p => p.name).filter(Boolean))

    const 폴더별 = new Map()
    const 훑기 = (dir, depth = 0) => {
      if (depth > 5) return
      let entries = []
      try { entries = fs.readdirSync(dir, { withFileTypes: true }) } catch { return }
      for (const e of entries) {
        if (e.name.startsWith('.') || isIgnored(e.name)) continue
        const full = path.join(dir, e.name)
        if (e.isDirectory()) { 훑기(full, depth + 1); continue }
        if (!isSupported(full)) continue
        const key = this.getProjectKey(full)
        if (!key) continue
        폴더별.set(key, (폴더별.get(key) || 0) + 1)
      }
    }
    훑기(this.watchDir)

    let 다시올릴파일 = 0
    const 다시올릴폴더 = []
    for (const [key, 개수] of 폴더별) {
      const 폴더이름 = key.split('/').pop()
      const 짝있음 = 경로.has(key) || 폴더이름수.get(폴더이름) === 1 || 이름.has(폴더이름)
      if (!짝있음) { 다시올릴파일 += 개수; 다시올릴폴더.push({ 폴더: key, 파일: 개수 }) }
    }
    return {
      웹프로젝트: 전부.filter(p => !p.deletedAt).length,
      폴더프로젝트: 폴더별.size,
      전체파일: [...폴더별.values()].reduce((a, b) => a + b, 0),
      다시올릴파일,
      다시올릴폴더: 다시올릴폴더.slice(0, 20),
    }
  }

  async 폴더기준으로다시(옵션 = {}) {
    const { 강행 = false } = 옵션
    const 미리 = await this.api.resetProjectsForResync({ apply: false })
    const 웹프로젝트 = 미리?.프로젝트수 || 0

    /* 지금 폴더에 프로젝트가 몇 개인지 센다 (파일이 든 마지막 폴더 = 프로젝트) */
    const 폴더들 = new Set()
    const 훑기 = (dir, depth = 0) => {
      if (depth > 5) return
      let entries = []
      try { entries = fs.readdirSync(dir, { withFileTypes: true }) } catch { return }
      for (const e of entries) {
        if (e.name.startsWith('.') || isIgnored(e.name)) continue
        const full = path.join(dir, e.name)
        if (e.isDirectory()) 훑기(full, depth + 1)
        else if (isSupported(full)) {
          const key = this.getProjectKey(full)
          if (key) 폴더들.add(key)
        }
      }
    }
    훑기(this.watchDir)
    const 폴더프로젝트 = 폴더들.size

    if (!강행 && 웹프로젝트 > 0 && 폴더프로젝트 < Math.ceil(웹프로젝트 * 0.5)) {
      return {
        막힘: true, 웹프로젝트, 폴더프로젝트,
        안내: `지금 폴더에는 프로젝트가 ${폴더프로젝트}개인데 웹에는 ${웹프로젝트}개예요. `
            + '폴더가 너무 적어서 멈췄습니다. 외장 디스크가 빠졌거나 폴더를 옮긴 건 아닌지 확인해 주세요.',
      }
    }

    const r = await this.api.resetProjectsForResync({ apply: true })

    /* 앱 기록을 비운다 — 이게 있어야 폴더를 처음부터 다시 읽는다 */
    this.state = { syncedFiles: {} }
    this.saveState()
    this.projectCache.clear()
    this.approvedFolders.clear()

    return { ...r, 웹프로젝트, 폴더프로젝트, 다시시작필요: true }
  }

  /* 웹에서 바꾼 분류에 맞춰 **프로젝트 폴더 하나**를 옮긴다.
     ⚠️ 이 길은 "웹이 바뀐 게 확실할 때" 만 온다 (기준값으로 계산해서 안다).
     ⚠️ 빈 분류 폴더를 지우지 않는다 — 사용자가 만들어 둔 자리일 수 있다. */
  _폴더분류옮기기(key, parts, 새분류, localFolders, folder) {
    try {
      const 프로젝트폴더 = parts.slice(1).join(path.sep)
      const 옛경로 = path.join(this.watchDir, parts[0], 프로젝트폴더)
      const 새폴더 = path.join(this.watchDir, 새분류)
      const 새경로 = path.join(새폴더, 프로젝트폴더)
      if (!fs.existsSync(옛경로)) return false
      if (fs.existsSync(새경로)) return false          // 같은 이름이 이미 있으면 손대지 않는다
      if (!fs.existsSync(새폴더)) fs.mkdirSync(새폴더, { recursive: true })
      const 새경로부모 = path.dirname(새경로)
      if (!fs.existsSync(새경로부모)) fs.mkdirSync(새경로부모, { recursive: true })
      fs.renameSync(옛경로, 새경로)

      const 새키 = [새분류, ...parts.slice(1)].join('/')
      const 옛앞 = key + '/'
      const 새앞 = 새키 + '/'
      for (const [rel, entry] of Object.entries(this.state.syncedFiles)) {
        if (!rel.startsWith(옛앞)) continue
        this.state.syncedFiles[새앞 + rel.slice(옛앞.length)] = entry
        delete this.state.syncedFiles[rel]
      }
      if (this.projectCache.has(key)) { this.projectCache.set(새키, this.projectCache.get(key)); this.projectCache.delete(key) }
      if (this.approvedFolders.has(key)) { this.approvedFolders.add(새키); this.approvedFolders.delete(key) }
      if (localFolders && folder) { localFolders.set(새키, folder); localFolders.delete(key) }
      this.saveState()
      console.log(`[분류] 웹에서 바꾼 대로 폴더를 옮겼다: ${key} → ${새키}`)
      return true
    } catch (e) {
      console.warn('[분류] 폴더를 못 옮겼다:', e?.message)
      return false
    }
  }

  async start() {
    this._startedAt = Date.now()  // 안전망: 시작 직후 30초간 unlinkDir 무시 (chokidar 초기화 노이즈 방지)
    await this.syncStateWithServer()

    const initialFiles = []

    this.watcher = chokidar.watch(this.watchDir, {
      persistent: true,
      ignoreInitial: false,
      depth: 5,
      awaitWriteFinish: { stabilityThreshold: 2000, pollInterval: 200 },
      ignored: (p) => {
        const name = p.split(/[/\\]/).pop()
        // _Trash 폴더 전체 무시 — 휴지통 이동 시 chokidar 가 unlink/add 트리거 못 하게
        if (p.includes(`${path.sep}_Trash${path.sep}`) || p.endsWith(`${path.sep}_Trash`)) return true
        return name.startsWith('.') || isIgnored(name)
      },
    })

    await new Promise((resolve) => {
      this.watcher.on('add', (p) => {
        if (!isSupported(p)) return
        if (!this._ready) { initialFiles.push(p); return }
        // Check if this is a rename (matching a pending unlink)
        if (this.tryMatchRename(p)) return
        this.syncFile(p)
      })

      this.watcher.on('addDir', (p) => {
        if (!this._ready) return
        this.tryMatchFolderRename(p)
      })

      this.watcher.on('unlink', (p) => {
        if (!isSupported(p)) return
        this.bufferUnlink(p)
      })

      this.watcher.on('unlinkDir', (p) => {
        this.bufferUnlinkDir(p)
      })

      this.watcher.on('ready', async () => {
        this._ready = true
        // 폴더별 그룹핑 후 파일명 알파벳 순 정렬 → 1번이 썸네일 됨
        initialFiles.sort((a, b) => a.localeCompare(b))
        this.onProgress({ phase: 'scanning', total: initialFiles.length, completed: 0 })

        for (let i = 0; i < initialFiles.length; i++) {
          await this.syncFile(initialFiles[i])
          this.onProgress({ phase: 'syncing', total: initialFiles.length, completed: i + 1 })
        }

        this.onProgress({ phase: 'watching', total: initialFiles.length, completed: initialFiles.length })
        await this._첫사이트()

        // 무압축 공유 업로드 폴링 시작
        this.startSharePolling()

        // 원격 카테고리 변경 감시 폴링 (30초 간격)
        this.startRemoteSyncPolling()

        // 기존 자산 contentHash 백필 (1회, 백그라운드)
        this.startContentHashBackfill()

        // 앱이 꺼진 사이 로컬에서 없어진 것 찾아내기 (되살리지 않기 + 사람에게 묻기)
        try { this.reconcileLocalDeletions() }
        catch (e) { console.warn('[reconcile] 실패:', e.message) }

        /* ⚠️ 자동 다운로드는 하지 않는다 — **로컬이 기준이다** (2026-09-04 대표님 결정).
           웹에서 뭘 고쳐도 로컬은 그대로 둔다. 웹 → 로컬은 사람이 버튼을 눌렀을 때만
           일어나고, 그때 "로컬에 덮어씌워진다" 고 미리 알린다.
           아래는 그 버튼이 쓸 기계를 준비만 해둔다 (타이머는 안 돈다). */
        this.prepareDownloadEngine()

        resolve()
      })
    })
  }

  // ── Phase 1 다운로드 폴링 ──
  // 30초마다 서버에 "since 이후 바뀐 내 자산" 물어봄. 있으면 로컬에 다운로드.
  // 다른 기기에서 올린 파일 → 여기로 내려옴 → atomicWrite → xattr 마킹 → 루프 방지.
  // 다운로드 풀스캔 즉시 실행 — 동기화 버튼에서 호출
  async triggerDownloadPollNow() {
    if (!this._downloadTick) {
      console.warn('[triggerDownloadPollNow] tick 함수 미준비 (startDownloadPolling 후에 가능)')
      return
    }
    console.log('[triggerDownloadPollNow] 수동 트리거 — 즉시 폴링 실행')
    return this._downloadTick()
  }

  /**
   * 웹 → 로컬 받기 기계를 준비한다. **타이머는 돌리지 않는다.**
   *
   * ⚠️ 예전엔 30초마다 저절로 돌면서 웹의 것을 로컬로 내려받았다.
   *    그래서 로컬을 정리해도 웹 구조가 도로 씌워졌다. 로컬이 기준이므로 자동으로는 안 받는다.
   *    받는 것은 `pullFromWebNow()` — 사람이 버튼을 눌렀을 때만이다.
   */
  prepareDownloadEngine() {
    return this.startDownloadPolling({ auto: false })
  }

  /**
   * 웹의 것을 로컬로 통째로 받아온다. **로컬을 덮어쓴다.**
   * 버튼에서만 부른다 — 부르는 쪽이 사용자에게 먼저 경고해야 한다.
   */
  async pullFromWebNow() {
    /* 사람이 '웹 것으로 맞춘다' 고 정했으므로, 치웠다는 표시도 지운다.
       안 지우면 지웠던 것들만 쏙 빠진 채로 받아져서 "덮어쓴다" 는 말과 안 맞는다. */
    this.restoreLocallyRemoved()
    if (this.state) {
      this.state.downloadSince = null
      this.state.downloadCursor = null
      this.saveState()
    }
    console.log('[pullFromWeb] 웹 → 로컬 전체 받기 시작 (사용자 요청)')
    return this.triggerDownloadPollNow()
  }

  startDownloadPolling({ auto = true } = {}) {
    const INTERVAL_MS = 30 * 1000
    // [v1.9.16] 진짜 영구 픽스 — bootstrap 모드는 hasMore=false 까지 cursor 로 끝까지 페이징.
    // 그 후에야 nextSince 박고 incremental 모드로 전환.
    // 매 시작 시 downloadSince 강제 null + cursor 도 null → 항상 처음부터 풀스캔.
    if (auto) {
      this.state.downloadSince = null
      this.state.downloadCursor = null
      this.saveState()
      console.log('[DownloadPoll] 시작 — full bootstrap (모든 자산 회수)')
    }

    const tick = async () => {
      try {
        const isBootstrap = !this.state.downloadSince
        let totalReceived = 0
        let safety = 0
        // 진단용 — 이번 tick 의 자산별 결과 요약
        const diag = { received: 0, downloaded: 0, skipped: {}, errors: [] }
        while (true) {
          safety++
          if (safety > 50) {
            console.warn('[DownloadPoll] safety break — 50 페이지 초과')
            break
          }
          const resp = await this.api.listAssetsSince(
            this.state.downloadSince,
            500,
            this.state.downloadCursor,
          )
          const { assets, nextSince, nextCursor, hasMore, mode, deletedProjects } = resp || {}
          // 웹에서 soft-delete 된 프로젝트 → 로컬 폴더 정리 (idempotent)
          if (Array.isArray(deletedProjects) && deletedProjects.length > 0) {
            /* ⚠️ 한 번 처리한 프로젝트는 다시 처리하지 않는다. — 2026-09-07
               앱을 켤 때마다 서버가 '삭제됨' 목록을 통째로 다시 내려준다. 예전엔 그때마다
               휴지통 옮기기를 또 돌려서, 사용자가 그 폴더를 다시 만들어 놓으면 또 쓸어갔다.
               (60초 dedupe 는 앱을 껐다 켜면 사라져서 소용이 없었다) */
            this.state.trashedProjects ||= {}
            for (const pid of deletedProjects) {
              const id = typeof pid === 'string' ? pid : pid?.id
              if (!id) continue
              if (this.state.trashedProjects[id]) continue   // 이미 처리함
              try {
                await this.removeDeletedProjectLocally(pid)
                this.state.trashedProjects[id] = new Date().toISOString()
                this.saveState()
              } catch (e) { console.warn(`[DownloadPoll] removeDeletedProjectLocally(${id}) 실패:`, e.message) }
            }
          }
          // 웹에서 복구된 프로젝트 → 로컬 _Trash 잔재 정리
          // 직전 폴링의 deletedProjects 와 이번 응답을 비교. 빠진 ID = 복구됨.
          // bootstrap 모드는 마지막 페이지에만 deletedProjects 가 실려서 그때만 비교.
          if (deletedProjects !== undefined && (mode === 'incremental' || !hasMore)) {
            const currentIds = new Set((deletedProjects || []).map(p => typeof p === 'string' ? p : p.id))
            const lastIds = new Set(this.state.lastDeletedProjectIds || [])
            const restoredIds = [...lastIds].filter(id => !currentIds.has(id))
            for (const rid of restoredIds) {
              /* 서버에서 되살아났다 → '처리함' 표시를 지운다.
                 안 지우면 나중에 진짜로 다시 지웠을 때 로컬이 안 따라간다. */
              if (this.state.trashedProjects) { delete this.state.trashedProjects[rid]; this.saveState() }
              try { await this.cleanupRestoredProjectLocally(rid) }
              catch (e) { console.warn(`[DownloadPoll] cleanupRestoredProjectLocally(${rid}) 실패:`, e.message) }
            }
            this.state.lastDeletedProjectIds = [...currentIds]
            this.saveState()
          }
          totalReceived += assets?.length || 0
          diag.received += assets?.length || 0
          console.log(`[DownloadPoll] page ${safety} mode=${mode} got=${assets?.length || 0} hasMore=${hasMore} cursor=${nextCursor || 'null'}`)
          if (Array.isArray(assets) && assets.length > 0) {
            // 진단 — 받은 자산 전체 목록 요약 (id, fileName, projectId, url 유무)
            console.log(`[DownloadPoll] received assets summary:`)
            for (const a of assets) {
              console.log(`  - ${a.fileName || a.id} | projId=${a.projectId} | url=${a.url ? 'YES' : 'NO'} | size=${a.fileSize} | isVideo=${!!a.isVideo} | sourceFile=${!!a.sourceFile}`)
            }
            for (const asset of assets) {
              try {
                const result = await this.downloadRemoteAsset(asset)
                if (result?.skipped) {
                  diag.skipped[result.reason] = (diag.skipped[result.reason] || 0) + 1
                } else if (result?.downloaded) {
                  diag.downloaded++
                }
              } catch (e) {
                diag.errors.push(`${asset.fileName || asset.id}: ${e.message}`)
                console.warn(`[DownloadPoll] ${asset.fileName || asset.id} 실패:`, e.message)
              }
            }
          }
          if (hasMore && nextCursor) {
            this.state.downloadCursor = nextCursor
            this.saveState()
            continue
          }
          if (nextSince) {
            this.state.downloadSince = nextSince
            this.state.downloadCursor = null
            this.saveState()
          }
          break
        }
        // 진단 결과 한 줄로 요약
        console.log(`[DownloadPoll] tick 완료 ─ ${isBootstrap ? 'BOOTSTRAP' : 'INCREMENTAL'}`)
        console.log(`  received: ${diag.received}`)
        console.log(`  downloaded: ${diag.downloaded}`)
        console.log(`  skipped:`, JSON.stringify(diag.skipped, null, 2))
        if (diag.errors.length > 0) console.log(`  errors:`, diag.errors)
        // 마지막 진단 결과 전역 보관 — DevTools 에서 window.__assiSyncDiag 로 조회
        this._lastDiag = diag
      } catch (e) {
        console.warn('[DownloadPoll] tick error:', e.message)
      }
    }

    this._downloadTick = tick  // 버튼에서 즉시 호출

    /* ⚠️ auto:false 면 타이머를 안 건다. 기계만 준비해두고 가만히 있는다.
       웹 → 로컬은 사람이 버튼을 눌렀을 때만 일어난다 (pullFromWebNow). */
    if (auto) {
      tick().catch(() => {})
      this._downloadTimer = setInterval(tick, INTERVAL_MS)
      if (this._downloadTimer.unref) this._downloadTimer.unref()
    } else {
      console.log('[DownloadPoll] 자동 받기 꺼짐 — 로컬이 기준. 버튼으로만 받는다')
    }

    // 로컬 휴지통 청소 — 시작 직후 1회 + 24시간마다
    this.cleanupLocalTrash().catch(() => {})
    this._trashCleanupTimer = setInterval(() => this.cleanupLocalTrash().catch(() => {}), 24 * 60 * 60 * 1000)
    if (this._trashCleanupTimer.unref) this._trashCleanupTimer.unref()
  }

  // 서버의 asset 1개를 로컬 폴더에 다운로드. 3중 방어선 적용.
  // 이미 로컬에 있는 파일(hash 일치)은 스킵. sourceFile/영상/오디오 일단 제외 (이미지만 V1).
  async downloadRemoteAsset(asset) {
    const tag = `[DRA:${asset.fileName || asset.id}]`
    if (!asset.url || !asset.fileName || !asset.projectId) {
      console.warn(`${tag} SKIP missing-fields url=${!!asset.url} fileName=${!!asset.fileName} projectId=${!!asset.projectId}`)
      return { skipped: true, reason: 'missing-fields', asset: { id: asset.id, fileName: asset.fileName, projectId: asset.projectId, hasUrl: !!asset.url } }
    }
    /* ⚠️ 한 번 로컬에서 없앤 것은 다시 안 받는다.
       예전엔 '디스크에 없으면 강제 재다운로드' 였다. 그래서 사용자가 로컬 폴더를 지우고
       정리해도, 앱을 켜면 서버 구조가 통째로 되살아나 정리한 것을 덮어썼다.
       (테스터 제보 2026-09-04) 로컬이 기준이므로 로컬에 없으면 없는 것이다. */
    if (this.isLocallyRemoved(asset.id)) {
      console.log(`${tag} SKIP locally-removed ─ 사용자가 로컬에서 치운 자산`)
      return { skipped: true, reason: 'locally-removed', asset: { id: asset.id, fileName: asset.fileName } }
    }
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      if (entry?.assetId === asset.id) {
        const absPath = path.join(this.watchDir, relPath)
        if (!fs.existsSync(absPath)) {
          console.warn(`${tag} SKIP locally-removed ─ ${relPath} 이 디스크에 없음 (사용자가 치운 것으로 본다)`)
          delete this.state.syncedFiles[relPath]
          this.markLocallyRemoved(asset.id, relPath)
          return { skipped: true, reason: 'locally-removed', asset: { id: asset.id, fileName: asset.fileName, relPath } }
        }
        console.log(`${tag} SKIP already-synced ─ ${relPath}`)
        return { skipped: true, reason: 'already-synced', asset: { id: asset.id, fileName: asset.fileName, relPath } }
      }
    }
    if (asset.fileSize != null && asset.fileSize === 0) {
      console.warn(`${tag} SKIP fileSize=0 (incomplete)`)
      return { skipped: true, reason: 'fileSize-zero', asset: { id: asset.id, fileName: asset.fileName } }
    }

    const project = await this.api.getProject(asset.projectId).catch((e) => {
      console.warn(`${tag} getProject ERROR ${e.message}`)
      return null
    })
    let category, projectName
    if (project) {
      console.log(`${tag} project loaded: name=${project.name} category=${project.category} uid=${project.uid?.slice(0,8)}...`)
      if (project.uid !== this.uid) {
        console.warn(`${tag} SKIP uid-mismatch project.uid=${project.uid?.slice(0,8)}... this.uid=${this.uid?.slice(0,8)}...`)
        return { skipped: true, reason: 'uid-mismatch', asset: { id: asset.id, fileName: asset.fileName, projectUid: project.uid, myUid: this.uid } }
      }
      category = project.category || 'MISC'
      projectName = project.name || asset.projectId
    } else {
      /* ⚠️ 프로젝트를 못 찾았다. 예전엔 _UNCATEGORIZED/<프로젝트ID>/ 로 떨궜는데,
         그러면 사용자 폴더에 정체불명의 영숫자 폴더가 생긴다 — 2026-09-07.
         「폴더는 카테고리/프로젝트명」 규칙과 정면으로 어긋난다.

         못 찾는 경우는 둘뿐이다.
         ① 잠깐 못 읽었다 (통신 문제, 삭제가 도는 중) → 다음 폴링에 다시 받으면 된다
         ② 프로젝트가 없어졌다 → 그 사진은 내려받을 자리가 없다
         어느 쪽이든 **갖다 놓을 자리를 지어내지 않는다.** 건너뛴다. */
      console.warn(`${tag} SKIP 프로젝트를 못 찾았다 (projectId=${asset.projectId}) — 자리를 지어내지 않는다`)
      return { skipped: true, reason: 'project-not-found', asset: { id: asset.id, fileName: asset.fileName, projectId: asset.projectId } }
    }
    console.log(`${tag} START → ${category}/${projectName}/${asset.fileName}`)
    // 폴더 규칙: watchDir/{카테고리}/{프로젝트명}/{파일명}
    const localDir = path.join(this.watchDir, category, projectName)
    const localPath = path.join(localDir, asset.fileName)
    const relPath = this.getRelPath(localPath)

    if (fs.existsSync(localPath)) {
      try {
        const localHash = await this.computeFileHash(localPath)
        if (localHash === asset.contentHash) {
          console.log(`${tag} SKIP hash-match (이미 디스크에 동일 내용)`)
          return { skipped: true, reason: 'hash-match', asset: { id: asset.id, fileName: asset.fileName, localPath } }
        }
      } catch {}
    }

    let buffer
    try {
      buffer = await this.api.downloadFile(asset.url)
      console.log(`${tag} fetched ${buffer.length} bytes from ${asset.url.slice(0, 60)}...`)
    } catch (e) {
      console.error(`${tag} downloadFile FAILED: ${e.message}`)
      throw e
    }

    // atomicWrite 로 쓰면서 Layer 1/2 동시 마킹
    const { atomicWrite } = require('./atomic-write')
    await atomicWrite(localPath, buffer, {
      hash: asset.contentHash,
      recentlyDownloaded: this.recentlyDownloaded,
      xattrId: asset.id,
    })

    // state 에 기록 (업로드 루프 재방지 — 이미 동기화된 파일로 표시)
    this.state.syncedFiles[relPath] = {
      assetId: asset.id,
      projectId: asset.projectId,
      storagePath: asset.storagePath || '',
      syncedAt: new Date().toISOString(),
      fileSize: buffer.length,
      contentHash: asset.contentHash || null,
      downloaded: true,
    }
    this.saveState()

    this.onFileStatus({
      path: relPath,
      status: 'done',
      progress: 100,
      fileName: asset.fileName,
      isVideo: false,
      size: fmt(buffer.length),
      phase: '다른 기기에서 받음',
      downloaded: true,
    })
    console.log(`${tag} ✅ DONE → ${relPath} (${buffer.length} bytes)`)
    return { downloaded: true, asset: { id: asset.id, fileName: asset.fileName, relPath, bytes: buffer.length } }
  }

  // 진단 dump — DevTools 에서 호출 가능. 현재 sync 상태 + 최근 tick 결과 + 디스크/state 정합성 검증.
  diagnose() {
    const watchDir = this.watchDir
    const syncedFiles = this.state.syncedFiles || {}
    const entries = Object.entries(syncedFiles).map(([relPath, e]) => {
      const absPath = path.join(watchDir, relPath)
      return {
        relPath,
        assetId: e?.assetId,
        projectId: e?.projectId,
        downloaded: !!e?.downloaded,
        existsOnDisk: fs.existsSync(absPath),
      }
    })
    const stale = entries.filter(e => !e.existsOnDisk)
    const downloads = entries.filter(e => e.downloaded)
    return {
      uid: this.uid,
      watchDir,
      downloadSince: this.state.downloadSince,
      downloadCursor: this.state.downloadCursor,
      totalSyncedEntries: entries.length,
      downloadedEntries: downloads.length,
      staleEntries: stale.length,
      stale: stale.slice(0, 20),  // 상위 20개만
      lastDiag: this._lastDiag || null,
    }
  }

  // ── Rename Detection ──

  bufferUnlink(filePath) {
    const relPath = this.getRelPath(filePath)
    if (this.isSelfMoved(relPath)) {
      console.log(`[bufferUnlink] ${relPath} — 우리가 방금 옮긴 자리다. 무시한다`)
      return
    }
    const entry = this.state.syncedFiles[relPath]
    if (!entry) return // not synced, nothing to do

    const projectKey = this.getProjectKey(filePath)
    const isVid = isVideo(filePath)
    const fileName = path.basename(filePath)
    const fileSize = entry.fileSize || 0  // state에 캐시된 사이즈 (없으면 0)

    const timer = setTimeout(() => {
      // No matching add arrived → proceed with normal delete
      this.pendingUnlinks.delete(relPath)
      this.handleDelete(filePath)
    }, 3500) // awaitWriteFinish(2000ms) + 여유분

    this.pendingUnlinks.set(relPath, {
      filePath, projectKey, isVideo: isVid, entry, timer,
      fileName, fileSize, contentHash: entry.contentHash || null,
    })
  }

  // Fast path: chokidar의 unlink+add 쌍을 rename으로 인식.
  // 우선순위: (1) 같은 폴더+같은 fileName+같은 미디어타입,
  //           (2) 같은 fileName+같은 미디어타입 (cross-folder, 이름 동일),
  //           (3) 같은 fileSize+같은 미디어타입 (cross-folder + 이름까지 변경, fileSize 알 때만)
  // 매칭 못 잡아도 syncFile의 contentHash dedup이 slow path로 잡아주니 손실 0.
  tryMatchRename(newFilePath) {
    const newProjectKey = this.getProjectKey(newFilePath)
    if (!newProjectKey) return false

    const newIsVideo = isVideo(newFilePath)
    const newFileName = path.basename(newFilePath)

    let newFileSize = 0
    try { newFileSize = fs.statSync(newFilePath).size } catch {}

    // 후보 수집 (미디어 타입 일치 필수)
    const candidates = []
    for (const [oldRelPath, pending] of this.pendingUnlinks) {
      if (pending.isVideo !== newIsVideo) continue
      const sameFolder = pending.projectKey === newProjectKey
      const sameName = pending.fileName === newFileName
      const sameSize = newFileSize > 0 && pending.fileSize > 0 && pending.fileSize === newFileSize

      let priority
      if (sameFolder && sameName) priority = 1
      else if (sameName) priority = 2
      else if (sameSize) priority = 3
      else continue
      candidates.push({ oldRelPath, pending, priority })
    }
    if (candidates.length === 0) return false

    candidates.sort((a, b) => a.priority - b.priority)
    const best = candidates[0]
    clearTimeout(best.pending.timer)
    this.pendingUnlinks.delete(best.oldRelPath)
    this.handleFileRename(best.oldRelPath, newFilePath, best.pending.entry)
    return true
  }

  async handleFileRename(oldRelPath, newFilePath, oldEntry) {
    const newRelPath = this.getRelPath(newFilePath)
    const newFileName = path.basename(newFilePath)
    const oldFileName = path.basename(oldRelPath)
    const newProjectName = this.getProjectName(newFilePath)
    const newProjectKey = this.getProjectKey(newFilePath)
    const newCategory = this.getCategoryName(newFilePath)
    const oldProjectId = oldEntry.projectId

    // Cross-folder인지 판단
    const oldProjectKey = oldRelPath.split('/').slice(0, -1).join('/')
    const isCrossFolder = oldProjectKey !== newProjectKey
    const isVid = isVideo(newFilePath) || /\.(mp4|mov|avi|mkv|webm|m4v|wmv|flv)$/i.test(oldEntry.storagePath || '')

    console.log(`[Rename] File: ${oldFileName} → ${newFileName}${isCrossFolder ? ` (cross-folder: ${oldProjectKey} → ${newProjectKey})` : ''}`)

    try {
      let newProjectId = oldProjectId
      if (isCrossFolder && newProjectName && newProjectKey) {
        // 사용자가 직접 옮긴 행위 = 의도적이므로 새 폴더 자동 승인
        this.approvedFolders.add(newProjectKey)
        newProjectId = await this.findOrCreateProject(newProjectName, newProjectKey, newCategory)
      }

      if (isCrossFolder && newProjectId !== oldProjectId) {
        // moveAsset: projectId 변경 + 양쪽 카운터 ±1
        await this.api.moveAsset(oldEntry.assetId, newProjectId, newFileName !== oldFileName ? newFileName : undefined)

        // 새 프로젝트 썸네일 비어 있으면 채움 (이미지면 url, 영상이면 videoThumbnailUrl)
        try {
          const newProj = await this.api.getProject(newProjectId)
          if (newProj && !newProj.thumbnailUrl) {
            // asset 정보 직접 fetch는 비용 — oldEntry에 url이 캐시 안 돼 있으므로 storage url은 이미 알고 있음
            // 영상은 thumbnailUrl 없으면 그냥 둠 (다음 reorder/setVideoThumbnail에서 처리)
            // 이미지면 createAsset 시점의 url 재구성 어려움 → skip하고 다음 동기화에 맡김
          }
        } catch {}
      } else {
        // 같은 폴더 내 rename — 기존 동작 유지
        if (newFileName !== oldFileName) {
          await this.api.updateAsset(oldEntry.assetId, {
            fileName: newFileName,
            updatedAt: new Date().toISOString(),
          })
        }
      }

      // Update local state: remove old relPath, add new one with same references (projectId 갱신)
      delete this.state.syncedFiles[oldRelPath]
      this.state.syncedFiles[newRelPath] = {
        ...oldEntry,
        projectId: newProjectId,
        syncedAt: new Date().toISOString(),
      }
      this.saveState()

      this.onFileStatus({ path: newRelPath, status: 'renamed', fileName: newFileName, oldFileName, isVideo: isVid })
    } catch (err) {
      console.error(`[Rename] Failed: ${oldRelPath} → ${newRelPath}`, err.message)
      // Fallback: 옛 자산은 휴지통으로 보내고(영구 삭제 아님 — 2026-09-22), 새 파일을 보통대로 올린다 (contentHash dedup이 또 매칭 시도)
      try {
        await this.api.trashAssets([oldEntry.assetId])
        delete this.state.syncedFiles[oldRelPath]
        this.saveState()
      } catch {}
      this.syncFile(newFilePath)
    }
  }

  bufferUnlinkDir(dirPath) {
    const folderKey = path.relative(this.watchDir, dirPath).split(path.sep).join('/')
    if (!folderKey) return

    if (this.isSelfMoved(folderKey)) {
      console.log(`[bufferUnlinkDir] ${folderKey} — 우리가 방금 옮긴 폴더다. 삭제 확인 안 띄운다`)
      this.onFolderRemoved()
      return
    }

    // Check if this folder has synced files (i.e., it's a known project folder)
    const hasSynced = Object.keys(this.state.syncedFiles).some(k => k.startsWith(folderKey + '/'))
    if (!hasSynced) {
      this.onFolderRemoved()
      return
    }

    // 안전망 1: watch dir 자체가 사라지면 (외장하드 분리 등) confirm 띄우지 말고 그냥 무시.
    //   사용자 의도가 아닌 시스템 이벤트로 처리.
    if (!fs.existsSync(this.watchDir)) {
      console.warn(`[bufferUnlinkDir] watch dir 사라짐 — 시스템 이벤트로 판단, ${folderKey} 삭제 confirm 스킵`)
      this.onFolderRemoved()
      return
    }

    // 안전망 2: 앱 시작 직후 (30초 이내) 발생한 unlinkDir 은 chokidar 초기화 이슈일 수 있음 → 스킵.
    if (this._startedAt && Date.now() - this._startedAt < 30 * 1000) {
      console.warn(`[bufferUnlinkDir] 시작 직후 ${folderKey} unlinkDir — 초기화 이벤트 가능성, confirm 스킵`)
      this.onFolderRemoved()
      return
    }

    // syncedFiles 스냅샷 캡처
    const snapshot = []
    let projectId = null
    let fileCount = 0
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      if (relPath.startsWith(folderKey + '/')) {
        snapshot.push({ relPath, ...entry })
        if (!projectId && entry?.projectId) projectId = entry.projectId
        fileCount++
      }
    }

    const parts = folderKey.split('/')
    const parentKey = parts.length > 1 ? parts.slice(0, -1).join('/') : ''

    const timer = setTimeout(() => {
      this.pendingUnlinkDirs.delete(folderKey)

      // 안전망 3: timeout 시점에 재확인 — watch dir 또 사라졌나? (외장하드 늦게 빠진 케이스)
      if (!fs.existsSync(this.watchDir)) {
        console.warn(`[bufferUnlinkDir/timeout] watch dir 사라짐 — ${folderKey} 삭제 confirm 스킵`)
        this.onFolderRemoved()
        return
      }

      // 안전망 4: 같은 시간대에 너무 많은 unlinkDir 폭주 (외장하드 빠짐 등 시스템 이벤트) → 스킵.
      //   3개 이상 동시 또는 직전 5초 내에 다른 unlinkDir 들어왔으면 mass event 로 판단.
      const recentDeletions = (this._recentUnlinks ||= [])
      const now = Date.now()
      this._recentUnlinks = recentDeletions.filter(t => now - t < 5000)
      this._recentUnlinks.push(now)
      if (this._recentUnlinks.length >= 3) {
        console.warn(`[bufferUnlinkDir/timeout] 5초 내 ${this._recentUnlinks.length}개 폴더 삭제 — mass event 로 판단, confirm 스킵`)
        this.onFolderRemoved()
        return
      }

      // 통과 — 진짜 사용자가 폴더 하나 지운 것. confirm 띄움.
      this.onFolderDeletionRequested?.({
        folderKey,
        folderName: folderKey.split('/').pop(),
        projectId,
        fileCount,
      })
      this.onFolderRemoved()
    }, 4000)

    this.pendingUnlinkDirs.set(folderKey, { dirPath, parentKey, folderKey, timer, snapshot, projectId })
  }

  tryMatchFolderRename(newDirPath) {
    const newFolderKey = path.relative(this.watchDir, newDirPath).split(path.sep).join('/')
    if (!newFolderKey) return

    const newParts = newFolderKey.split('/')
    const newParentKey = newParts.length > 1 ? newParts.slice(0, -1).join('/') : ''

    // Find a pending unlinkDir at the same depth/parent
    for (const [oldFolderKey, pending] of this.pendingUnlinkDirs) {
      if (pending.parentKey !== newParentKey) continue
      // Same parent directory → likely a rename
      clearTimeout(pending.timer)
      this.pendingUnlinkDirs.delete(oldFolderKey)
      this.handleFolderRename(oldFolderKey, newFolderKey)
      return
    }
  }

  async handleFolderRename(oldFolderKey, newFolderKey) {
    const oldName = oldFolderKey.split('/').pop()
    const newName = newFolderKey.split('/').pop()

    console.log(`[Rename] Folder: ${oldName} → ${newName}`)

    // Find projectId from synced files
    let projectId = this.projectCache.get(oldFolderKey)
    if (!projectId) {
      for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
        if (relPath.startsWith(oldFolderKey + '/')) {
          projectId = entry.projectId
          break
        }
      }
    }

    if (!projectId) {
      console.error(`[Rename] No projectId found for folder: ${oldFolderKey}`)
      this.onFolderRemoved()
      return
    }

    try {
      // Update project name in Firestore
      const updateData = { name: newName }

      // If category (top-level folder) changed, update that too
      const newParts = newFolderKey.split('/')
      if (newParts.length >= 2) {
        const rawCat = newParts[0]
        const norm = normalizeCategory(rawCat)
        updateData.category = DEFAULT_CATEGORIES.includes(norm) ? norm : rawCat
      }

      await this.api.updateProject(projectId, updateData)

      // Update projectCache
      this.projectCache.delete(oldFolderKey)
      this.projectCache.set(newFolderKey, projectId)

      // Update approvedFolders
      this.approvedFolders.delete(oldFolderKey)
      this.approvedFolders.add(newFolderKey)

      // Update all syncedFiles entries with old folder prefix
      const updates = {}
      for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
        if (relPath.startsWith(oldFolderKey + '/')) {
          const newRelPath = newFolderKey + relPath.slice(oldFolderKey.length)
          delete this.state.syncedFiles[relPath]
          updates[newRelPath] = { ...entry }
        }
      }
      Object.assign(this.state.syncedFiles, updates)
      this.saveState()

      console.log(`[Rename] Folder rename complete: ${oldName} → ${newName} (projectId: ${projectId})`)
      this.onFolderRemoved() // refresh UI
    } catch (err) {
      console.error(`[Rename] Folder rename failed: ${oldFolderKey} → ${newFolderKey}`, err.message)
      this.onFolderRemoved()
    }
  }

  async handleDelete(filePath) {
    const relPath = this.getRelPath(filePath)
    const entry = this.state.syncedFiles[relPath]
    if (!entry) return

    /* ⚠️ 안전망 (2026-09-22 검수) — 감시 폴더가 통째로 사라졌거나(외장하드 빠짐) 5초 안에 파일이 무더기로 사라지면
          사람이 지운 게 아니라 시스템 이벤트다. 웹을 건드리지 않는다 (폴더 삭제 쪽 안전망과 같은 규칙). */
    if (!fs.existsSync(this.watchDir)) { console.warn(`[handleDelete] watch dir 사라짐 — ${relPath} 웹 반영 안 함`); return }
    const now = Date.now()
    this._recentFileUnlinks = (this._recentFileUnlinks || []).filter(t => now - t < 5000)
    this._recentFileUnlinks.push(now)
    if (this._recentFileUnlinks.length >= 20) { console.warn(`[handleDelete] 5초 내 ${this._recentFileUnlinks.length}개 삭제 — 시스템 이벤트로 판단, ${relPath} 웹 반영 안 함`); return }

    try {
      /* ⚠️ 휴지통으로 보낸다 — 영구 삭제가 아니다 (2026-09-22 검수).
            예전엔 Finder 에서 사진 한 장 지우면 3.5초 뒤 저장소 파일과 문서를 **영구 삭제**했다. 웹의 "30일 휴지통" 약속과 어긋났고,
            폴더째 지우면 "웹에서도 지울까요?" 창이 뜨기 전에 안의 사진이 먼저 사라졌다. 저장소 파일은 지우지 않는다 — 휴지통 정리(cron)가 30일 뒤 지운다. */
      await this.api.trashAssets([entry.assetId])

      delete this.state.syncedFiles[relPath]
      this.saveState()
      this.onFileStatus({ path: relPath, status: 'deleted', fileName: path.basename(filePath) })
    } catch (err) {
      this.onError({ message: `Delete failed: ${relPath}`, error: err.message })
    }
  }

  // ── 무압축 공유 업로드 ──

  async checkPendingShares() {
    /* ⚠️ 10초마다 불리는데 큰 원본은 10초 넘게 걸린다 — 도는 중이면 새로 시작하지 않는다 (2026-09-11).
          예전엔 같은 업로드가 겹쳐 시작돼 메모리가 몇 배로 늘고, 파일 목록을 서로 덮어썼다. */
    if (this._shareRunning) return { busy: true }
    this._shareRunning = true
    try {
      const result = await this.api.getPendingShares()
      const shares = result?.shares || []
      if (shares.length > 0) {
        console.log(`[Share] Found ${shares.length} pending share(s)`)
        for (const share of shares) {
          await this.processShareUpload(share)
        }
      }
      return { found: shares.length }
    } catch (err) {
      console.error('[Share] Polling error:', err.message, err.stack)
      this.onProgress({ phase: 'share_error', message: err.message })
      return { error: err.message }
    } finally {
      this._shareRunning = false
    }
  }

  async processShareUpload(share) {
    const assets = share.assets || []
    const pending = assets.filter(a => a.uploadStatus === 'pending')
    /* ⚠️ 남은 파일이 없는데 아직 pending_upload 면 — 앞선 시도에서 마지막 "끝" 알림만 실패한 것이다 (2026-09-11 검토).
          그대로 두면 매번 여기서 돌아가서 공유창과 받는 페이지가 7일 동안 돈다. 파일 상태를 보고 끝을 다시 적는다. */
    if (pending.length === 0) {
      const 올림 = assets.filter(a => a.uploadStatus === 'uploaded').length
      if (assets.length > 0) {
        const 끝 = 올림 === assets.length ? 'ready' : (올림 > 0 ? 'partial' : 'failed')
        try { await this.api.updateShareProgress({ shareId: share.id, uploadedCount: 올림, status: 끝 }) } catch {}
      }
      return
    }
    /* ⚠️ 이 컴퓨터에 이 프로젝트 파일이 하나도 없으면 손대지 않는다 — 같은 계정의 다른 컴퓨터가 올릴 몫이다 (2026-09-11 검토).
          예전 같으면 파일이 없는 컴퓨터가 먼저 받아서 전부 "못 올림" 으로 끝내 버렸다. */
    const 이컴퓨터에있나 = Object.values(this.state.syncedFiles || {}).some(e => e.projectId === share.projectId)
    if (!이컴퓨터에있나) return
    /* ⚠️ 다시 해 보기는 간격을 둔다 (2026-09-11 검토) — 10초 간격 3번이면 30초짜리 네트워크 끊김에도 공유가 끝나 버렸다.
          1번째 실패 뒤 1분, 2번째 실패 뒤 5분 쉬고 다시 한다. 앱을 껐다 켜면 바로 다시 한다. */
    if (Date.now() < (this._shareRetryAt?.get(share.id) || 0)) return
    /* 몇 번째 시도인가 — 네트워크 같은 일시적인 실패는 3번까지만 다시 한다 (2026-09-11) */
    const 이번시도 = (share.attempts || 0) + 1
    const 일시실패 = []
    /* 웹 공유창에 "앱이 받아서 올리기 시작했다" 를 알린다 — 이게 없으면 앱이 꺼져 있는지 알 길이 없다 */
    try { await this.api.updateShareProgress({ shareId: share.id, pickedUp: true }) } catch {}

    console.log(`[Share] Processing share ${share.id}: ${pending.length} pending / ${assets.length} total, projectId=${share.projectId}`)

    let uploadedCount = assets.filter(a => a.uploadStatus === 'uploaded').length
    const total = assets.length
    const errors = []

    this.onProgress({ phase: 'share_uploading', total, completed: uploadedCount, shareId: share.id, projectName: share.projectName })

    for (const asset of pending) {
      // 파일 검색 단계
      this.onProgress({ phase: 'share_uploading', total, completed: uploadedCount, shareId: share.id, projectName: `${share.projectName} · ${asset.fileName} 검색 중` })

      const localPath = this.findLocalPath(asset.id, asset.fileName, share.projectId)
      if (!localPath) {
        const syncedCount = Object.keys(this.state.syncedFiles).length
        const err = `파일 없음: ${asset.fileName} (synced:${syncedCount})`
        console.error(`[Share] ${err}, assetId=${asset.id}, projectId=${share.projectId}`)
        errors.push(err)
        this.onProgress({ phase: 'share_uploading', total, completed: uploadedCount, shareId: share.id, projectName: `${share.projectName} · ❌ ${asset.fileName}` })
        /* 원본이 없으면 다시 해도 안 된다 — 바로 "못 올림" 으로 적는다 */
        try {
          await this.api.updateShareProgress({ shareId: share.id, assetId: asset.id, uploadStatus: 'failed', failReason: '보내는 분 컴퓨터의 싱크 폴더에서 원본을 찾지 못했어요' })
        } catch {}
        continue
      }

      console.log(`[Share] Found ${asset.fileName} at ${localPath}`)

      try {
        const fileStat = fs.statSync(localPath)
        const fileSize = fileStat.size

        this.onProgress({ phase: 'share_uploading', total, completed: uploadedCount, shareId: share.id, projectName: `${share.projectName} · ${asset.fileName} 업로드 중 (${fmt(fileSize)})` })

        this.onFileStatus({
          path: `share/${share.id}/${asset.fileName}`,
          status: 'uploading',
          progress: 0,
          fileName: asset.fileName,
          isVideo: asset.isVideo,
          size: fmt(fileSize),
          phase: '무압축 업로드 중...',
        })

        // Firebase Storage로 업로드 (R2 presigned URL 대신 — 서명 호환 문제 우회)
        /* ⚠️ 파일을 통째로 메모리에 읽지 않는다 (2026-09-11) — Node 는 2GB 넘는 파일을 한 번에 못 읽어서
              큰 원본 영상 공유가 실패했다. 아래에서 조금씩 흘려 보낸다. */
        const ext = path.extname(asset.fileName).toLowerCase().replace('.', '')
        const contentType = CONTENT_TYPE_MAP[ext] || 'application/octet-stream'
        console.log(`[Share] Storage upload ${asset.fileName} size=${fileSize} type=${contentType}`)

        // 1) Vercel API로 Storage signed upload URL 받기
        const shareStoragePath = asset.shareStoragePath
        if (!shareStoragePath) throw new Error('shareStoragePath 없음')

        const urlRes = await this.api.getStorageUploadUrl(shareStoragePath, contentType)
        if (!urlRes?.uploadUrl) throw new Error('Storage upload URL 발급 실패')

        // 2) Storage signed URL로 PUT 업로드 — 파일을 흘려 보낸다
        const uploadResult = await new Promise((resolve, reject) => {
          const url = new URL(urlRes.uploadUrl)
          const https = require('https')
          const req = https.request({
            hostname: url.hostname,
            path: url.pathname + url.search,
            method: 'PUT',
            headers: {
              'Content-Type': contentType,
              'Content-Length': fileSize,
              'x-goog-meta-firebasestoragedownloadtokens': urlRes.token,
            },
          }, (res) => {
            let body = ''
            res.on('data', chunk => body += chunk)
            res.on('end', () => resolve({ status: res.statusCode, body }))
            res.on('error', reject)
          })
          req.on('error', reject)
          /* ⚠️ 2분 동안 한 바이트도 안 오가면 끊는다 (2026-09-11 검토) — 잠자기·와이파이 변경으로 멈춘 업로드가
                "도는 중" 표시를 붙잡아 다른 공유까지 영원히 막았다. 끊으면 일시 실패로 쳐서 나중에 다시 한다. */
          req.setTimeout(120_000, () => req.destroy(new Error('업로드가 2분 동안 멈췄어요')))
          const 흐름 = fs.createReadStream(localPath)
          흐름.on('error', (e) => { req.destroy(e); reject(e) })
          흐름.pipe(req)
        })

        if (uploadResult.status >= 300) {
          throw new Error(`Storage ${uploadResult.status} ${uploadResult.body.slice(0, 100)}`)
        }

        uploadedCount++

        // Firestore 진행률 업데이트 (실제 원본 파일 크기로 보정)
        await this.api.updateShareProgress({
          shareId: share.id,
          assetId: asset.id,
          uploadStatus: 'uploaded',
          actualFileSize: fileSize, // 로컬 원본 크기 (압축 전)
          uploadedCount,
          status: uploadedCount === total ? 'ready' : undefined,
        })

        this.onFileStatus({
          path: `share/${share.id}/${asset.fileName}`,
          status: 'done',
          progress: 100,
          fileName: asset.fileName,
          isVideo: asset.isVideo,
          size: fmt(fileSize),
        })

        this.onProgress({ phase: 'share_uploading', total, completed: uploadedCount, shareId: share.id, projectName: share.projectName })
      } catch (err) {
        console.error(`[Share] Upload failed for ${asset.fileName}:`, err.message)
        errors.push(`${asset.fileName}: ${err.message}`)
        일시실패.push(asset)
        this.onFileStatus({
          path: `share/${share.id}/${asset.fileName}`,
          status: 'failed',
          fileName: asset.fileName,
          error: err.message,
        })
        this.onProgress({ phase: 'share_uploading', total, completed: uploadedCount, shareId: share.id, projectName: `${share.projectName} · ❌ ${err.message.slice(0, 40)}` })
      }
    }

    if (uploadedCount === total) {
      this.onProgress({ phase: 'share_complete', total, completed: total, shareId: share.id, projectName: share.projectName })
      return
    }

    // 실패 정보를 UI에 표시
    const errMsg = errors.length > 0 ? errors[0] : '알 수 없는 오류'
    this.onProgress({ phase: 'share_error', message: errMsg })
    console.error(`[Share] Failed: ${uploadedCount}/${total} uploaded (시도 ${이번시도}). Errors:`, errors)

    /* ⚠️ 끝없이 다시 하지 않는다 (2026-09-11).
          예전엔 하나라도 실패하면 10초마다 영원히 다시 했고, 웹 공유창은 끝나지 않고 돌기만 했다.
          네트워크 같은 일시적인 실패는 3번까지 다시 해 보고, 그래도 안 되면 끝을 적는다 —
          일부라도 올라갔으면 partial(올라간 것만 받을 수 있다), 하나도 못 올렸으면 failed(건수를 돌려준다). */
    if (일시실패.length > 0 && 이번시도 < 3) {
      try { await this.api.updateShareProgress({ shareId: share.id, attempts: 이번시도 }) } catch {}
      if (!this._shareRetryAt) this._shareRetryAt = new Map()
      this._shareRetryAt.set(share.id, Date.now() + (이번시도 === 1 ? 60_000 : 300_000))
      return
    }
    for (const a of 일시실패) {
      try {
        await this.api.updateShareProgress({ shareId: share.id, assetId: a.id, uploadStatus: 'failed', failReason: '업로드가 3번 모두 실패했어요' })
      } catch {}
    }
    try {
      await this.api.updateShareProgress({ shareId: share.id, attempts: 이번시도, status: uploadedCount > 0 ? 'partial' : 'failed' })
    } catch {}
  }

  // assetId로 로컬 파일 경로 역추적
  findLocalPath(assetId, fileName, projectId) {
    // 1차: assetId로 정확히 매칭
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      if (entry.assetId === assetId) {
        const full = path.join(this.watchDir, ...relPath.split('/'))
        if (fs.existsSync(full)) return full
      }
    }
    // 2차: projectId + fileName으로 매칭 (assetId가 안 맞을 때 fallback)
    if (fileName && projectId) {
      for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
        if (entry.projectId === projectId && relPath.endsWith('/' + fileName)) {
          const full = path.join(this.watchDir, ...relPath.split('/'))
          if (fs.existsSync(full)) {
            console.log(`[Share] Fallback match by fileName: ${fileName}`)
            return full
          }
        }
      }
    }
    /* ⚠️ 3차 "싱크 폴더 전체에서 파일 이름으로 찾기" 를 뺐다 (2026-09-11).
          IMG_0001.JPG · C0001.MP4 같은 이름은 촬영마다 겹친다 — 다른 고객의 원본을 집어 보낼 수 있었다.
          원본은 **이 프로젝트에 속한 파일**에서만 찾는다. 못 찾으면 "원본을 찾지 못했어요" 로 알린다. */
    return null
  }

  // ── Content-hash 백필 워커 ──
  // 앱 실행 시 1회 가동: state.syncedFiles 중 contentHash 없는 entry에 대해
  // 로컬 파일 sha256 계산 → state + Firestore 양쪽에 채워넣음.
  // 한 파일 처리 후 sleep 두어 CPU 점유 최소화.
  // 1000개 영상(평균 1GB) ≈ 30~40분 백그라운드 (한 번만 실행).
  startContentHashBackfill() {
    if (this._backfillRunning) return
    this._backfillRunning = true
    this._backfillStop = false
    ;(async () => {
      const sleep = (ms) => new Promise((r) => setTimeout(r, ms))
      let processed = 0
      let updated = 0
      let failed = 0
      const startedAt = Date.now()

      try {
        const entries = Object.entries(this.state.syncedFiles)
        const targets = entries.filter(([, e]) => !e.contentHash)
        if (targets.length === 0) {
          console.log('[Backfill] No assets need contentHash backfill')
          return
        }
        console.log(`[Backfill] Starting contentHash backfill for ${targets.length} assets`)

        for (const [relPath, entry] of targets) {
          if (this._backfillStop) break
          processed++
          const fullPath = path.join(this.watchDir, ...relPath.split('/'))
          if (!fs.existsSync(fullPath)) {
            // 로컬 파일 없음 — skip (다음 sync에서 정리)
            await sleep(50)
            continue
          }
          try {
            const stat = fs.statSync(fullPath)
            const hash = await this.computeFileHash(fullPath)
            // Firestore 업데이트 (assetId가 유효한 경우만)
            if (entry.assetId) {
              try {
                await this.api.updateAsset(entry.assetId, {
                  contentHash: hash,
                  // fileSize도 함께 채워두면 fast-path rename 매칭에 도움
                })
              } catch (apiErr) {
                console.warn(`[Backfill] API update failed for ${relPath}: ${apiErr.message}`)
                failed++
                await sleep(500)
                continue
              }
            }
            // state 갱신
            this.state.syncedFiles[relPath] = {
              ...entry,
              contentHash: hash,
              fileSize: entry.fileSize || stat.size,
            }
            updated++
            // 5개마다 state 저장 (디스크 IO 절약)
            if (updated % 5 === 0) this.saveState()
          } catch (err) {
            console.warn(`[Backfill] Hash failed for ${relPath}: ${err.message}`)
            failed++
          }
          // CPU 양보 — 한 파일 처리 후 100ms 휴식 (큰 영상 후엔 자동으로 더 길어짐)
          await sleep(100)
        }
        this.saveState()
        const elapsed = ((Date.now() - startedAt) / 1000).toFixed(1)
        console.log(`[Backfill] Done: ${updated} updated / ${failed} failed / ${processed} processed in ${elapsed}s`)
      } catch (err) {
        console.error('[Backfill] Worker crashed:', err.message)
      } finally {
        this._backfillRunning = false
      }
    })()
  }

  stopContentHashBackfill() {
    this._backfillStop = true
  }

  startSharePolling() {
    this._shareInterval = setInterval(() => this.checkPendingShares(), 10000) // 10초마다
  }

  stopSharePolling() {
    if (this._shareInterval) {
      clearInterval(this._shareInterval)
      this._shareInterval = null
    }
  }

  startRemoteSyncPolling() {
    this.stopRemoteSyncPolling()
    this._remoteSyncTimer = setInterval(async () => {
      try {
        await this.syncStateWithServer()
      } catch {}
    }, 30000) // 30초
  }

  stopRemoteSyncPolling() {
    if (this._remoteSyncTimer) {
      clearInterval(this._remoteSyncTimer)
      this._remoteSyncTimer = null
    }
  }

  stopDownloadPolling() {
    if (this._downloadTimer) {
      clearInterval(this._downloadTimer)
      this._downloadTimer = null
    }
    if (this._trashCleanupTimer) {
      clearInterval(this._trashCleanupTimer)
      this._trashCleanupTimer = null
    }
  }

  stop() {
    this.stopSharePolling()
    this.stopRemoteSyncPolling()
    this.stopContentHashBackfill()
    this.stopDownloadPolling()
    // Clear pending rename buffers
    for (const p of this.pendingUnlinks.values()) clearTimeout(p.timer)
    this.pendingUnlinks.clear()
    for (const p of this.pendingUnlinkDirs.values()) clearTimeout(p.timer)
    this.pendingUnlinkDirs.clear()
    this.watcher?.close()
    // Phase 0 방어선 정리
    this.recentlyDownloaded?.dispose()
    this.saveState()
  }

  // ── 탐색기: 이름 변경 ──

  async renameProject(projectKey, newName) {
    const projectId = this.projectCache.get(projectKey)
    if (!projectId) {
      // cache에 없으면 syncedFiles에서 찾기
      for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
        if (relPath.startsWith(projectKey + '/')) {
          this.projectCache.set(projectKey, entry.projectId)
          break
        }
      }
    }
    const pid = this.projectCache.get(projectKey)
    if (!pid) throw new Error('프로젝트를 찾을 수 없습니다')

    const oldPath = path.join(this.watchDir, ...projectKey.split('/'))
    const parts = projectKey.split('/')
    parts[parts.length - 1] = newName
    const newKey = parts.join('/')
    const newPath = path.join(this.watchDir, ...parts)

    if (fs.existsSync(newPath)) throw new Error('같은 이름의 폴더가 이미 있습니다')

    // 1. state를 먼저 업데이트 (chokidar 이벤트가 와도 무시되게)
    const updates = {}
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      if (relPath.startsWith(projectKey + '/')) {
        const newRelPath = newKey + relPath.slice(projectKey.length)
        delete this.state.syncedFiles[relPath]
        updates[newRelPath] = { ...entry }
      }
    }
    Object.assign(this.state.syncedFiles, updates)

    this.projectCache.delete(projectKey)
    this.projectCache.set(newKey, pid)
    this.approvedFolders.delete(projectKey)
    this.approvedFolders.add(newKey)
    this.saveState()

    // 2. 로컬 폴더 이름 변경
    fs.renameSync(oldPath, newPath)

    // 3. Firestore 프로젝트 이름 업데이트
    await this.api.updateProject(pid, { name: newName })

    return { ok: true, newKey }
  }

  async renameFile(relPath, newFileName) {
    const entry = this.state.syncedFiles[relPath]
    if (!entry) throw new Error('파일을 찾을 수 없습니다')

    const oldPath = path.join(this.watchDir, ...relPath.split('/'))
    const dir = path.dirname(oldPath)
    const newPath = path.join(dir, newFileName)

    if (fs.existsSync(newPath)) throw new Error('같은 이름의 파일이 이미 있습니다')

    // 1. state 먼저 업데이트
    const parts = relPath.split('/')
    parts[parts.length - 1] = newFileName
    const newRelPath = parts.join('/')

    delete this.state.syncedFiles[relPath]
    this.state.syncedFiles[newRelPath] = { ...entry }
    this.saveState()

    // 2. 로컬 파일 이름 변경
    fs.renameSync(oldPath, newPath)

    // 3. Firestore 에셋 이름 업데이트
    await this.api.updateAsset(entry.assetId, { fileName: newFileName })

    return { ok: true, newRelPath }
  }

  // ── 탐색기: 파일 목록 + 순서 변경 ──

  async getProjectFiles(projectKey) {
    const files = []
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      if (relPath.startsWith(projectKey + '/')) {
        files.push({
          relPath,
          fileName: relPath.split('/').pop(),
          assetId: entry.assetId,
          projectId: entry.projectId,
          isVideo: isVideo(relPath),
          url: null,
          order: 999,
          isThumbnail: false,
        })
      }
    }
    if (files.length === 0) return files

    // Firestore에서 order, url, 썸네일 정보 가져오기
    try {
      const { assets } = await this.api.getAssetsByProject(files[0].projectId)
      const assetMap = new Map()
      for (const a of assets) assetMap.set(a.id, a)

      const project = await this.api.getProject(files[0].projectId)
      const thumbnailUrl = project?.thumbnailUrl || null

      for (const f of files) {
        const a = assetMap.get(f.assetId)
        if (a) {
          f.order = a.order ?? 999
          f.url = a.url || null
          f.videoThumbnailUrl = a.videoThumbnailUrl || null
          // 이 파일이 프로젝트 썸네일인지 확인
          if (thumbnailUrl && f.url === thumbnailUrl) f.isThumbnail = true
        }
      }
      files.sort((a, b) => a.order - b.order || a.fileName.localeCompare(b.fileName))
    } catch {
      files.sort((a, b) => a.fileName.localeCompare(b.fileName))
    }

    return files
  }

  async reorderFiles(orderedAssetIds) {
    for (let i = 0; i < orderedAssetIds.length; i++) {
      await this.api.updateAsset(orderedAssetIds[i], { order: i })
    }
    return { ok: true }
  }
}

module.exports = { SyncEngine }
