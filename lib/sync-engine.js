...
class SyncEngine {
  ...
  startDownloadPolling() {
    ...
    // 첫 부팅 시 bootstrap — 이미 이 기기가 올린 과거 자산을 다시 받지 않도록
    // "지금 시각" 부터의 변경만 본다. (2기기 사용 시나리오에서는 별도 기기에서 받은 상태를
    //  Firestore snapshot 이 어차피 보존하므로 누락 위험 낮음.)
    if (!this.state.downloadSince) {
