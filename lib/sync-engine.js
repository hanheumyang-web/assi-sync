...
class SyncEngine {
  ...
  async downloadWorkspaceAsset(asset, ws) {
    ...
    // 이미 받은 자산
    const alreadyKnownRelPath = Object.keys(bucket.syncedFiles).find(
      k => bucket.syncedFiles[k]?.assetId === asset.id
    )
  ...
  async downloadRemoteAsset(asset) {
    ...
    // ── Guard 1: 이 기기가 올린/이미 받은 자산이면 스킵 (assetId 로 state 조회) ──
    // 장비 A가 업로드한 직후 다음 폴링 때 자기 자산을 다시 받지 않도록.
    const alreadyKnownRelPath = Object.keys(this.state.syncedFiles).find(
      k => this.state.syncedFiles[k]?.assetId === asset.id
    )
    if (alreadyKnownRelPath) return
