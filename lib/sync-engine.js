...
class SyncEngine {
  ...
  async syncFile(filePath, _fromBatch = false) {
    ...
    try {
      ...
      if (isVid) {
        ...
        await this.uploadToBunny(filePath, fileName, projectId, assetId, relPath, compressedSize)
  ...
  async getProjectFiles(projectKey) {
    ...
    // Firestore에서 order 정보 가져오기
    try {
      const { assets } = await this.api.getAssetsByProject(files[0].projectId)
      const orderMap = new Map()
      for (const a of assets) orderMap.set(a.id, a.order ?? 999)
      for (const f of files) f.order = orderMap.get(f.assetId) ?? 999
