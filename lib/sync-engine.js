...
// Phase 2 W2 — 선택적 의존 (workspaceManager/viewerNotifier 안 넘기면 Phase 1 동작과 동일)
const { determineDownloadSince } = require('./workspace-bootstrap')
const { isolateLocalChange } = require('./viewer-guard')
const { PERSONAL_CONTEXT } = require('./workspace-manager')
const { ensureMarker: ensureRootMarker } = require('./root-marker')
...
class SyncEngine {
  constructor({ uid, watchDir, statePath, api, onProgress, onFileStatus, onError, onNewFolder, onFolderRemoved, deviceDeps, workspaceManager, viewerNotifier }) {
    this.uid = uid
    ...
    // Phase 2 W2 — 선택적. 없으면 Phase 1 동작 (개인 폴더 owner 만). 있으면 팀 컨텍스트/viewer 가드
    this.workspaceManager = workspaceManager || null
    this.viewerNotifier = viewerNotifier || null
    ...
    // WorkspaceManager 가 sync-engine state 공유하도록 attach.
    // main.js 에서 workspaceManager 를 먼저 생성해 넘기지만 state 접근은 loadState() 후.
    if (this.workspaceManager?.attachState) {
      this.workspaceManager.attachState(this.state, () => this.saveState())
    }

    // createAsset 에 workspaceId 자동 주입 — 팀 폴더 업로드 시 workspace-owned 로 저장
    if (this.api?.setAssetContextInjector) {
      this.api.setAssetContextInjector((data) => {
        if (this._currentWsId && !data.workspaceId) {
          return { ...data, workspaceId: this._currentWsId }
        }
        return data
      })
    }
  ...
  async findOrCreateProject(name, projectKey, category) {
    const wsId = this._currentWsId || null
    const cacheKey = wsId ? `${wsId}:${projectKey}` : projectKey
    if (this.projectCache.has(cacheKey)) return this.projectCache.get(cacheKey)

    const result = await this.api.findOrCreateProject(name, category, wsId)
    if (!result?.projectId) {
    ...
    const wsId2 = this._currentWsId || null
    const cacheKey2 = wsId2 ? `${wsId2}:${projectKey}` : projectKey
    this.projectCache.set(cacheKey2, result.projectId)
  ...
  async syncFile(filePath, _fromBatch = false) {
    ...
    // ── Phase 2.5 — 업로드 컨텍스트 판별 ──
    // 팀 폴더 내부 파일이면 workspaceId 세팅 → 이후 findOrCreateProject/createAsset 가 팀 소유로 생성.
    // 모든 팀 멤버 = editor (viewer 개념 제거). 업로드 무제한 허용.
    this._currentWsId = null
    if (this.workspaceManager) {
      try {
        const ctx = this.workspaceManager.resolveContext(filePath)
        if (ctx && ctx !== PERSONAL_CONTEXT) {
          this._currentWsId = ctx
        }
      } catch (e) {
        console.warn('[workspace-ctx] resolve failed:', e.message)
      }
    }
  ...
  async start() {
    ...
    // 개인 루트 폴더 마커 확보 (이동 탐지용). state.personalRootId 에 저장.
    try {
      const marker = ensureRootMarker(this.watchDir, { type: 'personal' })
      if (!this.state.personalRootId || this.state.personalRootId !== marker.rootId) {
        this.state.personalRootId = marker.rootId
        this.saveState()
      }
    } catch (e) {
      console.warn('[RootMarker] personal marker failed (non-fatal):', e.message)
    }

    // Phase 1 다운로드 폴링 — 스캔 완료를 기다리지 않고 즉시 시작.
    // 업로드 스캔과 다운로드는 독립적이라 병행해도 안전.
    // 다운로드받은 파일은 xattr/recentlyDownloaded 로 마킹되어 chokidar 가 잡아도 업로드 루프 안 됨.
    ...
    await new Promise((resolve) => {
      ...
      this.watcher.on('ready', async () => {
        ...
        // Phase 1 다운로드 폴링은 start() 진입 즉시 이미 시작됨 (스캔 대기 안 함)
  ...
  startDownloadPolling() {
    ...
    // Bootstrap — Phase 2 W2: workspaceManager 있으면 신규 디바이스 여부로 분기.
    //   state 있음 → 이어쓰기 (기존 동작)
    //   state 비어있음 + deviceId 기등록 → NOW (재설치 방어)
    //   state 비어있음 + deviceId 신규 → 0 (통째 다운로드 — Mac 2번째 디바이스 gap 해결)
    // workspaceManager 없으면 Phase 1 동작 (기존대로 NOW).
    const doBootstrap = async () => {
      if (this.state.downloadSince) return
      if (this.workspaceManager) {
        try {
          const bucket = this.workspaceManager.getStateBucket(this.state, PERSONAL_CONTEXT)
          const deviceId = this.deviceRegistry?.deviceId || 'unknown-device'
          const { since, reason } = await determineDownloadSince({
            stateBucket: bucket, deviceId, workspaceId: null, api: this.api,
          })
          this.state.downloadSince = since === 0
            ? new Date(0).toISOString()
            : new Date(since).toISOString()
          console.log(`[DownloadPoll] bootstrap since=${this.state.downloadSince} (${reason})`)
        } catch (e) {
          console.warn('[DownloadPoll] bootstrap failed, fallback NOW:', e.message)
          this.state.downloadSince = new Date().toISOString()
        }
      } else {
        this.state.downloadSince = new Date().toISOString()
      }
    ...
    // 비동기 bootstrap — 첫 poll 전에 await 로 실행
    this._bootstrapPromise = doBootstrap()

    const pollOnce = async () => {
      const { assets, nextSince, hasMore } = await this.api.listAssetsSince(this.state.downloadSince, 200)
    ...
    const tick = async () => {
      try {
        // W2: 첫 poll 전에 bootstrap 완료 대기 (since 결정)
        if (this._bootstrapPromise) await this._bootstrapPromise
        // hasMore 일 때 즉시 연속 호출 (배치 500 초과분 drain). 과도 반복 방지 위해 10회 상한.
        let guard = 10
        while (guard-- > 0) {
    ...
    // ── Phase 2 W2 — 워크스페이스 자산 폴링 + 주기적 멤버십 refresh ──
    if (this.workspaceManager) {
      this._startWorkspacePolling()
    }
  }

  // 워크스페이스 polling — 각 팀마다 getWorkspaceAssets 주기 호출 + 팀 폴더/bootstrap 자동 처리.
  // 주기적으로 workspaceManager.refresh() 호출해서 새 팀 가입도 감지 (앱 재시작 없이도 동작).
  _startWorkspacePolling() {
    const WS_INTERVAL = 30 * 1000        // 30초 — 자산 폴링
    const REFRESH_INTERVAL = 5 * 60 * 1000 // 5분 — 팀 멤버십 새로고침

    const pollOneWorkspace = async (ws) => {
      const bucket = this.workspaceManager.getStateBucket(this.state, ws.id)

      // Bootstrap since — 새 팀 합류 시 0 으로 세팅 → 통째 다운로드
      if (!bucket.downloadSince) {
        try {
          const deviceId = this.deviceRegistry?.deviceId || 'unknown-device'
          const { since, reason } = await determineDownloadSince({
            stateBucket: bucket, deviceId, workspaceId: ws.id, api: this.api,
          })
          bucket.downloadSince = since === 0
            ? new Date(0).toISOString()
            : new Date(since).toISOString()
          console.log(`[ws-poll ${ws.id}] bootstrap since=${bucket.downloadSince} (${reason})`)
        } catch (e) {
          bucket.downloadSince = new Date().toISOString()
          console.warn(`[ws-poll ${ws.id}] bootstrap fallback:`, e.message)
        }
        this.saveState()
      }

      // ── 1. 재시도 큐 먼저 처리 (silent-skip 된 자산 복구 시도) ──
      // retryCount >= MAX_RETRY 면 permanentSkipped 로 이관.
      const MAX_RETRY = 10
      const MAX_RETRY_PER_TICK = 20
      if (!Array.isArray(bucket.retryQueue)) bucket.retryQueue = []
      if (!Array.isArray(bucket.permanentSkipped)) bucket.permanentSkipped = []
      if (bucket.retryQueue.length > 0) {
        const batch = bucket.retryQueue.splice(0, MAX_RETRY_PER_TICK)
        for (const entry of batch) {
          try {
            await this.downloadWorkspaceAsset(entry.asset, ws)
            // 다운로드 성공 또는 skip (assetId 가 bucket.syncedFiles 에 들어가면 성공) — 큐에서 제거됨
          } catch (e) {
            entry.retryCount = (entry.retryCount || 0) + 1
            entry.lastAttempt = new Date().toISOString()
            entry.lastError = e.message
            if (entry.retryCount >= MAX_RETRY) {
              bucket.permanentSkipped.push(entry)
              console.warn(`[ws-poll ${ws.id}] 영구 포기: ${entry.asset.fileName} (${entry.retryCount}회 실패)`)
            } else {
              bucket.retryQueue.push(entry)  // 다시 큐에 넣음
            }
          }
        }
        this.saveState()
      ...
      // ── 2. 새 배치 쿼리 (updatedAt > downloadSince) ──
      try {
        const res = await this.api.getWorkspaceAssets(ws.id, bucket.downloadSince, 200)
        const assets = res?.assets || []
        for (const asset of assets) {
          try { await this.downloadWorkspaceAsset(asset, ws) }
          catch (e) {
            console.warn(`[ws-poll ${ws.id}] ${asset.fileName}:`, e.message)
            // 일시 실패 → 재시도 큐에 추가 (중복 방지 assetId 체크)
            const already = bucket.retryQueue.some(q => q.asset?.id === asset.id)
              || bucket.permanentSkipped.some(q => q.asset?.id === asset.id)
            if (!already) {
              bucket.retryQueue.push({ asset, retryCount: 1, lastAttempt: new Date().toISOString(), lastError: e.message })
            }
          }
        }
        if (res?.nextSince) {
          bucket.downloadSince = res.nextSince
          this.saveState()
        }
      } catch (e) {
        // 에러 무조건 로그 (이전엔 DEBUG 플래그에 묶여 침묵 → 2번째 버그 원인).
        console.warn(`[ws-poll ${ws.id}] poll failed:`, e.message, e.stack?.split('\n').slice(0,3).join(' | '))
      }
    }

    const pollAllWorkspaces = async () => {
      for (const ws of this.workspaceManager.workspaces || []) {
        await pollOneWorkspace(ws)
      }
    }

    // 주기 refresh (팀 신규 합류 감지)
    const refreshMemberships = async () => {
      try { await this.workspaceManager.refresh() }
      catch (e) { if (process.env.ASSI_DEBUG_SYNC) console.warn('[ws-refresh]', e.message) }
    }

    // 첫 실행 시 즉시 refresh → polling (5분 대기 없음). Phase 2.5 beta.6 수정.
    refreshMemberships()
      .then(() => pollAllWorkspaces())
      .catch((e) => console.warn('[ws-poll] initial refresh failed:', e.message))
    this._wsPollTimer = setInterval(() => pollAllWorkspaces().catch(() => {}), WS_INTERVAL)
    this._wsRefreshTimer = setInterval(() => refreshMemberships().then(() => pollAllWorkspaces()).catch(() => {}), REFRESH_INTERVAL)
    if (this._wsPollTimer.unref) this._wsPollTimer.unref()
    if (this._wsRefreshTimer.unref) this._wsRefreshTimer.unref()
  }

  // 팀 자산 1개 로컬 다운로드. ~/ASSI - {팀명}/{카테고리}/{프로젝트명}/{파일명}
  async downloadWorkspaceAsset(asset, ws) {
    if (!asset.url || !asset.fileName || !asset.projectId) return
    const root = this.workspaceManager.getWorkspaceRoot(ws.id)
    const bucket = this.workspaceManager.getStateBucket(this.state, ws.id)
    if (!bucket.syncedFiles) bucket.syncedFiles = {}

    // 이미 받은 자산 — assetId 1차, contentHash 2차 (같은 파일이 새 assetId 로 재생성된 경우 대비)
    const alreadyKnownRelPath = Object.keys(bucket.syncedFiles).find(k => {
      const sf = bucket.syncedFiles[k]
      if (!sf) return false
      if (sf.assetId === asset.id) return true
      if (asset.contentHash && sf.contentHash === asset.contentHash) return true
      return false
    })
    if (alreadyKnownRelPath) return

    // size guards (OOM 방지 + 대용량 보류)
    if (asset.fileSize == null || asset.fileSize === 0) return
    const LARGE = 500 * 1024 * 1024
    if (asset.fileSize > LARGE) {
      if (!this.state.largeDownloadApproved?.[asset.id]) {
        this.onFileStatus({
          path: asset.fileName, status: 'pending', progress: 0, fileName: asset.fileName,
          size: fmt(asset.fileSize), phase: `대용량 ${fmt(asset.fileSize)} — 승인 필요`,
        })
        return
      }
    }

    // 프로젝트 정보 (workspace 소유 확인)
    // 실패 시 throw → 호출자의 retryQueue 가 잡아서 재시도. 일시 네트워크 오류 복구.
    let project
    try {
      project = await this.api.getProject(asset.projectId)
    } catch (e) {
      throw new Error(`project-fetch-failed: ${asset.projectId} (${e.message})`)
    }
    if (!project) throw new Error(`project-not-found: ${asset.projectId}`)
    if (project.ownerType === 'workspace' && project.ownerId !== ws.id) return

    const category = project.category || 'MISC'
    const projectName = project.name || asset.projectId
    const localDir = path.join(root, category, projectName)
    const localPath = path.join(localDir, asset.fileName)
    const relPath = path.relative(root, localPath).split(path.sep).join('/')

    // 해시 dedup — 같은 localPath 에 동일 해시 파일 이미 있으면 다운로드 스킵.
    // 단, 이 asset 을 bucket 에 등록은 해야 함 (안 그러면 같은 fileName 중복 asset
    // 이 영원히 syncedFiles 에 안 들어와서 missing 으로 잡힘).
    if (fs.existsSync(localPath) && asset.contentHash) {
      try {
        ...
        if (localHash === asset.contentHash) {
          const key = bucket.syncedFiles[relPath] ? `${relPath}#${asset.id}` : relPath
          bucket.syncedFiles[key] = {
            assetId: asset.id,
            ...
            downloaded: true,
            workspaceId: ws.id,
            aliasOf: key === relPath ? null : relPath,
          }
          this.saveState()
          return
        }
    ...
    this.onFileStatus({
      path: relPath, status: 'success', progress: 100, fileName: asset.fileName,
      size: asset.fileSize ? fmt(asset.fileSize) : '',
      phase: `팀 "${ws.name}" 다운로드 완료`,
    })
  }
  ...
  async downloadRemoteAsset(asset) {
    ...
    // ── Guard 1: 이 기기가 올린/이미 받은 자산이면 스킵 ──
    // 장비 A가 업로드한 직후 다음 폴링 때 자기 자산을 다시 받지 않도록.
    // assetId 1차, contentHash 2차 (같은 파일이 새 assetId 로 재생성된 경우 대비).
    const alreadyKnownRelPath = Object.keys(this.state.syncedFiles).find(k => {
      const sf = this.state.syncedFiles[k]
      if (!sf) return false
      if (sf.assetId === asset.id) return true
      if (asset.contentHash && sf.contentHash === asset.contentHash) return true
      return false
    ...
    // ── Guard 2: fileSize 알 수 없으면 보수적 스킵 (OOM 방지) ──
    // fetch → ArrayBuffer 가 메모리 다 올림. 크기 불명확하면 위험.
    // Bunny 인코딩 중이거나 메타 미완인 자산은 다음 tick 에 fileSize 채워진 후 처리.
    if (asset.fileSize == null || asset.fileSize === 0) {
      if (process.env.ASSI_DEBUG_SYNC) {
        console.log(`[Download] skip ${asset.fileName} — fileSize 불명, 다음 tick 에 재시도`)
      }
      return
    }

    // ── Guard 3: 대용량(>500MB) 다운로드는 명시 동의 전까지 보류 ──
    const LARGE_THRESHOLD = 500 * 1024 * 1024
    if (asset.fileSize > LARGE_THRESHOLD) {
  ...
  async handleDelete(filePath) {
    // ── 팀 폴더 파일 삭제 라우팅 (Phase 2.5 Stage D) ──
    if (this.workspaceManager) {
      try {
        const ctx = this.workspaceManager.resolveContext(filePath)
        if (ctx && ctx !== PERSONAL_CONTEXT) {
          const bucket = this.workspaceManager.getStateBucket(this.state, ctx)
          const root = this.workspaceManager.getWorkspaceRoot(ctx)
          const relToWs = path.relative(root, filePath).split(path.sep).join('/')
          const entry = (bucket.syncedFiles || {})[relToWs]
          if (!entry?.assetId) return
          try {
            // 서버 측에서 원 소유자 여부 판정 → trash 또는 unshare 자동 선택
            await this.api.deleteWorkspaceAsset(ctx, entry.assetId)
            delete bucket.syncedFiles[relToWs]
            this.saveState()
            this.onFileStatus({ path: relToWs, status: 'deleted', fileName: path.basename(filePath) })
          } catch (e) {
            this.onError({ message: `Team delete failed: ${relToWs}`, error: e.message })
          }
          return
        }
      } catch (e) { /* fall through to personal path */ }
    }
  ...
  stopDownloadPolling() {
    ...
    if (this._wsPollTimer) { clearInterval(this._wsPollTimer); this._wsPollTimer = null }
    if (this._wsRefreshTimer) { clearInterval(this._wsRefreshTimer); this._wsRefreshTimer = null }
  }
