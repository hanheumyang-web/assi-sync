...
class SyncEngine {
  ...
  async processShareUpload(share) {
    ...
    for (const asset of pending) {
      // state에서 assetId로 로컬 파일 경로 역추적
      const localPath = this.findLocalPath(asset.id)
      if (!localPath) {
        console.error(`[Share] Local file not found for asset ${asset.id} (${asset.fileName})`)
        continue
      }

      try {
        // 파일 크기 확인 (메모리에 올리지 않음)
        ...
        // R2 presigned URL로 PUT 업로드 (스트리밍 — 대용량 파일 OOM 방지)
  ...
  findLocalPath(assetId) {
    for (const [relPath, entry] of Object.entries(this.state.syncedFiles)) {
      if (entry.assetId === assetId) {
        return path.join(this.watchDir, ...relPath.split('/'))
      }
    }
    return null
  }
