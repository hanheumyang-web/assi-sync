  ...
  async processShareUpload(share) {
    ...
    for (const asset of pending) {
      ...
      try {
        ...
        // R2 presigned URL로 PUT 업로드 — https.request로 헤더 완전 제어
        const fileBuffer = fs.readFileSync(localPath)
        console.log(`[Share] PUT ${asset.fileName} size=${fileSize}`)

        const r2Result = await new Promise((resolve, reject) => {
          const url = new URL(asset.uploadUrl)
        ...
        if (r2Result.status >= 300) {
          throw new Error(`R2 ${r2Result.status} ${r2Result.body.slice(0, 100)}`)
        }
