  ...
  async processShareUpload(share) {
    ...
    for (const asset of pending) {
      ...
      try {
        ...
        // Content-Length는 fetch가 buffer에서 자동 계산 — 수동 설정 시 SignatureDoesNotMatch 발생 가능
        const fileBuffer = fs.readFileSync(localPath)
        const ct = asset.fileType || guessContentType(asset.fileName) || 'application/octet-stream'
        console.log(`[Share] PUT ${asset.fileName} size=${fileSize} contentType=${ct}`)
        ...
        if (!uploadRes.ok) {
          ...
          throw new Error(`R2 ${uploadRes.status} ${errText.slice(0, 100)}`)
        }
