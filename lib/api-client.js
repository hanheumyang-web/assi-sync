...
/* 서버가 거절하면 **사람 말**로 돌려준다 — 2026-09-25 검수.
   예전엔 `API error 402: {"error":"…"}` 를 통째로 화면에 찍었다. 서버는 이미 한국어로 이유를 보내고 있었다.
   code(project-limit 등)·limit·current 도 같이 실어, 앱이 무슨 거절인지 알아듣게 한다. */
async function 서버오류(res) {
  const text = await res.text().catch(() => '')
  let j = null
  try { j = JSON.parse(text) } catch (_) {}
  const err = new Error(j?.error || `서버 오류 (${res.status})`)
  err.status = res.status
  err.code = j?.code || null
  if (j?.limit != null) err.limit = j.limit
  if (j?.current != null) err.current = j.current
  return err
}
...
class ApiClient {
  ...
  async request(endpoint, body) {
    ...
    /* ⚠️ 1분 안에 답이 없으면 끊는다 (2026-09-11 검토) — 서버 호출이 멈추면 무압축 공유의 "도는 중" 표시가 풀리지 않아
          앱을 다시 켤 때까지 모든 공유가 멈췄다. 끊기면 오류로 돌아와 다음 확인에서 다시 한다. */
    ...
    if (res.status === 401) {
      ...
      if (!retry.ok) throw await 서버오류(retry)
      return retry.json()
    }

    if (!res.ok) throw await 서버오류(res)
    return res.json()
  }

  // ── 내 사이트 ──

  /* 첫 동기화가 끝나면 사이트까지 — 새로 만든 프로젝트 id 를 보내면 서버가 최신 12개를 공개하고 사이트를 발행한다 */
  async firstSiteFromSync(projectIds) {
    return this.request('firestore', { action: 'firstSiteFromSync', projectIds })
  }

  async getMySite() {
    return this.request('firestore', { action: 'getMySite' })
  }

  /* 웹을 로그인된 채 열기 위한 1회용 열쇠 (1시간) */
  async webLoginToken() {
    await this.ensureFreshToken()
    const r = await fetch('https://pofol.co/api/auth/webview-token', {
      method: 'POST', headers: { 'Authorization': `Bearer ${this.idToken}` }, signal: AbortSignal.timeout(15_000),
    })
    if (!r.ok) throw await 서버오류(r)
    return (await r.json()).customToken
  }

  // ── Projects ──

  /* folderKey = 감시 폴더 안의 상대 경로. 이름이 바뀌어도 짝을 잃지 않게 하는 열쇠다. */
  async findOrCreateProject(name, category, folderKey) {
    return this.request('firestore', {
      action: 'findOrCreateProject', name,
  ...
  // soft delete — 휴지통 보관 (30일 후 서버가 영구 삭제)
  async softDeleteProject(projectId) {
    return this.request('firestore', { action: 'softDeleteProject', projectId })
  }

  async restoreProject(projectId) {
    return this.request('firestore', { action: 'restoreProject', projectId })
  }

  // ── 휴지통 ──
  // 중복 자산 찾기 / 휴지통에 넣기 / 되돌리기. 넣어도 30일간은 되돌릴 수 있다.
  async findDuplicates(workspaceId) {
    return this.request('firestore', { action: 'findDuplicates', workspaceId: workspaceId || null })
  }

  async trashAssets(assetIds) {
    return this.request('firestore', { action: 'trashAssets', assetIds })
  }

  async listTrashedAssets() {
    return this.request('firestore', { action: 'listTrashedAssets' })
  }

  /* 웹을 비우고 폴더 기준으로 다시 올리기. apply 없이 부르면 세어만 본다. */
  async resetProjectsForResync({ apply = false } = {}) {
    return this.request('firestore', { action: 'resetProjectsForResync', apply, ...(apply ? { confirm: 'RESET' } : {}) })
  }

  async untrashAssets(assetIds) {
    return this.request('firestore', { action: 'untrashAssets', assetIds })
  }
  ...
  /* 사진 한 장을 읽는다 (표지가 이미 있는지 확인용) */
  async getAsset(assetId) {
    const r = await this.request('firestore', { action: 'getAsset', assetId })
    return r?.asset || null
  }
  ...
  // since 미지정 시 bootstrap 모드 (cursor 페이지네이션). 서버가 nextSince/nextCursor 돌려줌.
  async listAssetsSince(since, limit, cursor) {
    return this.request('firestore', { action: 'listAssetsSince', since, limit, cursor })
  }
  ...
  /* 올리다 실패한 빈 영상 자리를 치운다 — 안 치우면 Bunny 에 0바이트 영상이 쌓인다. */
  async deleteBunnyVideo(videoId) {
    return this.request('bunny', { action: 'deleteVideo', videoId })
  }
  ...
  /* pickedUp · attempts · failReason — 2026-09-11. 끝없이 다시 하지 않고, 웹이 "앱이 받아갔나" 를 알게 한다 */
  async updateShareProgress({ shareId, assetId, uploadStatus, uploadedCount, status, actualFileSize, pickedUp, attempts, failReason }) {
    return this.request('firestore', {
      action: 'updateShareProgress',
      shareId, assetId, uploadStatus, uploadedCount, status, actualFileSize, pickedUp, attempts, failReason,
    })
  }
