...
class ApiClient {
  ...
  async findOrCreateProject(name, category, workspaceId = null) {
  ...
  async deleteWorkspaceAsset(wsId, assetId) {
    return this.request('firestore', { action: 'deleteWorkspaceAsset', wsId, assetId })
  }
  ...
  async createAsset(data) {
    // sync-engine 이 _currentWsId 세팅해두면 wrapper 에서 자동 주입됨 (setAssetContextInjector 참조)
    if (this._assetContextInjector) data = this._assetContextInjector(data)
    return this.request('firestore', { action: 'createAsset', data })
  }

  /** sync-engine 에서 context 주입 함수 등록 (data → augmented data) */
  setAssetContextInjector(fn) { this._assetContextInjector = fn }
